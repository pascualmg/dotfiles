...
if ! systemctl is-active bluetooth.service &>/dev/null; then
	echo "<action=\`blueman-manager\`><fc=#444444><fn=1>󰂯</fn></fc></action>"
	exit 0
fi
...
if [ "$DEVICES" -eq 0 ]; then
	echo "<action=\`blueman-manager\`><fc=#444444><fn=1>󰂯</fn></fc></action>" # Sin dispositivos = gris
	exit 0
elif [ "$DEVICES" -eq 1 ]; then
	NAME=$(echo "" | bluetoothctl devices Connected 2>/dev/null | head -1 | cut -d' ' -f3-)
	echo "<action=\`blueman-manager\`><fc=${COLOR_BLUE}>󰂯 ${NAME}</fc></action>"
else
	echo "<action=\`blueman-manager\`><fc=${COLOR_BLUE}>󰂯 ${DEVICES}</fc></action>"
fi
