...
**greedyView (config actual):** Los workspaces se INTERCAMBIAN
...
**** Cambiar entre greedyView y view

Para probar =view= en lugar de =greedyView=, edita =xmonad.hs=:

#+BEGIN_SRC haskell
-- Cambiar esto (en prevWS' y nextWS'):
windows $ W.greedyView (sorted !! (i - 1))

-- Por esto:
windows $ W.view (sorted !! (i - 1))
#+END_SRC
...
|-----------+---------------|
| Monitor único (macbook) | Config actual (greedyView) |
| Ultrawide (aurin) | Config actual + ThreeColMid |
| Dual monitor trabajo | PhysicalScreens (M-w/M-e) |
| Quieres workspaces fijos por monitor | IndependentScreens |
| Te confunde el swap de workspaces | Cambiar a =view= |
