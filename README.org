...
| Volumen | =xmobar-volume.sh= | 󰕾 | Nivel PipeWire con gradiente | Siempre visible |
...
| Red | =xmobar-network.sh= | 󰈀 | IP principal (eth/wifi) | Siempre visible |
| Discos | =xmobar-disks.sh= | 󰋊 | Uso % + temperatura NVMe | Siempre visible |
| GPU | =xmobar-gpu.sh= | 󰢮 | Uso, temp, VRAM, watts | Auto-detecta NVIDIA/Intel/AMD |
| RAM | =xmobar-memory.sh= | 󰍛 | Uso con gradiente | Siempre visible |
| CPU Freq | =xmobar-cpu-freq.sh= | 󰾅 | Frecuencia + governor | Siempre visible |
| CPU | =xmobar-cpu.sh= | 󰻠 | Uso % + temperatura | Siempre visible |
