...
**** Crush: Sucesor de OpenCode (via NUR)

Crush es mantenido por *Charmbracelet* (creadores de Bubble Tea, Gum, etc.).
Tiene su propio NUR oficial con módulos para NixOS y Home Manager.

***** Instalación via NUR Charmbracelet

****** Paso 1: Añadir input en flake.nix
...
****** Paso 2: Pasar a módulos via specialArgs

#+BEGIN_SRC nix
outputs = { self, nixpkgs, charm-nur, ... }@inputs:
{
  nixosConfigurations.aurin = nixpkgs.lib.nixosSystem {
    system = "x86_64-linux";
    specialArgs = { inherit charm-nur; };  # <- Pasar a módulos
    modules = [
      charm-nur.nixosModules.crush  # <- Módulo NixOS
      ./hosts/aurin
    ];
  };
};
...
****** Paso 3: Habilitar en host config
...
****** Alternativa: Home Manager

#+BEGIN_SRC nix
# En flake.nix con home-manager integrado
...
|--------+------+-------------|
| =programs.crush.enable= | bool | Habilitar crush |
| =programs.crush.package= | package | Override del paquete |
| =programs.crush.settings= | attrs | Configuración JSON (providers, lsp, mcp) |

***** Ejemplo settings con LSP
