#+TITLE: Dotfiles - passh (Multi-Machine NixOS)
#+AUTHOR: passh
#+DATE: 2026-01-23
#+STARTUP: overview

* La Filosofía HHKB Aplicada al Sistema Operativo

** La Historia

La primera vez que probé un HHKB (Happy Hacking Keyboard), algo hizo clic. No fue inmediato - los primeros días maldecía la distribución, buscaba teclas que no estaban donde esperaba. Pero después de una semana, algo cambió. Mis dedos sabían dónde ir sin pensar. El teclado dejó de ser un periférico y se convirtió en *una extensión de mis manos*.

Ahora llevo el HHKB a todas partes: desktop, portátil, oficina. El hardware cambia (PC, Mac, Linux), pero la interfaz con la que pienso permanece. Mis manos no tienen que reaprender.

Entonces pensé: *¿Y si pudiera hacer lo mismo con TODO mi entorno?*

No solo el teclado - también mi shell, mis aliases de Git, mi configuración de Vim, mis keybindings de XMonad, mis herramientas de desarrollo. Todo.

*El HHKB se convirtió en una extensión de mis manos.*
*NixOS se convirtió en una extensión de mi cerebro.*

#+BEGIN_QUOTE
"When America's cowboys were in the middle of a trip and their horse died, they would leave the horse there. But even if they were in the middle of a desert, they would take their saddle with them. The horse was a consumable good, but the saddle was an interface that their bodies had gotten used to. In the same vein, PCs are consumable goods, while keyboards are important interfaces."

— Eiiti Wada, creador del Happy Hacking Keyboard
#+END_QUOTE

*Y si pudieras llevarte no solo la montura (el teclado), sino también las riendas, el mapa del camino, y la forma en que piensas sobre el viaje?*

** El Problema que Todos Hemos Vivido

Imagina estos escenarios (que has vivido 1000 veces):

1. *Formatear / Reinstalar el sistema*
   - "Mierda, ¿cuál era el plugin de VSCode para formatear?"
   - "¿Cómo coño se configuraba el Dockerfile para el proyecto?"
   - "Tenía un alias de Git que era la hostia... ¿cuál era?"
   - "Las extensiones de Chrome, los favoritos, las sesiones guardadas..."
   - *4 horas después:* "Creo que ya está... pero me falta algo y no sé qué"

2. *Nuevo portátil (del trabajo, personal, o porque el viejo murió)*
   - Teams, Slack, Zoom, Discord
   - Docker, Docker Compose, Kubernetes
   - IntelliJ IDEA, VSCode, Postman, DBeaver, pgAdmin
   - Node, Python, Go, Rust (con sus versiones específicas)
   - Nginx, PostgreSQL, Redis, MongoDB
   - Git config (nombre, email, aliases), SSH keys, GPG keys
   - Fuentes (Nerd Fonts), temas, wallpapers, iconos
   - *2-3 DÍAS perdidos* instalando y configurando
   - *1 semana más* recordando "ah, me faltaba esto"

3. *Desktop potente en casa + Portátil + Servidor en la nube*
   - Cada máquina configurada de forma diferente
   - "En el desktop funcionaba, en el portátil no... ¿qué cambiará?"
   - Instalas algo nuevo → tienes que acordarte de instalarlo en las otras 2
   - Actualizas config → ¿en cuál de las 3 era la versión buena?
   - *Caos absoluto* - nunca sabes qué funciona dónde

** La Solución: Sistema Operativo Declarativo (NixOS)

#+BEGIN_SRC bash
# Máquina nueva (desktop gaming con RTX 5080)
git clone https://github.com/tu-usuario/dotfiles ~/dotfiles
sudo nixos-rebuild switch --flake ~/dotfiles#aurin

# 20 minutos después:
# ✅ Nvidia drivers configurados
# ✅ Docker + compose funcionando
# ✅ IntelliJ IDEA con tus plugins
# ✅ Fish shell con tu config
# ✅ XMonad con tus keybindings
# ✅ Claude AI agent con tus custom agents
# ✅ Crush AI apuntando a tu Ollama
# ✅ Alacritty con tu colorscheme
# ✅ TODO exactamente como lo tenías

# Portátil (MacBook Pro)
git pull  # Actualizas cambios del desktop
sudo nixos-rebuild switch --flake ~/dotfiles#macbook

# 15 minutos después:
# ✅ MISMA configuración
# ✅ MISMO entorno
# ✅ Solo difiere en hardware (HiDPI, batería, WiFi)
#+END_SRC

** El Momento "OMG wtf is magic 🪄 XD!" - Caso Real de Esta Semana

*Viernes noche, 23 enero 2026:*

El ejemplo viene por que es con lo que estoy jugando esta semana, probando todos las uis alternativas a claude code y a ver si por fin alguna me deja conectarme en local con ollama. el Hype es el Hype :)
1. *En desktop (aurin)*: "Voy a probar Crush, un nuevo AI agent"
   - Creo =crush.nix= (90 líneas)
   - Añado config =crush.json= con Ollama local
   - =git commit && git push=

2. *En MacBook (10 minutos después)*:
   - =git pull=
   - =sudo nixos-rebuild switch=
   - *Magia*: Crush instalado, MISMA config
   - Como la config apunta a =campo.zapto.org:11434= (mi casa)...
   - *¡Uso la GPU 5080 de mi workstation (con mis 72 cores chinos ) desde el portátil!* 🤯

3. *Lo que NO tuve que hacer*:
   - ❌ Reinstalar Crush en Mac
   - ❌ Reconfigurar providers/models
   - ❌ Copiar API keys manualmente
   - ❌ Recordar qué versión instalé en aurin
   - ✅ Solo: =git pull && rebuild=

** No Es Solo "Dotfiles" - Es Tu Sistema Completo

#+BEGIN_SRC nix
# modules/home-manager/programs/ai-agents/crush.nix
# 90 líneas que instalan Crush en TODAS tus máquinas
let
  charm-nur = builtins.fetchGit {
    url = "https://github.com/charmbracelet/nur";
    ref = "master";
  };
  crush = pkgs.callPackage "${charm-nur}/pkgs/crush" {};
in
{
  home.packages = [ crush ];
  # Config symlinkeado desde ~/dotfiles/crush/.config/crush/crush.json
  # Editas UNA VEZ → funciona en TODAS las máquinas
}
#+END_SRC

** El Patrón: Clone-First Architecture

*REGLA DE ORO:* Todas las máquinas son CLONES IDÉNTICOS. Solo difieren en hardware.

#+BEGIN_EXAMPLE
dotfiles/
├── modules/base/           ← COMÚN a TODAS las máquinas
│   ├── desktop.nix         (XMonad, GNOME, LightDM)
│   └── virtualization.nix  (Docker, libvirt)
├── hardware/               ← Hardware-ESPECÍFICO
│   ├── nvidia/rtx5080.nix  (aurin)
│   └── apple/macbook-pro.nix (macbook)
└── hosts/                  ← Overrides MÍNIMOS
    ├── aurin/              (VPN Vocento, FiiO DAC)
    └── macbook/            (casi vacío - todo viene de base/)

AI Agents (añadir nuevo = 30 líneas):
├── claude-code.nix         (85 líneas)
├── crush.nix               (90 líneas)
└── opencode.nix            (83 líneas)

¿Quieres aider? → Creas aider.nix (30 líneas) + 1 import
¿Quieres cursor? → Creas cursor.nix (30 líneas) + 1 import
¿Quieres foo? → Creas foo.nix (XX líneas) + 1 import y ya esta :)

#+END_EXAMPLE

** Para Quién Es Esto

*Este repo es para ti si:*

1. *Tienes múltiples máquinas* (desktop + laptop + servidor)
2. *Odias reinstalar/reconfigurar* herramientas
3. *Quieres reproducibilidad* ("funcionaba ayer, ¿por qué no hoy?")
4. *Valoras tu tiempo* más que 2 días configurando Teams/Slack/Docker
5. *Te gusta la filosofía HHKB*: tu entorno es TU montura, llévala contigo

*NO es para ti si:*

- Usas Windows/Mac y no quieres cambiar (aunque... ¿seguro?) 
- "Me gusta configurar todo a mano cada vez" (respetable, pero raro)
- Tienes miedo a =git= y archivos de texto (tranqui, hay tutoriales)

** El Secreto: LLM como Asistente Experto

*Este repo está diseñado para trabajar con LLMs* (Claude, ChatGPT, etc.):

#+BEGIN_SRC text
Tú: "Quiero añadir Neovim con mi config personalizada"

LLM: *Lee README.org + ai-agents/ pattern*
     "Voy a crear neovim.nix siguiendo el patrón:
     
     1. fetchGit de tu config (si está en GitHub)
     2. Symlink desde ~/dotfiles/nvim/
     3. Añadir import en default.nix
     4. Rebuild en todas las máquinas
     
     ¿Continúo?"

*5 minutos después:* Neovim configurado en desktop, laptop y servidor.
#+END_SRC

*Patrones documentados:*
- ✅ Cómo añadir AI agents (fetchGit + callPackage)
- ✅ Cómo gestionar configs (symlinks editables)
- ✅ Cómo manejar licencias unfree (ejemplo: Crush)
- ✅ Cómo separar hardware vs config base
- ✅ Clone-first philosophy (todo en comments)

** Empieza Aquí

1. *Curiosea el repo* - lee los módulos, están comentados a muerte
2. *Forkea y adapta* - cambia =passh= por tu usuario, ajusta hardware
3. *Pregunta a un LLM* - "¿Cómo añado X siguiendo el patrón de este repo?"
4. *Disfruta* - nunca más "joder, ¿cómo configuré esto?"

#+BEGIN_QUOTE
*"No es solo un sistema operativo declarativo.*
*Es llevarte tu caballo, tu montura, tus riendas y el mapa del camino."*

— La filosofía del repo, 2026
#+END_QUOTE

---

* TL;DR - Setup Rapido

#+BEGIN_SRC bash
# Clonar dotfiles
git clone <repo> ~/dotfiles
cd ~/dotfiles

# Aplicar NixOS config (FLAKES - metodo actual)
# IMPORTANTE: Ejecutar 'hostname' primero para saber en que maquina estas
# IMPORTANTE: Todas las maquinas necesitan --impure (ver seccion abajo)

# Aurin
sudo nixos-rebuild switch --flake ~/dotfiles#aurin --impure

# MacBook
sudo nixos-rebuild switch --flake ~/dotfiles#macbook --impure

# Vespino
sudo nixos-rebuild switch --flake ~/dotfiles#vespino --impure

# Test sin aplicar cambios permanentes
sudo nixos-rebuild test --flake ~/dotfiles#aurin --impure

# Rollback de emergencia
sudo nixos-rebuild switch --rollback
#+END_SRC

** ¿Por qué --impure? (Pure vs Impure en Nix)

*** El Concepto

Nix es *reproducible* por defecto: dado el mismo input, produce el mismo output.
Esto se llama *evaluación pura* (pure evaluation).

*Impure* significa que el resultado puede variar dependiendo de factores externos.

*** Cuándo necesitas --impure

| Caso | Ejemplo | Por qué es impure |
|------+---------+-------------------|
| *Leer archivos externos* | =builtins.readFile "/etc/hosts"= | El archivo puede cambiar |
| *fetchGit con branch* | =ref = "master"= | La branch cambia con commits |
| *Paths fuera del flake* | =./../../archivo.txt= | No está en el flake lockfile |
| *Variables de entorno* | =builtins.getEnv "HOME"= | Depende del sistema |

*** En este repo usamos --impure por:

1. *Hosts Vocento* (aurin/vespino):
   #+BEGIN_SRC nix
   # hosts/aurin/default.nix
   networking.extraHosts = builtins.readFile "/home/passh/src/vocento/autoenv/hosts_all.txt";
   #+END_SRC
   Lee un archivo externo al flake → impure.

2. *Crush NUR* (todas las máquinas):
   #+BEGIN_SRC nix
   # modules/home-manager/programs/ai-agents/crush.nix
   charm-nur = builtins.fetchGit {
     url = "https://github.com/charmbracelet/nur";
     ref = "master";  # ← Branch que cambia = IMPURE
   };
   #+END_SRC
   Fetch de una branch que evoluciona → impure.

*** Cómo hacer fetchGit PURE (si quisieras)

#+BEGIN_SRC nix
# IMPURE - branch que cambia
charm-nur = builtins.fetchGit {
  url = "https://github.com/charmbracelet/nur";
  ref = "master";  # ← Cambia con nuevos commits
};

# PURE - commit fijo (hash inmutable)
charm-nur = builtins.fetchGit {
  url = "https://github.com/charmbracelet/nur";
  rev = "a1b2c3d4e5f6...";  # ← Siempre el mismo contenido
};
#+END_SRC

*Trade-off:*
- =ref = "master"= → Siempre última versión, pero necesita --impure
- =rev = "hash"= → Reproducible sin --impure, pero hay que actualizar manualmente

*** Resumen

| Modo | Comando | Cuándo usar |
|------+---------+-------------|
| *Pure* | =nixos-rebuild switch --flake .#host= | Cuando TODO está en flake.lock |
| *Impure* | =nixos-rebuild switch --flake .#host --impure= | Cuando lees archivos externos o fetch branches |

*En este repo:* Siempre usa =--impure=. Es seguro y necesario.

* 🚀 Use This Repo (Work in Progress)

*NOTA:* Este repo está diseñado para ser *forkeable*, pero actualmente tiene valores hardcoded (username =passh=, paths, info personal). Si quieres usarlo como base para tu sistema, sigue esta guía y adapta lo necesario.

** Quick Start for Others

*** 1. Fork & Clone

#+BEGIN_SRC bash
# Fork este repo en GitHub primero, luego:
git clone https://github.com/TU_USUARIO/dotfiles ~/dotfiles
cd ~/dotfiles
#+END_SRC

*** 2. Generar Hardware Config en Tu Máquina

#+BEGIN_SRC bash
# En tu máquina nueva (booted con NixOS ISO):
sudo nixos-generate-config --show-hardware-config > \
  hardware-configuration.nix

# Crear directorio para tu host
mkdir -p hosts/TU_MAQUINA
mv hardware-configuration.nix hosts/TU_MAQUINA/
#+END_SRC

*** 3. Crear Host Config

#+BEGIN_SRC bash
# Copia un template existente (macbook es el más simple)
cp hosts/macbook/default.nix hosts/TU_MAQUINA/default.nix
vim hosts/TU_MAQUINA/default.nix
#+END_SRC

Adapta según tu hardware:
- =imports= - Elige módulos hardware (NVIDIA, Apple, etc.)
- =services= - Habilita/deshabilita según necesites
- =networking.hostName= - Se configura automáticamente en flake

*** 4. Añadir al flake.nix

#+BEGIN_SRC nix
# En flake.nix, añade tu máquina:
nixosConfigurations.TU_MAQUINA = mkSystem {
  hostname = "TU_MAQUINA";
  hardware = [
    # Elige los que necesites:
    # ./hardware/nvidia/rtx5080.nix
    # ./hardware/apple/macbook-pro-13-2.nix
    # etc.
  ];
};
#+END_SRC

*** 5. ⚠️ IMPORTANTE: Personalizar Valores Hardcoded

*CRÍTICO:* Estos archivos tienen info personal que DEBES cambiar:

#+BEGIN_SRC text
[ ] flake.nix
    └─ users.passh → users.TU_USUARIO

[ ] modules/home-manager/core.nix
    ├─ username = "passh" → "TU_USUARIO"
    ├─ homeDirectory = "/home/passh" → "/home/TU_USUARIO"
    └─ programs.git.userEmail/userName → TU info

[ ] claude-code/.claude/CLAUDE.md
    ├─ Eliminar: Red local (IPs, passwords)
    ├─ Eliminar: GitHub pascualmg → TU_USUARIO
    └─ Actualizar: Preferencias personales

[ ] claude-code/.claude/agents/
    └─ Eliminar agentes personales:
       - vocento-software-architect.md (específico empresa)
       - carlos-buenosvinos.md (personal)
       - chema.md (personal)
       - escriba.md (personal)

[ ] hosts/aurin/default.nix (si copias)
    └─ Eliminar: VPN Vocento, hosts corporativos

[ ] .gitignore
    └─ Añadir: tus secrets (.credentials.json, etc.)
#+END_SRC

*** 6. Build & Install

#+BEGIN_SRC bash
# Build
sudo nixos-rebuild switch --flake ~/dotfiles#TU_MAQUINA

# Si falla, verifica:
nix flake check --show-trace
#+END_SRC

** What You Get

| Feature | Status |
|---------+--------|
| *Clone-first architecture* | ✅ Modular (base + hardware + host) |
| *Desktop ready* | ✅ GNOME + XMonad + LightDM configured |
| *AI agents (claude-code)* | ✅ Nixified, portable, customizable |
| *Home-manager* | ✅ 200+ packages, declarative configs |
| *Multi-machine support* | ✅ Easy to add new machines |
| *Hardware support* | ✅ NVIDIA, Apple, pro audio modules |
| *Wayland compositors* | ✅ Hyprland, niri, GNOME Wayland (SDDM) |
| *Plug-and-play* | ❌ WIP - requires manual adaptation |

** What's Personal (Must Customize)

| Item | Location | Action Required |
|------+----------+-----------------|
| *Username* | =flake.nix=, =core.nix= | Change "passh" to yours |
| *Git config* | =core.nix= | Update email/name |
| *CLAUDE.md* | =claude-code/.claude/= | Remove IPs/passwords/personal info |
| *AI agents* | =claude-code/.claude/agents/= | Remove personal agents |
| *VPN Vocento* | =hosts/aurin/=, =modules/services/= | Remove if not applicable |
| *Hosts file* | =hosts/aurin/default.nix= | Remove corporate hosts |
| *Secrets* | Various | Add to =.gitignore= |

** Roadmap: Full Template Support

*Current state:* Functional but requires manual adaptation

*Planned improvements* (after stow elimination + opencode integration):
- [ ] Parametrized username (=mainUser= variable)
- [ ] Template generator script (=./scripts/new-machine.sh=)
- [ ] Secrets management (sops-nix or agenix)
- [ ] Clean separation: personal vs. template
- [ ] Testing with real users (feedback loop)

*ETA:* When stow migration complete (~2-3 weeks)

** Community & Contributions

Si usas este repo y mejoras algo:

1. Fork & create branch: =feat/FEATURE=
2. Keep the clone-first philosophy
3. Document changes (README + comments)
4. Submit Pull Request

*Feedback wanted:* Si eres early adopter, abre un issue con tu experiencia.

** Support

*Issues:* [[https://github.com/pascualmg/dotfiles/issues]]
*Discussions:* [[https://github.com/pascualmg/dotfiles/discussions]]

*Philosophy:* This repo prioritizes *clarity* and *modularity* over convenience.
You'll need to understand NixOS basics to adapt it successfully.

* Arquitectura Clone-First (2026-01-19)

** Filosofia

*Todas las maquinas son CLONES IDENTICOS.* Solo se diferencian en hardware.

La configuracion se divide en 3 capas:

#+BEGIN_SRC text
modules/base/       -> Config comun a TODAS las maquinas (desktop, servicios, etc.)
hardware/           -> Config especifica de hardware (nvidia, apple, audio)
hosts/*/            -> Solo servicios y overrides especificos de cada maquina
#+END_SRC

** Estructura del Proyecto

#+BEGIN_SRC text
dotfiles/
|-- flake.nix                    * Entry point (mkSystem para clone-first)
|-- modules/
|   |-- base/                    * BASE UNIFICADA (todas las maquinas)
|   |   |-- default.nix          # Imports: boot, nix-settings, packages, desktop, etc.
|   |   |-- desktop.nix          # GNOME + XMonad (sesiones)
|   |   |-- sddm.nix             # SDDM display manager (X11 + Wayland)
|   |   |-- sunshine.nix         # Streaming server (opcional)
|   |   `-- virtualization.nix   # Docker + libvirt
|   |-- common/                  . Modulos legacy (usados por mkNixosConfig)
|   |-- desktop/                 * Wayland: hyprland.nix, niri.nix
|   `-- home-manager/            * Configuracion usuario passh
|-- hardware/                    * MODULOS HARDWARE
|   |-- nvidia/
|   |   |-- rtx5080.nix          # Aurin: RTX 5080 (open drivers)
|   |   `-- rtx2060.nix          # Vespino: RTX 2060
|   |-- apple/
|   |   |-- macbook-pro-13-2.nix # MacBook: keyd, HiDPI, bateria
|   |   `-- snd-hda-macbookpro.nix # MacBook: audio CS8409
|   `-- audio/
|       `-- fiio-k7.nix          # Aurin: DAC/AMP FiiO K7
|-- hosts/                       * HOST-SPECIFIC (solo overrides)
|   |-- aurin/
|   |   |-- hardware-configuration.nix
|   |   `-- default.nix          # VPN bridge, hosts Vocento, etc.
|   |-- macbook/
|   |   |-- hardware-configuration.nix
|   |   `-- default.nix          # Casi vacio (todo viene de base + hardware)
|   `-- vespino/
|       |-- hardware-configuration.nix
|       |-- default.nix          # NFS, Minecraft, VPN VM
|       `-- minecraft.nix        # Servidor Minecraft
|-- xmonad/                      * XMonad config (stow)
|-- nixos-*/                     . Legacy (pre-flakes, backup)
`-- README.org                     Este archivo
#+END_SRC

*Leyenda:* =* Activo= | =. Legacy/migrado=

** Como se construye cada maquina

#+BEGIN_SRC nix
# En flake.nix
aurin = mkSystem {
  hostname = "aurin";
  hardware = [
    ./hardware/nvidia/rtx5080.nix
    ./hardware/audio/fiio-k7.nix
  ];
};

macbook = mkSystem {
  hostname = "macbook";
  hardware = [
    nixos-hardware.nixosModules.apple-macbook-pro
    ./hardware/apple/macbook-pro-13-2.nix
    ./hardware/apple/snd-hda-macbookpro.nix
  ];
};

vespino = mkSystem {
  hostname = "vespino";
  hardware = [
    ./hardware/nvidia/rtx2060.nix
  ];
};
#+END_SRC

** Maquinas

| Maquina | Hardware | Rol | Display Manager |
|---------+----------+-----+-----------------|
| aurin   | Dual Xeon E5-2699v3, RTX 5080, 128GB | Produccion, desarrollo | SDDM |
| macbook | MacBook Pro 2016, Intel, HiDPI | Laptop, movilidad | SDDM |
| vespino | AMD CPU, RTX 2060 | Testing, Minecraft, NFS | SDDM |

** VPN Vocento (Ivanti/Pulse Secure) - EXPERIMENTAL [2026-01-21]

*** El Problema

Vocento usa *Ivanti VPN* (Pulse Secure) que NO funciona nativamente en NixOS.
El cliente propietario tiene dependencias imposibles de satisfacer.

*** La Solucion: VM como Gateway VPN

#+BEGIN_SRC text
┌─────────────────────────────────────────────────────────────────────────┐
│                              HOST (NixOS)                               │
│                                                                         │
│   ┌──────────────────┐         ┌──────────────┐       ┌──────────────┐ │
│   │  Interface Ext.  │         │    br0       │       │  VM Ubuntu   │ │
│   │ (WiFi/Ethernet)  │   NAT   │192.168.53.10 │◄─────►│192.168.53.12 │ │
│   │                  │◄───────►│   Bridge     │       │              │ │
│   └────────┬─────────┘         └──────────────┘       │ ┌──────────┐ │ │
│            │                          ▲               │ │ Ivanti   │ │ │
│            ▼                          │               │ │ Client   │ │ │
│       Internet                   Rutas corp.          │ └────┬─────┘ │ │
│                                  via VM               │      │tun0   │ │
│                                                       └──────┼───────┘ │
└───────────────────────────────────────────────────────────────┼─────────┘
                                                                ▼
                                                        Redes Vocento
                                                        10.180.0.0/16
                                                        10.182.0.0/16
                                                        192.168.196.0/24
                                                        etc...
#+END_SRC

*** Modulos NixOS

Dos modulos declarativos que trabajan juntos:

| Modulo | Funcion | Ubicacion |
|--------+---------+-----------|
| =vocento-vpn-bridge.nix= | Bridge br0, NAT, rutas, DNS | =modules/services/= |
| =ivanti-vpn-vm.nix= | VM libvirt declarativa | =modules/services/= |

**** vocento-vpn-bridge.nix

Configura toda la red del host:

#+BEGIN_SRC nix
services.vocento-vpn-bridge = {
  enable = true;
  externalInterface = "wlp0s20f0u7u4";  # Interface con internet
  # hostAddress = "192.168.53.10";       # IP del host en bridge (default)
  # vmAddress = "192.168.53.12";         # IP de la VM (default)
};
#+END_SRC

*Que hace:*
- Crea bridge =br0= con IP =192.168.53.10/24=
- Configura NAT (MASQUERADE) para que la VM salga a internet
- Añade rutas estaticas: =10.180.0.0/16 via 192.168.53.12=, etc.
- Configura DNS apuntando a servidores Vocento
- Añade script =vpn-bridge-status= para diagnostico

**** ivanti-vpn-vm.nix

Define la VM de forma declarativa:

#+BEGIN_SRC nix
services.ivanti-vpn-vm = {
  enable = true;
  networkMode = "bridge";        # "nat" para testing, "bridge" para produccion
  vmAddress = "192.168.53.12";   # IP fija de la VM
};
#+END_SRC

*Que hace:*
- Genera XML de libvirt embebido en Nix
- Servicio systemd que define la VM al boot
- Script =vpn-vm= con comandos: start, stop, ssh, setup-network

*** Comandos Disponibles

#+BEGIN_SRC bash
# Gestion de la VM
vpn-vm start         # Arranca VM + abre virt-viewer
vpn-vm stop          # Apaga la VM
vpn-vm ssh           # SSH a la VM
vpn-vm status        # Estado y conectividad
vpn-vm setup-network # Configura IP en disco (VM apagada)

# Diagnostico del bridge
vpn-bridge-status    # Estado completo del sistema VPN
#+END_SRC

*** Reproducir en Equipo Nuevo

#+BEGIN_SRC bash
# 1. Copiar disco de la VM (17GB) desde maquina existente
scp origen:/var/lib/libvirt/images/ivanti-vpn-clone.qcow2 \
    /var/lib/libvirt/images/

# 2. Configurar en hosts/NUEVO/default.nix:
#+END_SRC

#+BEGIN_SRC nix
imports = [
  ../../modules/services/ivanti-vpn-vm.nix
  ../../modules/services/vocento-vpn-bridge.nix
];

services.ivanti-vpn-vm = {
  enable = true;
  networkMode = "bridge";
};

services.vocento-vpn-bridge = {
  enable = true;
  externalInterface = "enp0s31f6";  # <-- CAMBIAR a tu interface
};
#+END_SRC

#+BEGIN_SRC bash
# 3. Rebuild
sudo nixos-rebuild switch --flake ~/dotfiles#NUEVO

# 4. Configurar red en la VM (solo primera vez, automatico!)
vpn-vm setup-network

# 5. Arrancar y conectar
vpn-vm start
# -> En la VM: conectar cliente Ivanti
# -> Listo! bitbucket.vocento.com accesible desde el host
#+END_SRC

*** Estado: EXPERIMENTAL

| Aspecto | Estado |
|---------+--------|
| Funciona en macbook | ✓ Probado |
| Funciona en vespino | ⏳ Pendiente probar |
| Funciona en aurin | ⏳ NO migrar (usar snapshot existente) |
| Reproducible | ✓ Modulos declarativos |
| Disco VM en Nix | ✗ Manual (QCOW2 externo) |

*** Notas del NixOS Guru

*Puntos fuertes:*
- Estructura modular correcta
- Tipos bien definidos (submodules para rutas)
- Scripts helper muy utiles
- Documentacion en cabeceras

*Mejoras sugeridas:*
- Añadir assertions para dependencias
- Hacer UUID de VM configurable
- Cuidado con referencias circulares en defaults

*Seguridad:*
- =checkReversePath = false= necesario pero riesgoso
- =--impure= requerido si usas hostsFile

*** Arquitectura Detallada

#+BEGIN_SRC text
                    MODULOS NIX
                    ───────────
┌─────────────────────────────────────────────────────────┐
│ modules/services/vocento-vpn-bridge.nix                │
│                                                         │
│  options:                                              │
│    - enable                                            │
│    - externalInterface (requerido)                     │
│    - hostAddress (default: 192.168.53.10)              │
│    - vmAddress (default: 192.168.53.12)                │
│    - vpnRoutes (lista de redes corporativas)           │
│    - dnsServers (default: DNS Vocento + 8.8.8.8)       │
│                                                         │
│  configura:                                            │
│    - networking.bridges.br0                            │
│    - networking.interfaces.br0.ipv4.routes             │
│    - networking.nat                                    │
│    - environment.etc."resolv.conf"                     │
└─────────────────────────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ modules/services/ivanti-vpn-vm.nix                     │
│                                                         │
│  options:                                              │
│    - enable                                            │
│    - networkMode ("nat" | "bridge")                    │
│    - vmAddress                                         │
│    - hostAddress                                       │
│                                                         │
│  configura:                                            │
│    - systemd.services.define-ivanti-vpn-vm             │
│    - environment.systemPackages [vpn-vm, guestfs-tools]│
│                                                         │
│  genera:                                               │
│    - XML libvirt con bridge o NAT segun modo           │
└─────────────────────────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
│ hosts/macbook/default.nix                              │
│                                                         │
│  services.ivanti-vpn-vm.enable = true;                 │
│  services.ivanti-vpn-vm.networkMode = "bridge";        │
│  services.vocento-vpn-bridge.enable = true;            │
│  services.vocento-vpn-bridge.externalInterface = "...";│
└─────────────────────────────────────────────────────────┘
#+END_SRC

*** Troubleshooting

| Problema | Solucion |
|----------+----------|
| VM sin internet | Verificar NAT: =sudo iptables -t nat -L= |
| Host no resuelve DNS Vocento | Verificar ruta a DNS: =ping 192.168.201.38= |
| No llega a redes corporativas | Verificar VPN conectada en VM: =vpn-vm ssh= + =ip route= |
| Bridge no existe | =sudo systemctl start br0-netdev.service= |
| VM no arranca | Verificar disco: =ls /var/lib/libvirt/images/*.qcow2= |

*** Arbol del Proyecto VPN

#+BEGIN_SRC text
dotfiles/
├── flake.nix                         # Entry point - define las 3 maquinas
│
├── modules/
│   ├── base/
│   │   └── virtualization.nix        # Habilita libvirtd (REQUERIDO)
│   │
│   └── services/
│       ├── vocento-vpn-bridge.nix    # [VPN] Bridge + NAT + Rutas + DNS
│       │   ├── options:
│       │   │   ├── enable
│       │   │   ├── externalInterface  ← CAMBIAR segun maquina
│       │   │   ├── hostAddress        (192.168.53.10)
│       │   │   ├── vmAddress          (192.168.53.12)
│       │   │   └── vpnRoutes          (10.180.0.0/16, etc.)
│       │   └── configura:
│       │       ├── networking.bridges.br0
│       │       ├── networking.nat
│       │       └── /etc/resolv.conf
│       │
│       └── ivanti-vpn-vm.nix         # [VPN] VM declarativa en libvirt
│           ├── options:
│           │   ├── enable
│           │   ├── networkMode        ("nat" | "bridge")
│           │   └── vmAddress
│           └── configura:
│               ├── systemd.services.define-ivanti-vpn-vm
│               └── scripts: vpn-vm
│
├── hosts/
│   ├── aurin/default.nix             # NO USA modulos VPN (tiene su propia config)
│   ├── macbook/default.nix           # USA modulos VPN ✓
│   └── vespino/default.nix           # PENDIENTE probar modulos VPN
│
└── /var/lib/libvirt/images/
    └── ivanti-vpn-clone.qcow2        # Disco VM (17GB) - EXTERNO AL REPO
#+END_SRC

*** Disaster Recovery: Recrear VM desde Cero

Si pierdes el disco QCOW2, aqui esta como recrear la VM:

**** 1. Descargar Ubuntu 22.04 LTS

#+BEGIN_SRC bash
# Desktop (tiene GUI para el cliente Ivanti)
wget https://releases.ubuntu.com/22.04/ubuntu-22.04.4-desktop-amd64.iso
#+END_SRC

**** 2. Crear VM en virt-manager

#+BEGIN_SRC text
- Nombre: ivanti-vpn
- RAM: 4096 MB
- CPUs: 2
- Disco: 20GB (qcow2)
- Red: Bridge br0 (o NAT para instalar)
- Instalar Ubuntu con usuario: passh / password: capullo100
#+END_SRC

**** 3. Configurar Ubuntu

#+BEGIN_SRC bash
# En la VM:
sudo apt update && sudo apt upgrade -y
sudo apt install -y openssh-server net-tools

# Configurar IP fija (si usas bridge)
sudo tee /etc/netplan/01-bridge.yaml << 'EOF'
network:
  version: 2
  ethernets:
    ens3:
      addresses: [192.168.53.12/24]
      routes:
        - to: default
          via: 192.168.53.10
      nameservers:
        addresses: [8.8.8.8]
EOF
sudo netplan apply
#+END_SRC

**** 4. Instalar cliente Ivanti/Pulse Secure

#+BEGIN_SRC bash
# Obtener el .deb de Vocento (pedir a IT o descargar del portal)
# Tipicamente: ps-pulse-linux-22.x.xxx.deb

sudo dpkg -i ps-pulse-linux-*.deb
sudo apt --fix-broken install -y

# El cliente queda en /opt/pulsesecure/
# Lanzar: /opt/pulsesecure/bin/pulseUI
#+END_SRC

**** 5. Configurar conexion VPN

#+BEGIN_SRC text
- Abrir pulseUI
- Añadir conexion:
  - Nombre: Vocento
  - URL: (preguntar a IT, tipicamente https://vpn.vocento.com)
- Conectar con credenciales corporativas + Okta push
#+END_SRC

**** 6. Verificar funcionamiento

#+BEGIN_SRC bash
# En la VM:
ip route | grep tun    # Debe mostrar tun0
ping 10.180.0.1        # Debe responder (si existe)

# En el host:
ping 192.168.53.12     # VM alcanzable
host bitbucket.vocento.com  # DNS funciona
curl https://bitbucket.vocento.com  # Conectividad OK
#+END_SRC

*** Backup del Disco (Oro en Pano)

**** Backup a USB

#+BEGIN_SRC bash
# 1. Apagar la VM
vpn-vm stop

# 2. Montar USB (ajustar /dev/sdX)
sudo mount /dev/sdb1 /mnt/usb

# 3. Copiar disco (17GB, tarda un rato)
sudo cp /var/lib/libvirt/images/ivanti-vpn-clone.qcow2 /mnt/usb/
# O con progreso:
sudo rsync -ah --progress /var/lib/libvirt/images/ivanti-vpn-clone.qcow2 /mnt/usb/

# 4. Verificar
ls -lh /mnt/usb/ivanti-vpn-clone.qcow2

# 5. Desmontar
sudo umount /mnt/usb
#+END_SRC

**** Backup comprimido (mas pequeno)

#+BEGIN_SRC bash
# Comprimir (17GB -> ~6GB)
sudo qemu-img convert -O qcow2 -c \
  /var/lib/libvirt/images/ivanti-vpn-clone.qcow2 \
  /mnt/usb/ivanti-vpn-backup-$(date +%Y%m%d).qcow2

# Restaurar
sudo cp /mnt/usb/ivanti-vpn-backup-*.qcow2 \
  /var/lib/libvirt/images/ivanti-vpn-clone.qcow2
#+END_SRC

**** Ubicaciones de backup recomendadas

| Ubicacion | Prioridad | Notas |
|-----------+-----------+-------|
| USB externo | Alta | Portatil, offline |
| aurin:/backup/ | Media | Acceso rapido por red |
| NAS/Synology | Baja | Si tienes uno |

*** Cambio de Red WiFi

*Pregunta:* ¿Que pasa si cambio de WiFi y tengo otra IP?

*Respuesta:* *No afecta* porque:

#+BEGIN_SRC text
Tu IP WiFi cambia (192.168.1.50 → 192.168.2.100)
                    ↓
            NO IMPORTA porque:
                    ↓
┌─────────────────────────────────────────────────┐
│ br0: 192.168.53.10  ← IP FIJA (interna)        │
│ VM:  192.168.53.12  ← IP FIJA (interna)        │
│ NAT: usa INTERFACE, no IP                       │
│      (wlp0s20f0u7u4 sigue siendo la misma)     │
└─────────────────────────────────────────────────┘
#+END_SRC

*Solo falla si:* Cambias de interface fisica (ej: de WiFi USB a Ethernet).
En ese caso, cambia =externalInterface= en el config y rebuild.

** AI Agents (claude-code, opencode) - NIXIFICADO [2026-01-22]

*** El Problema: Stow → Home-Manager Migration

Históricamente, las configuraciones de =claude-code= se gestionaban con GNU Stow:
- Symlinks manuales desde =~/dotfiles/claude-code/.claude/= a =~/.claude/=
- No portable entre máquinas
- Stow es deprecated, se está eliminando progresivamente
- Difícil de versionar cambios de forma declarativa

*Objetivo:* Nixificar toda la config AI para que sea:
- *Declarativa* - Gestionada por home-manager, no scripts
- *Portable* - Clone-first: misma config en aurin, macbook, vespino
- *Versionada* - Cambios trackeados en git
- *Multi-cliente* - Soportar claude-code, opencode, gemini-cli, etc.

*** La Solución: Módulo home-manager Unificado

#+BEGIN_SRC text
┌─────────────────────────────────────────────────────────────────────┐
│                    ARQUITECTURA AI AGENTS                           │
│                                                                     │
│  dotfiles/claude-code/.claude/    ← SOURCE (git versionado)        │
│         ├── CLAUDE.md                                               │
│         ├── settings.json                                           │
│         ├── agents/                                                 │
│         │   ├── nixos-guru.md                                       │
│         │   ├── vocento-software-architect.md                       │
│         │   └── ... (10 agentes)                                    │
│         └── skills/                                                 │
│                                                                     │
│                    ▼ (home-manager symlinks)                        │
│                                                                     │
│  ~/.claude/                       ← RUNTIME (symlinkado)           │
│         ├── CLAUDE.md        → dotfiles/claude-code/.claude/...    │
│         ├── settings.json    → dotfiles/claude-code/.claude/...    │
│         ├── agents/          → dotfiles/claude-code/.claude/agents/│
│         ├── skills/          → dotfiles/claude-code/.claude/skills/│
│         │                                                           │
│         └── [STATE - NO GESTIONADO]                                │
│             ├── .claude.json        (stats, history - 698 líneas)  │
│             ├── history.jsonl       (conversaciones)               │
│             ├── debug/              (logs)                          │
│             └── cache/              (temp files)                    │
└─────────────────────────────────────────────────────────────────────┘
#+END_SRC

*** Separación: Config vs State

*CRÍTICO:* Nix solo gestiona CONFIGURACIÓN, no estado runtime.

| Tipo | Archivos | Gestionado por Nix |
|------+----------+--------------------|
| *Config* | =settings.json=, =CLAUDE.md=, =agents/=, =skills/= | ✅ SÍ (symlinks) |
| *State* | =.claude.json=, =history.jsonl=, =debug/=, =cache/= | ❌ NO (runtime) |

*Por qué?*
- Config: Definiciones estáticas (preferencias, agentes)
- State: Datos mutables (historial, estadísticas, sesiones)

Si gestionaras state con Nix, cada =nixos-rebuild= resetearía tu historial.

*** Módulo: modules/home-manager/programs/ai-agents.nix

*Ubicación:* Importado en =core.nix= (portable, no en =passh.nix= desktop-only)

#+BEGIN_SRC nix
# modules/home-manager/programs/ai-agents.nix
{ config, lib, pkgs, ... }:

let
  homeDir = config.home.homeDirectory;
  dotfilesDir = "${homeDir}/dotfiles";
in
{
  # ===========================================================================
  # Claude Code - Configuration symlinks
  # ===========================================================================
  
  home.file = {
    ".claude/settings.json".source = 
      config.lib.file.mkOutOfStoreSymlink 
      "${dotfilesDir}/claude-code/.claude/settings.json";
    
    ".claude/CLAUDE.md".source =
      config.lib.file.mkOutOfStoreSymlink
      "${dotfilesDir}/claude-code/.claude/CLAUDE.md";
    
    ".claude/agents".source =
      config.lib.file.mkOutOfStoreSymlink
      "${dotfilesDir}/claude-code/.claude/agents";
    
    ".claude/skills".source =
      config.lib.file.mkOutOfStoreSymlink
      "${dotfilesDir}/claude-code/.claude/skills";
  };
  
  # ===========================================================================
  # State directories (runtime, not managed by Nix)
  # ===========================================================================
  
  home.activation.createClaudeState = lib.hm.dag.entryAfter ["writeBoundary"] ''
    mkdir -p ~/.claude/{debug,file-history,session-env,todos,cache,projects}
  '';
  
  # ===========================================================================
  # Transition helper: Clean old stow symlinks
  # ===========================================================================
  
  home.activation.cleanOldClaudeStow = lib.hm.dag.entryBefore ["checkLinkTargets"] ''
    for link in ~/.claude/{settings.json,CLAUDE.md,agents,skills}; do
      if [[ -L "$link" ]] && [[ "$(readlink "$link")" == *"/dotfiles/claude-code/"* ]]; then
        rm -f "$link"
      fi
    done
  '';
}
#+END_SRC

*** Integración en core.nix

#+BEGIN_SRC nix
# modules/home-manager/core.nix
{ config, pkgs, lib, ... }:

{
  imports = [
    ./programs/ai-agents.nix  # AI agents config (portable)
  ];
  
  # ... resto de config core ...
}
#+END_SRC

*** Eliminar claude-code de Stow

#+BEGIN_SRC nix
# modules/home-manager/passh.nix (línea ~252)

# ANTES:
${pkgs.stow}/bin/stow -v -R -t ${config.home.homeDirectory} \
  composer xmonad claude-code

# DESPUÉS:
${pkgs.stow}/bin/stow -v -R -t ${config.home.homeDirectory} \
  composer xmonad
# claude-code → Migrado a home-manager (modules/home-manager/programs/ai-agents.nix)
#+END_SRC

*** ¿Por qué mkOutOfStoreSymlink?

*Comparación de métodos:*

#+BEGIN_SRC nix
# ❌ MAL - Copia al Nix store (read-only, duplicado)
home.file.".claude/CLAUDE.md".source = ./claude-code/.claude/CLAUDE.md;
# Resultado: /nix/store/hash-CLAUDE.md (inmutable)

# ✅ BIEN - Symlink fuera del store (mutable, single source of truth)
home.file.".claude/CLAUDE.md".source = 
  config.lib.file.mkOutOfStoreSymlink "${homeDir}/dotfiles/...";
# Resultado: ~/.claude/CLAUDE.md → ~/dotfiles/claude-code/.claude/CLAUDE.md
#+END_SRC

*Ventajas:*
- Replica exactamente el comportamiento de stow
- Cambios en dotfiles son inmediatos (no rebuild necesario)
- Source of truth sigue siendo =~/dotfiles/=
- Editable con cualquier editor (no read-only)

*** Aplicar la Configuración

#+BEGIN_SRC bash
# 1. Git pull (si ya está en origin)
cd ~/dotfiles
git pull

# 2. Aplicar (aurin)
sudo nixos-rebuild switch --flake ~/dotfiles#aurin --impure

# 3. Verificar symlinks
ls -la ~/.claude/
readlink ~/.claude/CLAUDE.md
# → /home/passh/dotfiles/claude-code/.claude/CLAUDE.md

# 4. Verificar que no hay symlinks rotos
find ~/.claude -xtype l
# → (vacío = todo OK)
#+END_SRC

*** Reproducir en Máquina Nueva

#+BEGIN_SRC bash
# 1. Clonar dotfiles
git clone <repo> ~/dotfiles
cd ~/dotfiles

# 2. Rebuild (automáticamente configura AI agents)
sudo nixos-rebuild switch --flake ~/dotfiles#macbook

# 3. LISTO - claude-code funcionando con tus agentes y config
claude-code
#+END_SRC

*No se requiere:*
- ❌ Ejecutar =stow=
- ❌ Copiar archivos manualmente
- ❌ Configurar nada adicional

*Clone-first en acción:* Misma config en aurin, macbook, vespino.

*** Estado Actual: Multi-Cliente Preparado

| Cliente | Estado | Ubicación en ai-agents.nix |
|---------+--------+----------------------------|
| *claude-code* | ✅ ACTIVO | Symlinks configurados |
| *opencode* | ⏳ EXPERIMENTAL | Comentado (pending FASE 0) |
| *gemini-cli* | 📋 FUTURO | No implementado |
| *github-copilot* | 🤔 INVESTIGAR | Usa =~/.config/github-copilot/= |

*** Plan Futuro: Shared Agents (Multi-Cliente)

*Actualmente:* Cada cliente tiene su propia config en =dotfiles/CLIENTE/=

#+BEGIN_SRC text
dotfiles/
├── claude-code/.claude/    ← Solo claude-code
└── opencode/.opencode/     ← Solo opencode (futuro)
#+END_SRC

*Propuesta investigar:* Config compartida (si clientes son compatibles)

#+BEGIN_SRC text
dotfiles/
├── ai-agents/              ← SHARED entre todos los clientes
│   ├── GLOBAL.md          ← Instrucciones universales
│   ├── agents/            ← Agentes compartidos
│   └── mcp-servers.json   ← MCP (Model Context Protocol)
├── claude-code/           ← Solo overrides específicos
└── opencode/              ← Solo overrides específicos
#+END_SRC

*Requisitos para implementar:*
1. *Investigar compatibilidad* - ¿opencode lee formato .md de claude-code?
2. *Probar MCP* - ¿opencode soporta mismo formato MCP?
3. *Arquitectura adaptadores* - Módulo que traduce shared → per-client

*Estado:* ON HOLD hasta investigar opencode (FASE 0)

** Voice Input & Text-to-Speech - LOCAL AI [2026-01-27]

*** El Problema: Productividad en Dictado y Generación de Audio

Históricamente:
- Dictado de voz → Google/Apple (cloud, privacidad comprometida)
- Text-to-Speech → Servicios online (Eleven Labs, Play.ht - caros)
- Voice cloning → Solo en Colab/servicios externos
- Latencia: 5-10 segundos por request
- Privacidad: Todo tu audio sube a servidores externos

*Objetivo:* Sistema local completo de voz con IA en GPU:
- *STT (Speech-to-Text)* - Transcripción local con Whisper
- *TTS (Text-to-Speech)* - Generación de voz con Qwen3-TTS
- *Voice Cloning* - Clonar cualquier voz con 5-15 segundos
- *Latencia* - 2-5 segundos (todo local en RTX 5080)
- *Privacidad* - Nada sale de tu máquina

*** Arquitectura: Pipeline Completo STT + TTS

#+BEGIN_SRC text
┌─────────────────────────────────────────────────────────────────────┐
│                   VOICE PROCESSING PIPELINE                         │
│                                                                     │
│  ┌──────────────────────────────────────────────────────┐          │
│  │  SPEECH-TO-TEXT (Whisper)                            │          │
│  │                                                       │          │
│  │  Audio (mic/meeting) → Whisper GPU → Text            │          │
│  │  - Model: ggml-small.bin (Spanish optimized)         │          │
│  │  - Speed: ~0.14x realtime (RTX 5080)                 │          │
│  │  - Latency: 2-3s for 10s audio                       │          │
│  └──────────────────────────────────────────────────────┘          │
│                            ▼                                        │
│                        TEXT EDITING                                 │
│                     (manual or automated)                           │
│                            ▼                                        │
│  ┌──────────────────────────────────────────────────────┐          │
│  │  TEXT-TO-SPEECH (Qwen3-TTS)                          │          │
│  │                                                       │          │
│  │  Text + Voice Reference → Qwen3-TTS → Audio (cloned) │          │
│  │  - Model: Qwen3-TTS-12Hz-1.7B-Base (~3.5GB)          │          │
│  │  - Speed: 2-5s per generation                        │          │
│  │  - Quality: High-fidelity voice cloning              │          │
│  └──────────────────────────────────────────────────────┘          │
│                                                                     │
│  STORAGE:                                                           │
│  - Whisper models: ~/.local/share/whisper/models/                  │
│  - Qwen models:    ~/.cache/huggingface/hub/                       │
│  - Recordings:     ~/recordings/YYYY-MM-DD_HH-MM-meeting/          │
│  - Voice refs:     ~/voice-cloning/references/                     │
│  - Generated TTS:  ~/voice-cloning/output/                         │
└─────────────────────────────────────────────────────────────────────┘
#+END_SRC

*** Componentes Instalados

**** 1. Whisper (Speech-to-Text)

*Módulo:* =modules/home-manager/programs/whisper.nix=

*Scripts instalados:*
- =whisper-brutal= - Transcripción básica desde audio
- =voice-input-toggle= - Dictado interactivo (M-r)
- =meeting-recorder-toggle= - Grabación larga de meetings (M-S-r)

*Keybindings XMonad:*
#+BEGIN_SRC haskell
-- xmonad/.config/xmonad/xmonad.hs
, ("M-r", spawn "voice-input-toggle")      -- Dictar texto
, ("M-S-r", spawn "meeting-recorder-toggle") -- Grabar meeting
#+END_SRC

*Features:*
- ✅ Auto-detect RØDE NT-USB Mini microphone
- ✅ Fallback a @DEFAULT_SOURCE@ si no encuentra RØDE
- ✅ Transcripción en español (configurable)
- ✅ GPU acceleration (Vulkan backend)
- ✅ Inyección automática de texto con xdotool

**** 2. Qwen3-TTS (Text-to-Speech + Voice Cloning)

*Módulo:* =modules/home-manager/programs/qwen-tts.nix=

*Script instalado:*
- =qwen-tts-clone= - Voice cloning y TTS

*Features:*
- ✅ Voice cloning con 5-15 segundos de audio
- ✅ Multi-idioma (Spanish, English, Chinese, Japanese, etc.)
- ✅ Generación local en GPU (RTX 5080)
- ✅ Alta calidad (48kHz/96kHz support)
- ✅ Modelo auto-descarga en primer uso

*** Uso: Voice Input (Dictado)

**** Keybinding: M-r (Mod+r)

#+BEGIN_SRC bash
# Flujo:
# 1. Presionar M-r → empieza grabación (notificación roja)
# 2. Hablar tu texto
# 3. Presionar M-r de nuevo → para, transcribe, inyecta texto

# Ejemplo:
# - Abres Emacs/IntelliJ/navegador
# - M-r
# - "Este es un comentario que quiero añadir al código"
# - M-r
# → Texto aparece donde está el cursor
#+END_SRC

*Performance:*
- Grabación: instantánea
- Transcripción: ~2-3s para 10s de audio
- Inyección: instantánea
- *Total: 3-5 segundos de latencia*

*Audio quality settings:*
#+BEGIN_SRC bash
# whisper-core.sh - record_audio()
-ar 16000   # 16kHz (optimal para Whisper)
-ac 1       # Mono
-acodec pcm_s16le  # 16-bit PCM
#+END_SRC

*** Uso: Meeting Recorder (Grabación Larga)

**** Keybinding: M-S-r (Mod+Shift+r)

#+BEGIN_SRC bash
# Flujo:
# 1. Presionar M-S-r → empieza grabación (notificación persistente)
# 2. Hablar durante la reunión (minutos/horas)
# 3. Presionar M-S-r de nuevo → para, transcribe, guarda

# Salida:
# ~/recordings/2026-01-27_15-34-meeting/
#   ├── audio.wav           # Grabación completa (16kHz mono)
#   ├── transcription.txt   # Texto transcrito
#   └── metadata.json       # Duración, fecha, etc.
#+END_SRC

*Audio sources captured:*
- 🎤 Microphone (tu voz)
- 🔊 System audio (speakers/headphones) - YouTube, VLC, etc.
- ⚖️ Mix ratio: Mic 1x, Monitor 8x (configurable)
- 🎚️ Dynamic normalization (balance automático)

*Audio quality settings:*
#+BEGIN_SRC bash
# whisper-core.sh - record_audio_full()
[0:a]volume=1.0[mic]           # Micrófono nivel natural
[1:a]volume=8.0[monitor]       # System audio amplificado 8x
amix + dynaudnorm              # Mezcla + normalización
volume=3.0[out]                # Amplificación final 3x
#+END_SRC

*** Uso: Voice Cloning (TTS)

**** Paso 1: Crear Audio de Referencia

Graba 5-15 segundos de la voz que quieres clonar (tuya u otra):

#+BEGIN_SRC bash
# Opción A: Grabación manual de alta calidad (96kHz)
ffmpeg -f pulse -i @DEFAULT_SOURCE@ \
  -ar 96000 \
  -ac 1 \
  -acodec pcm_s32le \
  ~/voice-cloning/references/mi-voz.wav
# Ctrl+C para parar

# Opción B: Usar meeting-recorder (16kHz, suficiente)
meeting-recorder-toggle
# Hablar 5-10 segundos
meeting-recorder-toggle
# Usar ~/recordings/FECHA/audio.wav como referencia

# Opción C: Usar cualquier audio existente
cp mi-grabacion-existente.mp3 ~/voice-cloning/references/voz.wav
#+END_SRC

*Recomendación:*
- Duración: 5-15 segundos (ni muy corto ni muy largo)
- Calidad: 48kHz mínimo (96kHz óptimo)
- Voz: Clara, sin ruido de fondo
- Contenido: Frase natural, no robótica

**** Paso 2: Transcribir el Audio de Referencia

CRÍTICO: Necesitas el texto EXACTO de lo que dice el audio.

#+BEGIN_SRC bash
# Opción A: Manual (si sabes qué dice)
echo "Hola, soy Pascual y esta es mi voz de referencia" \
  > ~/voice-cloning/references/mi-voz.txt

# Opción B: Whisper auto-transcripción
whisper-cli \
  -m ~/.local/share/whisper/models/ggml-small.bin \
  -f ~/voice-cloning/references/mi-voz.wav \
  -l es \
  -nt \
  > ~/voice-cloning/references/mi-voz.txt
#+END_SRC

**** Paso 3: Generar Audio Clonado

#+BEGIN_SRC bash
# Uso básico
qwen-tts-clone \
  --reference ~/voice-cloning/references/mi-voz.wav \
  --reference-text "$(cat ~/voice-cloning/references/mi-voz.txt)" \
  --target-text "Bienvenidos a mi podcast sobre NixOS y programación funcional" \
  --language Spanish \
  --output ~/voice-cloning/output/podcast-intro.wav

# Shortcuts con alias cortos
qwen-tts-clone \
  -r ~/voice-cloning/references/mi-voz.wav \
  -rt "$(cat ~/voice-cloning/references/mi-voz.txt)" \
  -t "Este es el texto que quiero generar con mi voz clonada" \
  -l Spanish \
  -o output.wav
#+END_SRC

*Performance (primera ejecución):*
- Descarga modelo: ~3.5GB (2-3 minutos)
- Generación: 5-10 segundos

*Performance (posteriores):*
- Generación: 2-5 segundos

*Idiomas soportados:*
- Spanish, English, Chinese, Japanese, Korean
- German, French, Russian, Portuguese, Italian
- Auto (detecta automáticamente)

*** Casos de Uso Reales

**** 1. Dictado Rápido en Desarrollo

#+BEGIN_SRC bash
# Estás en Emacs/IntelliJ escribiendo código
# Necesitas comentar una función compleja

M-r  # Empezar grabación
"Esta función implementa el patrón repository para abstraer 
 el acceso a datos y facilitar testing mediante dependency injection"
M-r  # Parar → texto inyectado automáticamente

# Resultado: comentario insertado donde está el cursor
#+END_SRC

**** 2. Transcripción de Reuniones

#+BEGIN_SRC bash
# Meeting de 1 hora con cliente

M-S-r  # Empezar grabación
# ... reunión completa ...
M-S-r  # Parar

# Salida automática:
# ~/recordings/2026-01-27_16-00-meeting/
#   ├── audio.wav (60 minutos, 60MB)
#   ├── transcription.txt (texto completo)
#   └── metadata.json

# Editar transcripción:
emacs ~/recordings/2026-01-27_16-00-meeting/transcription.txt
# → Resumen, correcciones, highlights
#+END_SRC

**** 3. Generar Audiolibros con Tu Voz

#+BEGIN_SRC bash
# Tienes un artículo/tutorial escrito en Markdown
# Quieres convertirlo a audio con tu voz

# 1. Grabar voz de referencia (una vez)
ffmpeg -f pulse -i @DEFAULT_SOURCE@ \
  -ar 96000 -ac 1 -acodec pcm_s32le \
  ~/voice-cloning/references/mi-voz-audiolibro.wav
# Leer 10 segundos de texto natural

# 2. Transcribir referencia
echo "Texto exacto que leíste en la grabación" \
  > ~/voice-cloning/references/mi-voz-audiolibro.txt

# 3. Convertir artículo a audio
qwen-tts-clone \
  -r ~/voice-cloning/references/mi-voz-audiolibro.wav \
  -rt "$(cat ~/voice-cloning/references/mi-voz-audiolibro.txt)" \
  -t "$(cat mi-articulo-completo.txt)" \
  -l Spanish \
  -o audiolibro-capitulo1.wav

# Resultado: Audio de 10-20 minutos con TU voz narrando
#+END_SRC

**** 4. Crear Intros/Outros para YouTube/Podcasts

#+BEGIN_SRC bash
# Crear intro personalizada sin grabar

qwen-tts-clone \
  -r ~/voice-cloning/references/mi-voz.wav \
  -rt "$(cat ~/voice-cloning/references/mi-voz.txt)" \
  -t "Hola, bienvenidos a Code Brutal, el podcast donde 
      hablamos de arquitectura de software sin pelos en la lengua.
      Soy Pascual y hoy vamos a hablar de Domain-Driven Design" \
  -l Spanish \
  -o youtube-intro-episode-05.wav

# Editar en Audacity/ffmpeg:
ffmpeg -i youtube-intro-episode-05.wav \
  -filter:a "afade=t=in:d=1,afade=t=out:d=2" \
  youtube-intro-final.wav
#+END_SRC

**** 5. Traducción + Voice Cloning

#+BEGIN_SRC bash
# Grabar en español
meeting-recorder-toggle
# Hablar en español...
meeting-recorder-toggle

# Transcripción automática en español
RECORDING_DIR=$(ls -td ~/recordings/* | head -1)
SPANISH_TEXT=$(cat $RECORDING_DIR/transcription.txt)

# Traducir con LLM (ejemplo con Claude)
echo "$SPANISH_TEXT" | claude --prompt "Translate to English, maintain tone" \
  > english-translation.txt

# Generar audio en inglés con voz clonada
qwen-tts-clone \
  -r ~/voice-cloning/references/my-english-voice.wav \
  -rt "Hello, this is my reference voice for English" \
  -t "$(cat english-translation.txt)" \
  -l English \
  -o translated-audio-english.wav
#+END_SRC

*** Pipeline Completo: STT → Edit → TTS

*Workflow típico:*

#+BEGIN_SRC bash
# 1. GRABAR (STT)
meeting-recorder-toggle
# ... hablar durante la reunión/presentación ...
meeting-recorder-toggle

# Resultado:
MEETING=$(ls -td ~/recordings/* | head -1)
# $MEETING/audio.wav
# $MEETING/transcription.txt

# 2. EDITAR TRANSCRIPCIÓN
emacs $MEETING/transcription.txt
# - Corregir errores de Whisper
# - Resumir contenido
# - Mejorar redacción
# - Añadir estructura

# 3. REGENERAR AUDIO CON EDICIONES (TTS)
qwen-tts-clone \
  -r $MEETING/audio.wav \
  -rt "$(head -50c $MEETING/transcription.txt)" \
  -t "$(cat $MEETING/transcription-editada.txt)" \
  -l Spanish \
  -o $MEETING/audio-mejorado.wav

# Resultado final:
# - Audio original: $MEETING/audio.wav
# - Transcripción editada: $MEETING/transcription-editada.txt
# - Audio regenerado: $MEETING/audio-mejorado.wav
#+END_SRC

*Beneficios:*
- ✅ Calidad superior (sin "uhms", pausas, errores)
- ✅ Contenido editado (resumen, mejoras)
- ✅ Misma voz (clonada del audio original)
- ✅ Todo local (privacidad total)

*** Performance Benchmarks (RTX 5080)

*Whisper (Speech-to-Text):*
| Audio Duration | Transcription Time | Speed Factor |
|----------------+--------------------+--------------|
| 10 segundos    | ~1.4 segundos      | 0.14x        |
| 1 minuto       | ~8.4 segundos      | 0.14x        |
| 10 minutos     | ~84 segundos       | 0.14x        |
| 1 hora         | ~8.4 minutos       | 0.14x        |

*Qwen3-TTS (Text-to-Speech):*
| Text Length      | Generation Time | Notes                    |
|------------------+-----------------+--------------------------|
| 1 frase (~10 palabras) | 2-3 segundos | Latencia inicial modelo |
| 1 párrafo (~50 palabras) | 3-5 segundos | Generación incremental |
| 1 página (~300 palabras) | 10-15 segundos | Batch generation |

*VRAM Usage:*
| Model        | VRAM Used | Total Available |
|--------------+-----------+-----------------|
| Whisper      | ~2GB      | 16GB (RTX 5080) |
| Qwen3-TTS    | ~4-6GB    | 16GB (RTX 5080) |
| Both running | ~8GB      | 16GB (RTX 5080) |

*** Configuración Avanzada

**** Ajustar Calidad de Audio (Meeting Recorder)

Editar =scripts/whisper-core.sh= - función =record_audio_full()=:

#+BEGIN_SRC bash
# ACTUAL: Monitor amplificado 8x
[1:a]volume=8.0[monitor]

# Si system audio suena muy alto:
[1:a]volume=4.0[monitor]

# Si tu micrófono se escucha bajo:
[0:a]volume=2.0[mic]

# Volumen final:
volume=3.0[out]  # Subir a 5.0 si todo suena bajo
#+END_SRC

Rebuild después de cambios:
#+BEGIN_SRC bash
sudo nixos-rebuild switch --flake ~/dotfiles#aurin --impure
#+END_SRC

**** Cambiar Idioma de Transcripción

Editar scripts para cambiar idioma por defecto:

#+BEGIN_SRC bash
# scripts/voice-input-toggle
# Línea ~85 (función transcribe_audio)
TRANSCRIPT=$(transcribe_audio "$AUDIO_FILE" "es")  # Cambiar "es" por "en", "zh", etc.

# scripts/meeting-recorder-toggle  
# Línea ~85
TRANSCRIPT=$(transcribe_audio "$AUDIO_FILE" "es")
#+END_SRC

**** Usar Otro Modelo Whisper

Por defecto: =ggml-small.bin= (461MB, español optimizado)

Opciones:
- =ggml-tiny.bin= (75MB, rápido pero menos preciso)
- =ggml-base.bin= (142MB, balance)
- =ggml-medium.bin= (1.5GB, más preciso)
- =ggml-large.bin= (2.9GB, máxima precisión, más lento)

Descargar modelo alternativo:
#+BEGIN_SRC bash
cd ~/.local/share/whisper/models/
curl -LO https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-medium.bin
#+END_SRC

Editar =scripts/whisper-core.sh=:
#+BEGIN_SRC bash
# Línea 20
MODEL="$HOME/.local/share/whisper/models/ggml-medium.bin"
#+END_SRC

*** Troubleshooting

**** Voice Input No Inyecta Texto

Problema: Transcripción funciona pero no inyecta texto.

Solución:
#+BEGIN_SRC bash
# Verificar xdotool instalado
which xdotool

# Probar inyección manual
xdotool type "test"

# Si falla: verificar window manager
echo $XDG_SESSION_TYPE  # Debe ser x11, no wayland
#+END_SRC

**** Meeting Recorder: No Captura System Audio

Problema: Solo graba micrófono, no audio del sistema.

Solución:
#+BEGIN_SRC bash
# Ver monitors disponibles
pactl list sources short | grep monitor

# Debe haber al menos un monitor
# Si no hay: verificar PulseAudio/PipeWire config

# Ajustar volumen monitor en whisper-core.sh
[1:a]volume=12.0[monitor]  # Subir de 8.0 a 12.0
#+END_SRC

**** Qwen3-TTS: Model Download Fails

Problema: =qwen-tts-clone= falla al descargar modelo.

Solución:
#+BEGIN_SRC bash
# Descargar modelo manualmente
mkdir -p ~/.cache/huggingface/hub
cd ~/.cache/huggingface/hub

# Descargar con git lfs
git lfs install
git clone https://huggingface.co/Qwen/Qwen3-TTS-12Hz-1.7B-Base

# Retry script
qwen-tts-clone --reference test.wav ...
#+END_SRC

**** VRAM Out of Memory

Problema: =RuntimeError: CUDA out of memory=

Solución:
#+BEGIN_SRC bash
# Verificar VRAM disponible
nvidia-smi

# Cerrar procesos que usen GPU:
# - Navegadores (Chrome/Firefox GPU acceleration)
# - OBS Studio
# - Otros modelos LLM

# Si persiste: usar modelo más pequeño o CPU
# (no implementado aún - TODO)
#+END_SRC

*** Archivos y Módulos

| File/Module | Purpose |
|-------------|---------|
| =modules/home-manager/programs/whisper.nix= | Whisper STT config |
| =modules/home-manager/programs/qwen-tts.nix= | Qwen3-TTS config |
| =scripts/whisper-core.sh= | Shared audio functions |
| =scripts/voice-input-toggle= | Interactive dictation (M-r) |
| =scripts/meeting-recorder-toggle= | Long recording (M-S-r) |
| =scripts/qwen-tts-clone= | Voice cloning CLI |
| =xmonad/.config/xmonad/xmonad.hs= | Keybindings |

*** Estructura de Directorios

#+BEGIN_SRC text
~/
├── .local/share/whisper/models/    # Whisper models
│   └── ggml-small.bin               # Spanish optimized (461MB)
│
├── .cache/huggingface/hub/          # Qwen3-TTS models
│   └── Qwen3-TTS-12Hz-1.7B-Base/   # Auto-downloaded (~3.5GB)
│
├── recordings/                      # Meeting recordings
│   └── 2026-01-27_16-00-meeting/
│       ├── audio.wav                # Original audio
│       ├── transcription.txt        # Whisper output
│       └── metadata.json            # Duration, date, etc.
│
└── voice-cloning/
    ├── references/                  # Voice samples for cloning
    │   ├── mi-voz.wav              # Reference audio (5-15s)
    │   └── mi-voz.txt              # Reference transcription
    │
    └── output/                      # Generated TTS audio
        └── podcast-intro.wav
#+END_SRC

*** Clone-First: Portabilidad

Ambos sistemas (Whisper + Qwen3-TTS) están en =modules/home-manager/core.nix= → portables a todas las máquinas.

*Estado actual:*
| Machine  | Whisper | Qwen3-TTS | Notes                       |
|----------+---------+-----------+-----------------------------|
| *aurin*  | ✅      | ✅        | RTX 5080 (16GB) - Full GPU  |
| *macbook*| ✅      | ⚠️        | CPU only (sin NVIDIA)       |
| *vespino*| ✅      | ⚠️        | CPU only (sin NVIDIA)       |

*Qwen3-TTS en CPU:* Funciona pero MUY lento (~30-60s por generación vs 2-5s GPU).

*TODO:* Implementar fallback CPU automático para máquinas sin GPU.

*** Referencias

*Whisper:*
- Modelo: https://github.com/ggerganov/whisper.cpp
- Paper: https://arxiv.org/abs/2212.04356

*Qwen3-TTS:*
- Modelo: https://huggingface.co/Qwen/Qwen3-TTS-12Hz-1.7B-Base
- Notebook Colab: (original inspiration)


*** Comandos Útiles

#+BEGIN_SRC bash
# Ver symlinks de claude-code
ls -la ~/.claude/
readlink ~/.claude/CLAUDE.md

# Ver agentes disponibles
ls ~/dotfiles/claude-code/.claude/agents/

# Editar agente (cambios inmediatos, no rebuild)
vim ~/dotfiles/claude-code/.claude/agents/nixos-guru.md

# Verificar que claude-code lee la config
cat ~/.claude/settings.json

# Backup del estado (antes de rebuild)
cp ~/.claude.json ~/backup/.claude.json.$(date +%Y%m%d-%H%M)

# Rollback si algo falla
sudo nixos-rebuild switch --rollback
#+END_SRC

*** Troubleshooting

| Problema | Solución |
|----------+----------|
| Symlinks rotos después de rebuild | =find ~/.claude -xtype l= + eliminar manualmente |
| claude-code no lee config nueva | Reiniciar terminal / =hash -r= |
| Cambios en agentes no se aplican | Symlinks apuntan fuera del store, son inmediatos |
| =.claude.json= se resetea | *NO DEBE PASAR* - no gestionado por Nix, verificar |
| Error al hacer rebuild | =nix flake check= + revisar sintaxis |

*** Arquitectura Detallada: Flujo de Configuración

#+BEGIN_SRC text
                         FLUJO DE CONFIGURACIÓN
                         ─────────────────────

┌────────────────────────────────────────────────────────────────────┐
│ 1. SOURCE (git)                                                    │
│    ~/dotfiles/claude-code/.claude/                                 │
│         ├── CLAUDE.md              ← Editable con cualquier editor │
│         ├── settings.json                                          │
│         ├── agents/                                                │
│         └── skills/                                                │
└────────────────────────────────────────────────────────────────────┘
                              │
                              │ git commit + push
                              ▼
┌────────────────────────────────────────────────────────────────────┐
│ 2. GIT REMOTE (github.com/pascualmg/dotfiles)                      │
│    Versionado, histórico, branches                                 │
└────────────────────────────────────────────────────────────────────┘
                              │
                              │ git pull (en otra máquina)
                              ▼
┌────────────────────────────────────────────────────────────────────┐
│ 3. NIX BUILD (nixos-rebuild switch)                                │
│    modules/home-manager/programs/ai-agents.nix                     │
│         └─> Genera symlinks con mkOutOfStoreSymlink                │
└────────────────────────────────────────────────────────────────────┘
                              │
                              │ home.activation
                              ▼
┌────────────────────────────────────────────────────────────────────┐
│ 4. RUNTIME (symlinks activos)                                      │
│    ~/.claude/CLAUDE.md → ~/dotfiles/claude-code/.claude/CLAUDE.md  │
│    ~/.claude/agents → ~/dotfiles/claude-code/.claude/agents        │
│                                                                    │
│    + State directories creados (debug/, cache/, etc.)              │
│    + Old stow symlinks limpiados (transition helper)               │
└────────────────────────────────────────────────────────────────────┘
                              │
                              │ claude-code lee config
                              ▼
┌────────────────────────────────────────────────────────────────────┐
│ 5. AI CLIENT (claude-code)                                         │
│    Lee: ~/.claude/CLAUDE.md (via symlink)                          │
│    Lee: ~/.claude/agents/* (via symlink)                           │
│    Escribe: ~/.claude.json (state, NO gestionado por Nix)          │
└────────────────────────────────────────────────────────────────────┘
#+END_SRC

*** Comparación: Antes vs Después

*ANTES (Stow):*
#+BEGIN_SRC bash
cd ~/dotfiles
stow -v -R -t ~ claude-code
# → Symlinks manuales, no versionados en sistema
# → Difícil de replicar en nueva máquina
# → No integrado con NixOS rebuild
#+END_SRC

*DESPUÉS (Home-Manager):*
#+BEGIN_SRC bash
sudo nixos-rebuild switch --flake ~/dotfiles#aurin --impure
# → Todo declarativo en Nix
# → Reproducible en cualquier máquina (clone-first)
# → Rollback automático si falla
#+END_SRC

*** Notas del NixOS Guru

*Puntos fuertes:*
- ✅ Separación config/state correcta
- ✅ =mkOutOfStoreSymlink= apropiado para dotfiles editables
- ✅ Transition helper para limpiar stow (temporal, removible después)
- ✅ Integrado en =core.nix= (portable, no desktop-only)
- ✅ Clone-first: funciona igual en todas las máquinas

*Mejoras sugeridas:*
- Considerar =programs.ai-agents.enable= si quieres opt-in por máquina
- Añadir assertions para verificar que =dotfiles/= existe
- Documentar formato de agentes .md (para contribuidores)

*Warnings:*
- ⚠️ NO commitear =.claude.json= (contiene history + stats)
- ⚠️ NO commitear =.credentials.json= (auth tokens)
- ⚠️ Symlinks son inmutables post-activation (no editables vía GUI)

*** Logs de Implementación

*Fecha:* 2026-01-22
*Ejecutor:* opencode CLI
*Tiempo:* ~10 minutos
*Commit:* =4c32211= feat: nixify claude-code config (remove stow dependency)

*Cambios:*
- ✅ Creado =modules/home-manager/programs/ai-agents.nix=
- ✅ Modificado =modules/home-manager/core.nix= (import)
- ✅ Modificado =modules/home-manager/passh.nix= (stow cleanup)
- ✅ Verificado con =nix flake check=
- ✅ Aplicado en aurin (test + switch)
- ✅ Pushed a origin/master

*Testing:*
#+BEGIN_SRC bash
# Verificación post-aplicación
ls -la ~/.claude/
readlink ~/.claude/CLAUDE.md
find ~/.claude -xtype l  # → vacío (sin symlinks rotos)
cat ~/.claude/settings.json  # → lee correctamente
#+END_SRC

*Resultado:* ✅ ÉXITO - claude-code nixificado completamente

*** AI Agents - Arquitectura Clone-First [2026-01-23]

**** 📁 Esquema de Directorios

#+BEGIN_EXAMPLE
dotfiles/
│
├── modules/home-manager/programs/ai-agents/
│   ├── default.nix           # Imports: claude-code, crush, opencode
│   ├── claude-code.nix       # 85 líneas - Symlinks + state dirs
│   ├── crush.nix             # 49 líneas - Symlink config
│   └── opencode.nix          # 83 líneas - Symlink + state dirs
│
├── claude-code/              # ← EDITAMOS AQUÍ (config versionado)
│   └── .claude/
│       ├── settings.json     # Config Claude
│       ├── CLAUDE.md         # Instructions
│       ├── agents/           # Custom agents
│       └── skills/           # Custom skills
│
├── crush/                    # ← EDITAMOS AQUÍ (config versionado)
│   └── .config/crush/
│       └── crush.json        # Config Crush (providers, models)
│
└── opencode/                 # ← EDITAMOS AQUÍ (config versionado)
    └── .config/opencode/
        └── package.json      # Config OpenCode (plugins TypeScript)
#+END_EXAMPLE

**** 🔗 Symlinks Generados (home-manager)

#+BEGIN_EXAMPLE
~/.claude/settings.json         → ~/dotfiles/claude-code/.claude/settings.json
~/.claude/CLAUDE.md             → ~/dotfiles/claude-code/.claude/CLAUDE.md
~/.claude/agents                → ~/dotfiles/claude-code/.claude/agents
~/.claude/skills                → ~/dotfiles/claude-code/.claude/skills

~/.config/crush/crush.json      → ~/dotfiles/crush/.config/crush/crush.json

~/.config/opencode/package.json → ~/dotfiles/opencode/.config/opencode/package.json
#+END_EXAMPLE

*IMPORTANTE:* Los symlinks apuntan a =~/dotfiles= (editable), NO al nix store.

**** 📊 Estado vs Config

**Config (gestionado por Nix):**
- Versionado en git
- En =~/dotfiles/<agent>/=
- Symlinkeado a =~/=
- Editable directamente

**Estado (runtime, NO gestionado):**

#+BEGIN_EXAMPLE
~/.claude/
├── debug/                  # Logs de debug
├── cache/                  # Cache de sesiones
├── history.jsonl           # Historial de comandos
├── file-history/           # Archivos editados
└── ...                     # Más dirs de estado

~/.local/share/opencode/
├── auth.json               # OAuth tokens (Anthropic, OpenAI)
└── storage/                # Sessions, messages, diffs

# Crush no tiene estado persistente
#+END_EXAMPLE

**** 🔄 Workflow: Añadir Nuevo Agente (ej: "aider")

**Paso 1: Crear config en dotfiles**

#+BEGIN_SRC bash
mkdir -p ~/dotfiles/aider/.config/aider
vim ~/dotfiles/aider/.config/aider/config.yaml
# Editar configuración del agente
#+END_SRC

**Paso 2: Crear módulo Nix (~30 líneas)**

#+BEGIN_SRC nix
# modules/home-manager/programs/ai-agents/aider.nix
{ config, lib, pkgs, ... }:

let
  dotfilesDir = "${config.home.homeDirectory}/dotfiles";
in
{
  home.activation.setupAider = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    echo "📁 Configurando aider..."
    mkdir -p ~/.config/aider
    
    # Remove nix store symlink if exists
    if [[ -L ~/.config/aider/config.yaml ]] && [[ "$(readlink ~/.config/aider/config.yaml)" == *"/nix/store/"* ]]; then
      rm ~/.config/aider/config.yaml
    fi
    
    # Create symlink to dotfiles (editable, not in store)
    if [[ ! -e ~/.config/aider/config.yaml ]]; then
      ln -sf "${dotfilesDir}/aider/.config/aider/config.yaml" ~/.config/aider/config.yaml
    fi
  '';
}
#+END_SRC

**Paso 3: Añadir import (1 línea)**

#+BEGIN_SRC nix
# modules/home-manager/programs/ai-agents/default.nix
{
  imports = [
    ./claude-code.nix
    ./crush.nix
    ./opencode.nix
    ./aider.nix        # ← Añadir esta línea
  ];
}
#+END_SRC

**Paso 4: Deploy**

#+BEGIN_SRC bash
cd ~/dotfiles
git add .
git commit -m "feat(ai-agents): Add aider agent"
git push

# En cualquier máquina
git pull
sudo nixos-rebuild switch --flake ~/dotfiles#$(hostname) --impure

# Verificar
readlink ~/.config/aider/config.yaml
# → /home/passh/dotfiles/aider/.config/aider/config.yaml ✅
#+END_SRC

**** ✅ Beneficios Clone-First

| Aspecto | Implementación |
|---------+----------------|
| *Config* | En =~/dotfiles/<agent>/= (versionado git) |
| *Módulo* | Symlink simple (~30-85 líneas) |
| *Escalabilidad* | Añadir agente = create file + 1 import |
| *Edición* | Editas JSON/YAML directo (fuera de nix store) |
| *Deploy* | =git pull && nixos-rebuild= en todas las máquinas |
| *Portabilidad* | Misma config en aurin, macbook, vespino, android |

**** 🎯 Filosofía

#+BEGIN_QUOTE
*"Config en dotfiles/, Nix solo hace symlinks"*

- ❌ NO: Parametrización Nix (=programs.crush.enable=, =settings= options)
- ✅ SÍ: Symlinks simples (activation scripts)
- ✅ SÍ: Config editable directamente (JSON, YAML, Markdown)
- ✅ SÍ: Clone-first (mismo código en todas las máquinas)

*Añadir nuevo agente = 30 líneas .nix + 1 import. Done.*
#+END_QUOTE

**** 🚀 Agentes Actuales

| Agente | Líneas Nix | Config | Package Source | Estado |
|--------+------------+--------+----------------|--------|
| =claude-code= | 85 | =.claude/= | nixpkgs | ✅ Activo |
| =crush= | 90 | =.config/crush/= | NUR (fetchGit) | ✅ Activo (v0.35.0+) |
| =opencode= | 83 | =.config/opencode/= | nixpkgs | ✅ Activo |

*Total módulos:* 3 agentes = 258 líneas Nix (vs 139 monolítico anterior)

*Próximos fáciles de añadir:* =aider=, =cursor=, =continue= (~30 líneas cada uno)

*** OpenCode / Crush - Investigación [2026-01-23]

**** Estado del Ecosistema

| Herramienta | Estado | nixpkgs | Versión |
|-------------+--------+---------+---------|
| =claude-code= | ✅ Activo | =pkgs.claude-code= | Latest |
| =opencode= | ⚠️ Archivado (Sep 2025) | =pkgs.opencode= | 1.1.23 (última) |
| =crush= | ✅ Activo | NUR Charmbracelet | 0.34.1+ |

**** OpenCode: Funciona con OAuth Anthropic Max

A pesar de estar "archivado", =opencode= en nixpkgs *FUNCIONA* correctamente:
- Versión 1.1.23 en nixpkgs es estable
- OAuth con Anthropic Max funciona
- Multi-provider (Anthropic, OpenAI, OpenRouter, etc.)

#+BEGIN_SRC bash
# Ya instalado via home-manager
opencode --version  # 1.1.23
#+END_SRC

**** Crush: AI Agent (Clone-First)

Crush es mantenido por *Charmbracelet* (creadores de Bubble Tea, Gum, etc.).
Soporta múltiples providers: Ollama, Anthropic, OpenAI, OpenRouter.

***** Implementación (Clone-First)

Crush usa el mismo patrón que claude-code y opencode:
- Config en =~/dotfiles/crush/.config/crush/crush.json= (editable)
- Módulo en =modules/home-manager/programs/ai-agents/crush.nix= (symlink)
- Habilitado por defecto en todas las máquinas

***** Módulo Nix (NUR Charmbracelet)

Crush usa NUR (Nix User Repository) de Charmbracelet para versión actualizada (0.35.0+).

****** Por qué NUR y no nixpkgs:

- nixpkgs: =crush 0.22.1= (antigua)
- NUR Charmbracelet: =crush 0.35.0+= (latest)
- Crush tiene licencia unfree (=fsl11Mit=)

****** Solución: fetchGit + callPackage (sin modificar flake)

#+BEGIN_SRC nix
# modules/home-manager/programs/ai-agents/crush.nix
{ config, lib, pkgs, ... }:

let
  homeDir = config.home.homeDirectory;
  dotfilesDir = "${homeDir}/dotfiles";
  
  # Fetch NUR Charmbracelet (NO requiere añadir al flake.nix)
  charm-nur = builtins.fetchGit {
    url = "https://github.com/charmbracelet/nur";
    ref = "master";
  };
  
  # Rebuild crush con NUESTRO pkgs (hereda allowUnfree del flake)
  crush = pkgs.callPackage "${charm-nur}/pkgs/crush" {};
in
{
  # Instalar crush desde NUR
  home.packages = [ crush ];  # v0.35.0+
  
  # Symlink config desde dotfiles (editable)
  home.activation.setupCrush = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    mkdir -p ~/.config/crush
    if [[ ! -e ~/.config/crush/crush.json ]]; then
      ln -sf "${dotfilesDir}/crush/.config/crush/crush.json" ~/.config/crush/crush.json
    fi
  '';
}
#+END_SRC

****** Por qué funciona:

1. =fetchGit= descarga NUR sin modificar flake.nix
2. =pkgs.callPackage= rebuilda usando NUESTRO pkgs
3. Nuestro pkgs tiene =allowUnfree = true= (flake.nix)
4. Crush hereda allowUnfree → se instala sin problemas
5. Clone-first: funciona igual en todas las máquinas

****** Alternativas rechazadas:

- ❌ Añadir =charm-nur= al flake → overlays complejos
- ❌ Usar =pkgs.crush= de nixpkgs → versión antigua (0.22.1)
- ❌ =NIXPKGS_ALLOW_UNFREE=1= → no declarativo

***** Config Actual (Ollama Remoto)

#+BEGIN_SRC json
// ~/dotfiles/crush/.config/crush/crush.json
{
  "providers": {
    "ollama-aurin": {
      "name": "Ollama (Aurin RTX 5080)",
      "base_url": "http://campo.zapto.org:11434/v1/",
      "type": "openai-compat",
      "models": [
        {"name": "Qwen3 14B", "id": "qwen3:14b", "context_window": 32768},
        {"name": "GPT-OSS 20B", "id": "gpt-oss:20b", "context_window": 32768},
        {"name": "Devstral Small 2", "id": "devstral-small-2:latest", "context_window": 32768}
      ]
    }
  }
}
#+END_SRC

***** Uso

#+BEGIN_SRC bash
# Ejecutar crush
crush

# Shortcuts
Ctrl+O  # Seleccionar modelo
Ctrl+K  # Comandos
Ctrl+?  # Ayuda

# Editar config
vim ~/dotfiles/crush/.config/crush/crush.json
# Cambios se aplican inmediatamente (symlink)
#+END_SRC

**** Ollama: Modelos Locales para Agentes

***** Modelos con Soporte de Tools (Agénticos)

*IMPORTANTE:* Solo estos modelos soportan function calling para agentes AI:

| Modelo | VRAM | Tools | Notas |
|--------+------+-------+-------|
| =qwen3:14b= | ~9GB | ✅ | Recomendado, ligero |
| =gpt-oss:20b= | ~13GB | ✅ | Buen balance |
| =devstral-small-2:latest= | ~15GB | ✅ | Límite 16GB VRAM |
| =qwen3-coder:30b= | ~18GB | ✅ | Mejor para código |

***** Modelos SIN soporte de tools (no sirven para agentes):

- =qwen2.5-coder:*= - Sin tool calling
- =deepseek-r1:*= - Solo reasoning, sin tools
- =gemma3:*= - Sin tools
- =mistral:latest= - Versión antigua sin tools
- =llama3.1:latest= - Sin tools

***** Configuración Crush para Ollama Remoto

#+BEGIN_SRC json
{
  "providers": {
    "ollama-aurin": {
      "name": "Ollama (Aurin RTX 5080)",
      "base_url": "http://campo.zapto.org:11434/v1/",
      "type": "openai-compat",
      "models": [
        {"name": "Qwen3 14B", "id": "qwen3:14b", "context_window": 32768},
        {"name": "GPT-OSS 20B", "id": "gpt-oss:20b", "context_window": 32768},
        {"name": "Devstral Small 2", "id": "devstral-small-2:latest", "context_window": 32768}
      ]
    }
  }
}
#+END_SRC

***** Instalar Modelos con Tools

#+BEGIN_SRC bash
# En la máquina con Ollama (aurin):
ollama pull qwen3:14b
ollama pull gpt-oss:20b
ollama pull devstral-small-2

# Modelos adicionales recomendados:
ollama pull qwen3-coder:30b    # Mejor para código
ollama pull mistral-small3.2   # Buen tool calling
#+END_SRC

**** Referencias AI Agents

- *Crush*: https://github.com/charmbracelet/crush
- *Charm NUR*: https://github.com/charmbracelet/nur
- *OpenCode (archivado)*: https://github.com/opencode-ai/opencode
- *Ollama Tools Models*: https://ollama.com/search?c=tools
- *Agent Skills*: https://agentskills.io

* Live ISO - Tu Entorno Portable

#+BEGIN_QUOTE
*Tu entorno completo, booteable en cualquier PC.*
Trabaja donde quieras. Prueba hardware nuevo. Comparte tu setup.
#+END_QUOTE

** Por que esto es revolucionario

Tradicionalmente, compartir tu entorno requeria:
1. Instalar el OS
2. Copiar dotfiles
3. Instalar paquetes manualmente
4. Rezar para que funcione igual

*Con NixOS + Flakes + Live ISO:*
1. Descargar ISO
2. Bootear
3. Ya tienes TODO funcionando identico

| Antes (dotfiles tradicionales) | Ahora (NixOS Live ISO) |
|--------------------------------+------------------------|
| "Funciona en mi maquina" | Funciona EN CUALQUIER maquina |
| Horas configurando | Bootea y listo |
| Dependencias rotas | Todo declarativo |
| Instalacion manual | Un comando |

** Casos de uso

*** Trabajo portatil
Llevas un USB con tu Live. Llegas a cualquier PC, booteas, y tienes
tu XMonad + fish + emacs + todo exactamente como en casa.

*** Probar hardware nuevo
Antes de comprar/instalar, booteas el Live y compruebas que todo funciona:
WiFi, GPU, audio, etc.

*** Rescate de sistemas
Tu NixOS no arranca? Booteas el Live, montas el disco, arreglas y listo.
Tienes todas tus herramientas disponibles.

*** Demo para otros
Alguien quiere ver tu setup? Le pasas la ISO. La prueba en VM o USB
sin tocar su sistema.

*** Clonar maquinas rapido
Nueva maquina? Booteas Live, particionas, y:
=nixos-install --flake github:pascualmg/dotfiles#aurin=

*** Compartir el repo
Cualquiera puede descargar tu ISO y probar tu entorno completo.
No necesitan saber Nix, solo bootear.

** Que incluye la ISO

*** Sistema base
| Componente | Version/Config |
|------------+----------------|
| Kernel | Linux 6.12 (estable) |
| Boot | BIOS + UEFI |
| Drivers | Intel, AMD, NVIDIA (nouveau) |
| Firmware | WiFi, Bluetooth, todo incluido |
| Red | NetworkManager |
| SSH | Habilitado (para instalar remoto) |

*** Tu entorno completo
| Categoria | Incluido |
|-----------+----------|
| WM | XMonad + xmobar (tu config exacta) |
| Shell | Fish con tus aliases y funciones |
| Terminal | Alacritty (tu tema) |
| Editores | Doom Emacs, Neovim |
| Dev | Git, gh, nodejs, php, python... |
| Tools | 200+ paquetes que usas |

*** Herramientas extra para Live
| Tool | Uso |
|------+-----|
| GParted | Particionar discos (GUI) |
| GNOME Disks | Gestionar discos |
| Calamares | Instalador grafico |
| Firefox | Navegador |
| htop/btop | Monitorizar sistema |
| p7zip/unrar | Descomprimir |

** Generar la ISO

#+BEGIN_SRC bash
cd ~/dotfiles

# Generar ISO (primera vez tarda, luego usa cache)
nix build .#nixosConfigurations.live.config.system.build.isoImage --impure

# Ver resultado
ls -lh result/iso/
# nixos-passh-live-26.05.*-x86_64-linux.iso (~3-4 GB)
#+END_SRC

** Grabar en USB

*** Opcion 1: Ventoy (Recomendado)

[[https://www.ventoy.net/][Ventoy]] permite tener MULTIPLES ISOs en un solo USB.

#+BEGIN_SRC bash
# Solo copiar la ISO
cp result/iso/nixos-passh-live-*.iso /run/media/passh/Ventoy/
#+END_SRC

Ventajas:
- Multiples ISOs en un USB
- No hay que "quemar" nada
- Añadir/quitar ISOs es copiar/borrar archivos

*** Opcion 2: dd (USB dedicado)

#+BEGIN_SRC bash
# 1. Identificar USB (CUIDADO: no borrar disco equivocado!)
lsblk

# 2. Grabar (reemplaza sdX por tu USB)
sudo dd if=result/iso/nixos-passh-live-*.iso of=/dev/sdX bs=4M status=progress
sync
#+END_SRC

** Usar como Live

*** Bootear

1. Conectar USB
2. Reiniciar PC
3. Entrar en BIOS/UEFI (F2, F12, DEL, ESC segun fabricante)
4. Seleccionar boot desde USB
5. Elegir "NixOS Live"

*** Tu entorno arranca automaticamente

#+BEGIN_SRC
┌────────────────────────────────────────────────────────────┐
│                                                            │
│   XMonad arranca directamente                              │
│   Usuario: passh (sin password)                            │
│                                                            │
│   Keybindings:                                             │
│   ─────────────                                            │
│   Mod+Shift+Enter  → Terminal (Alacritty)                  │
│   Mod+p            → Launcher (dmenu)                      │
│   Mod+Shift+q      → Cerrar ventana                        │
│   Mod+1-9          → Cambiar workspace                     │
│                                                            │
│   WiFi:                                                    │
│   ─────                                                    │
│   nmtui            → TUI para conectar                     │
│   nm-applet        → Icono en xmobar                       │
│                                                            │
└────────────────────────────────────────────────────────────┘
#+END_SRC

** Instalar en disco

*** Opcion 1: Calamares (GUI, facil)

#+BEGIN_SRC bash
calamares
#+END_SRC

Sigue el wizard grafico: idioma → teclado → disco → usuario → instalar.

*** Opcion 2: Manual con tu Flake (recomendado)

#+BEGIN_SRC bash
# 1. Entrar como root
sudo -i

# 2. Particionar disco (ejemplo: NVMe con GPT + EFI)
parted /dev/nvme0n1 -- mklabel gpt
parted /dev/nvme0n1 -- mkpart ESP fat32 1MB 512MB
parted /dev/nvme0n1 -- set 1 esp on
parted /dev/nvme0n1 -- mkpart primary 512MB 100%

# 3. Formatear
mkfs.fat -F 32 -n BOOT /dev/nvme0n1p1
mkfs.ext4 -L nixos /dev/nvme0n1p2

# 4. Montar
mount /dev/disk/by-label/nixos /mnt
mkdir -p /mnt/boot
mount /dev/disk/by-label/BOOT /mnt/boot

# 5. Generar hardware-configuration.nix para esta maquina
nixos-generate-config --root /mnt

# 6. Instalar desde el flake (elige segun la maquina)
nixos-install --flake github:pascualmg/dotfiles#aurin
# o: nixos-install --flake github:pascualmg/dotfiles#macbook
# o: nixos-install --flake github:pascualmg/dotfiles#vespino

# 7. Poner password a tu usuario
nixos-enter --root /mnt -c 'passwd passh'

# 8. Reboot
reboot
#+END_SRC

*** Crear config para maquina nueva

Si es una maquina nueva que no es aurin/macbook/vespino:

#+BEGIN_SRC bash
# 1. Copiar hardware-configuration.nix generado
cp /mnt/etc/nixos/hardware-configuration.nix ~/dotfiles/hosts/nuevamaquina/

# 2. Crear hosts/nuevamaquina/default.nix (puede estar vacio)
mkdir -p ~/dotfiles/hosts/nuevamaquina
echo '{ }' > ~/dotfiles/hosts/nuevamaquina/default.nix

# 3. Añadir a flake.nix
# nuevamaquina = mkSystem { hostname = "nuevamaquina"; };

# 4. Commit y push
cd ~/dotfiles
git add -A && git commit -m "feat: add nuevamaquina"
git push

# 5. Instalar
nixos-install --flake github:pascualmg/dotfiles#nuevamaquina
#+END_SRC

** Compatibilidad de hardware

| Hardware | Soporte | Notas |
|----------+---------+-------|
| CPU Intel | Excelente | Cualquier generacion |
| CPU AMD | Excelente | Ryzen, EPYC, etc |
| GPU Intel | Excelente | Aceleracion completa |
| GPU AMD | Excelente | Drivers open |
| GPU NVIDIA | Basico | nouveau (sin CUDA/gaming) |
| WiFi Intel | Excelente | iwlwifi incluido |
| WiFi Atheros | Excelente | ath9k/ath10k incluido |
| WiFi Broadcom | Bueno | Firmware incluido |
| WiFi Realtek | Variable | Mayoria funciona |
| Bluetooth | Bueno | BlueZ incluido |
| NVMe/SSD/HDD | Excelente | Todos soportados |
| USB 3.x | Excelente | xHCI incluido |

*** Limitaciones

- *NVIDIA propietario*: El Live usa nouveau. Para gaming/CUDA, instala con tu config (=rtx5080.nix=, etc.)
- *Secure Boot*: Puede necesitar desactivarse en BIOS
- *Hardware muy nuevo*: Si kernel 6.12 no lo soporta, no funcionara

** Estructura de archivos

#+BEGIN_SRC
modules/iso/
└── default.nix          # Configuracion del Live ISO

flake.nix
└── nixosConfigurations
    ├── aurin            # Workstation
    ├── vespino          # Servidor
    ├── macbook          # Laptop
    └── live             # ISO booteable  ← ESTE
#+END_SRC

** Especificaciones tecnicas

| Propiedad | Valor |
|-----------+-------|
| Tipo ISO | Hibrida (BIOS + UEFI) |
| Compresion | SquashFS + zstd |
| Tamano | ~3-4 GB |
| USB minimo | 8 GB (16 GB con Ventoy) |
| RAM minima | 4 GB (8 GB recomendado) |
| Instalador | Calamares (grafico) |

** Crear tu propia Live ISO (para otros usuarios)

Si tienes tu propio flake y quieres generar tu Live ISO:

#+BEGIN_SRC nix
# En tu flake.nix, añade:
nixosConfigurations.live = nixpkgs.lib.nixosSystem {
  system = "x86_64-linux";
  modules = [
    # Base del instalador
    "${nixpkgs}/nixos/modules/installer/cd-dvd/installation-cd-graphical-calamares.nix"

    # Tu config
    ./modules/iso        # Config especifica ISO
    ./modules/base       # Tu base

    # Home manager con tu config
    home-manager.nixosModules.home-manager
    { home-manager.users.tuusuario = import ./modules/home-manager; }
  ];
};
#+END_SRC

Genera con:
#+BEGIN_SRC bash
nix build .#nixosConfigurations.live.config.system.build.isoImage --impure
#+END_SRC

* Configuraciones Principales

** Home Manager (~/.config/home-manager/home.nix) *

*Gestiona:* Instalacion de paquetes user-level con Nix

*Caracteristicas:*
- Paquetes desde =stable=, =unstable= y =master=
- Desactiva gestion de configs (usamos stow)
- 200+ paquetes instalados

*Paquetes destacados:*
| Categoria       | Paquetes                                      |
|-----------------+-----------------------------------------------|
| Shells          | fish, zsh, bash                               |
| Editores        | emacs (unstable), neovim, doom-emacs          |
| Development     | php83, nodejs_24, go, python3, rust           |
| Window Manager  | xmonad, xmobar, picom, dmenu                  |
| Tools           | docker, lazydocker, k9s, direnv               |
| Media           | vlc, spotify, obs-studio                      |
| Communication   | telegram-desktop, slack, discord              |
| AI              | ollama, claude-code                           |
| Browsers        | firefox, brave, google-chrome                 |

*Aplicar cambios:*
#+BEGIN_SRC bash
# Con flakes (recomendado) - home-manager integrado en nixos-rebuild
sudo nixos-rebuild switch --flake ~/dotfiles#aurin --impure
#+END_SRC

** NixOS Aurin (/etc/nixos/configuration.nix) *

*Ver documentacion completa:* [[file:nixos-aurin/README.org][nixos-aurin/README.org]]

*Sistema:* Dual Xeon E5-2699v3 + RTX 5080 + FiiO K7 + 128GB RAM

*Caracteristicas principales:*
- NVIDIA RTX 5080 (drivers open-source beta)
- PipeWire optimizado para FiiO K7 DAC/AMP
- Sunshine streaming server (NVENC)
- Virtualizacion KVM/QEMU + Docker
- Ollama + Open WebUI (AI local)
- Dual NIC + bridge para VMs
- NUMA optimizations para dual socket

*Scripts del sistema:*
| Comando          | Descripcion                      |
|------------------+----------------------------------|
| =aurin-info=     | Info completa del sistema        |
| =fiio-k7-test=   | Test DAC/AMP FiiO K7             |
| =sunshine-test=  | Test streaming Sunshine          |
| =xeon-stress=    | Stress test dual Xeon            |
| =numa-info=      | Info NUMA dual socket            |
| =temp-monitor=   | Monitor temperaturas             |

** XMonad (~/.config/xmonad/xmonad.hs) *

*Tiling window manager* escrito en Haskell

*Caracteristicas:*
- Layouts: Tall, ThreeCol, Grid, MultiCol
- Scratchpads: terminal, emacs, jetbrains-toolbox
- Grid selector (M-g)
- Integracion con xmobar
- Keybindings personalizados

*Keybindings importantes:*
| Tecla               | Accion                              |
|---------------------+-------------------------------------|
| =M-p=               | Dmenu (launcher)                    |
| =M-g=               | Grid selector                       |
| =M-S-<Enter>=       | Alacritty                           |
| =M-S-c=             | Cerrar ventana                      |
| =M-<Space>=         | Cambiar layout                      |
| =M-S-f=             | Fullscreen                          |
| =M-[1-9]=           | Cambiar workspace                   |
| =M-S-[1-9]=         | Mover ventana a workspace           |
| =M-j/k=             | Focus siguiente/anterior            |
| =M-h/l=             | Resize master                       |
| =M-S-a=             | Sink all (des-float todas)          |
| =M-S-s=             | Sticky window (all workspaces)      |
| =M-S-k=             | Kill sticky copies                  |
| =M-<F1>=            | Scratchpad terminal                 |
| =M-<F2>=            | Scratchpad Emacs                    |
| =M-<F3>=            | JetBrains Toolbox                   |
| =M-<Print>=         | Flameshot screenshot                |
| =XF86AudioMute=     | Toggle mute                         |
| =XF86AudioRaise=    | Volumen +5%                         |
| =XF86AudioLower=    | Volumen -5%                         |

*Dmenu:* Busca en flatpak apps + nix-profile + $PATH

*Recargar config:*
#+BEGIN_SRC bash
# Recompilar y recargar (HOT RELOAD - no pierde ventanas!)
M-q

# O manualmente
xmonad --recompile && xmonad --restart
#+END_SRC

*** Multi-Monitor y Workspaces

XMonad tiene un modelo de workspaces MUY diferente a otros WMs (GNOME, KDE, i3).
Entender esto es clave para no volverse loco con múltiples monitores.

**** El modelo "peculiar" de XMonad

#+BEGIN_SRC text
En GNOME/KDE:
  Monitor 1: WS1, WS2, WS3 (fijos a este monitor)
  Monitor 2: WS4, WS5, WS6 (fijos a este monitor)
  → Cada monitor tiene SUS workspaces

En XMonad (por defecto):
  Pool compartido: WS1, WS2, WS3, WS4, WS5, WS6, WS7, WS8, WS9
  Monitor 1: muestra WS1 (puede mostrar cualquiera)
  Monitor 2: muestra WS3 (puede mostrar cualquiera)
  → Los workspaces FLOTAN entre monitores
#+END_SRC

**** greedyView vs view (comportamiento al cambiar workspace)

**greedyView:** Los workspaces se INTERCAMBIAN

#+BEGIN_SRC text
Estado inicial:
  Monitor 1: [WS1]     Monitor 2: [WS3]

Estás en Monitor 1, pulsas M-3:
  Monitor 1: [WS3]     Monitor 2: [WS1]  ← ¡SWAP!

El WS3 viene a tu monitor, el WS1 va al otro.
#+END_SRC

**view (config actual):** Solo mueve el FOCO

#+BEGIN_SRC text
Estado inicial:
  Monitor 1: [WS1]     Monitor 2: [WS3]

Estás en Monitor 1, pulsas M-3:
  Monitor 1: [WS1]     Monitor 2: [WS3]  ← Sin cambios
                            ^
                       Foco aquí ahora

Tu cursor/foco salta al Monitor 2 donde ya está WS3.
Ideal para: "reunión en monitor 2, trabajo en monitor 1"
#+END_SRC

**** Cambiar entre view y greedyView

*Config actual usa =view=.* Para volver a =greedyView= (swap), edita =xmonad.hs=:

#+BEGIN_SRC haskell
-- Cambiar esto (en keybindings M-[1-9]):
, ("M-1", windows $ W.view "1")

-- Por esto:
, ("M-1", windows $ W.greedyView "1")
#+END_SRC

Luego =M-q= para hot-reload y probar.

**** IndependentScreens (workspaces por monitor)

Para tener workspaces INDEPENDIENTES por monitor (como GNOME/KDE):

#+BEGIN_SRC haskell
import XMonad.Layout.IndependentScreens

-- En main:
xmonad $ def
    { workspaces = withScreens 2 ["1","2","3","4","5","6","7","8","9"]
    -- ...
    }

-- Keybindings cambian:
-- M-1 en monitor 0 → workspace "0_1"
-- M-1 en monitor 1 → workspace "1_1"
#+END_SRC

Con 2 monitores tendrías 18 workspaces: =0_1= a =0_9= y =1_1= a =1_9=.

**** PhysicalScreens (navegar entre monitores)

Para teclas dedicadas a saltar entre monitores físicos:

#+BEGIN_SRC haskell
import XMonad.Actions.PhysicalScreens

-- Keybindings:
, ("M-w", viewScreen def 0)      -- Ir a monitor 0 (izquierda/principal)
, ("M-e", viewScreen def 1)      -- Ir a monitor 1 (derecha/secundario)
, ("M-r", viewScreen def 2)      -- Ir a monitor 2 (si existe)

-- Mover ventana a otro monitor:
, ("M-S-w", sendToScreen def 0)
, ("M-S-e", sendToScreen def 1)
#+END_SRC

**** CycleWS (navegación avanzada)

Para navegar solo por workspaces no vacíos o del monitor actual:

#+BEGIN_SRC haskell
import XMonad.Actions.CycleWS

-- Solo workspaces con ventanas:
, ("M-<Right>", moveTo Next NonEmptyWS)
, ("M-<Left>", moveTo Prev NonEmptyWS)

-- Mover ventana al siguiente workspace no vacío:
, ("M-S-<Right>", shiftTo Next NonEmptyWS)

-- Alternar entre los 2 últimos workspaces usados:
, ("M-<Tab>", toggleWS)
#+END_SRC

**** Configuración recomendada para ultrawide (aurin)

Para el monitor ultrawide 5120x1440 de aurin, el layout =ThreeColMid=
funciona muy bien (master en el centro, slaves a los lados):

#+BEGIN_SRC haskell
-- Ya configurado en myLayoutHook:
ThreeColMid 1 (3/100) (2/3)  -- 1 master, incremento 3%, ratio 2/3
#+END_SRC

**** xrandr: detectar y configurar monitores

#+BEGIN_SRC bash
# Ver monitores conectados
xrandr

# Configurar monitor ultrawide como primario
xrandr --output DP-4 --mode 5120x1440 --rate 120 --primary

# Añadir segundo monitor a la derecha
xrandr --output HDMI-0 --mode 1920x1080 --right-of DP-4

# Apagar monitor
xrandr --output HDMI-0 --off
#+END_SRC

**** autorandr: perfiles automáticos

Para cambiar automáticamente entre configs de monitores:

#+BEGIN_SRC bash
# Guardar configuración actual como perfil
autorandr --save trabajo

# Listar perfiles
autorandr --list

# Aplicar perfil
autorandr --load trabajo

# Detectar y aplicar automáticamente
autorandr --change
#+END_SRC

**** arandr: configuración gráfica

=arandr= es la GUI para xrandr. Permite arrastrar monitores visualmente
y genera scripts de configuración.

#+BEGIN_SRC bash
# Lanzar
arandr

# Guardar layout → genera script en ~/.screenlayout/
#+END_SRC

**** Resumen: qué elegir

| Escenario | Recomendación |
|-----------+---------------|
| Monitor único | Config actual (view) funciona igual |
| Ultrawide (aurin) | Config actual + ThreeColMid |
| Dual monitor + reunión fija | Config actual (view) ✓ |
| Quieres swap de workspaces | Cambiar a =greedyView= |
| Quieres workspaces fijos por monitor | IndependentScreens |

** Picom (~/.config/picom/picom.conf) *

*Compositor X11* para transparencias, sombras, animaciones, blur

*Caracteristicas:*
- Backend: GLX (optimizado para RTX 5080)
- VSync: enabled (120Hz)
- Refresh rate: 120
- Transparencias: terminals, rofi, etc.
- Sombras suaves en ventanas
- No borders cuando fullscreen

*Configuracion importante:*
#+BEGIN_SRC conf
backend = "glx";
vsync = true;
refresh-rate = 120;
unredir-if-possible = true;  # Mejor para gaming/video
#+END_SRC

*Recargar:*
#+BEGIN_SRC bash
killall picom
picom &
#+END_SRC

** Fish Shell (~/.config/fish/config.fish) *

*Shell moderno* con autocompletado y syntax highlighting

*Config actual:*
#+BEGIN_SRC fish
# PATH additions
set -x PATH /home/passh/.config/emacs/bin $PATH      # Doom Emacs
set -x PATH /home/passh/node_modules/.bin $PATH      # Node local
set -x PATH /home/passh/.local/bin $PATH             # Local bins
#+END_SRC

*Features:*
- =completions/= - Autocompletados personalizados
- =conf.d/= - Configs adicionales
- =functions/= - Funciones personalizadas

*Uso:*
#+BEGIN_SRC bash
# Cambiar a fish
chsh -s $(which fish)

# O usar temporalmente
fish
#+END_SRC

** Alacritty (~/.config/alacritty/) *

*Terminal emulator* GPU-accelerated

*Configuracion:* Ver =~/.config/alacritty/alacritty.yml=

*Features:*
- Renderizado GPU (OpenGL)
- Scrollback history
- True color support
- Integracion con tmux/byobu

** La Increíble XMobar (~/.config/xmobar/) *

*Status bar minimalista y elegante* para XMonad. Filosofía: solo iconos Nerd Font
y valores, sin texto redundante. Si un monitor no aplica a la máquina, simplemente
no aparece.

*** Filosofía de diseño

- *Solo iconos*: Nerd Fonts para máxima densidad de información
- *Smart monitors*: Si no aplica (ej: batería en desktop), no muestra nada (exit 0)
- *Auto-detección*: GPU detecta NVIDIA/Intel/AMD automáticamente
- *Colores con significado*: Gradiente verde→amarillo→rojo según estado
- *Mismo código, todas las máquinas*: Clone-first, sin configs por máquina

*** Vista previa

#+BEGIN_SRC text
┌──────────────────────────────────────────────────────────────────────────────┐
│ 󰡨02  󰕾124%  󰈀(192.168.2.147)  󰋊21% 48°  󰢮04% 󰔐50° 󰍛1.3G 󰚥33W  󰍛08%  󰾅1.8GHz  󰻠04% 󰔐46° │
└──────────────────────────────────────────────────────────────────────────────┘
  │      │           │              │            │                   │         │
  │      │           │              │            │                   │         └─ CPU (uso% + temp)
  │      │           │              │            │                   └─ CPU Freq (GHz + governor)
  │      │           │              │            └─ GPU (uso% temp VRAM watts)
  │      │           │              └─ Discos (uso% + temp)
  │      │           └─ Red (IP)
  │      └─ Volumen (%)
  └─ Docker (containers)
#+END_SRC

En laptops también aparecen: 󱊣 Batería, ⌨ HHKB BT, 󰤨 WiFi

*** Modos disponibles

| Modo | Descripción | Uso |
|------+-------------+-----|
| Full | Todo en una barra | Monitor único, aurin ultrawide |
| Split | Dos barras separadas | Multi-monitor, MacBook + externo |

*** Modo Split (MacBook)

#+BEGIN_SRC text
┌─────────────────────────────────────────────────────────────┐
│ 󰥔 Hora │ [1] 2 3 4 │ Layout │ Título ventana               │  ← Arriba (workspaces)
└─────────────────────────────────────────────────────────────┘
                    ... contenido ...
┌─────────────────────────────────────────────────────────────┐
│             󰡨 󰕾 󱊣 ⌨ 󰤨 󰈀 󰋊 󰢮 󰍛 󰾅 󰻠                        │  ← Abajo (monitores)
└─────────────────────────────────────────────────────────────┘
#+END_SRC

*Archivos:*
| Archivo | Posición | Contenido |
|---------+----------+-----------|
| =xmobar-workspaces.hs= | Top | Hora + Workspaces + Layout + Título |
| =xmobar-monitors.hs= | Bottom | Todos los monitores del sistema |
| =xmobar-full.hs= | (Full) | Todo junto para monitor único |

*** Monitores disponibles (scripts/xmobar-*.sh)

**** Hardware (siempre visibles)

| Monitor | Script | Icono | Descripción |
|---------+--------+-------+-------------|
| CPU | =xmobar-cpu.sh= | 󰻠 | Uso % + temperatura |
| CPU Freq | =xmobar-cpu-freq.sh= | 󰾅 | Frecuencia + governor |
| RAM | =xmobar-memory.sh= | 󰍛 | Uso con gradiente |
| GPU | =xmobar-gpu.sh= | 󰢮 | Uso, temp, VRAM, watts (NVIDIA/Intel/AMD) |
| Discos | =xmobar-disks.sh= | 󰋊 | Uso % + temperatura NVMe |
| Volumen | =xmobar-volume.sh= | 󰕾 | Nivel PipeWire con gradiente |
| Red | =xmobar-network.sh= | 󰈀󰖩 | Tráfico en tiempo real (󰁞down 󰁆up) con gradientes + IP |

**** Sistema (nuevos 2026-01)

| Monitor | Script | Icono | Descripción | Smart |
|---------+--------+-------+-------------+-------|
| Load | =xmobar-load.sh= | 󰊚 | Load average (1min) | Verde < 70%, amarillo < 100%, rojo >= cores |
| Swap | =xmobar-swap.sh= | 󰾴 | Uso swap en MB | Solo si se está usando swap |
| Uptime | =xmobar-uptime.sh= | 󰅐 | Tiempo activo (1h52m) | Siempre visible |
| Brightness | =xmobar-brightness.sh= | 󰃠󰃟󰃞 | Brillo pantalla % | Solo en laptops con brightnessctl |
| Updates | =xmobar-updates.sh= | 󰚰 | Días desde flake.lock update | Solo si > 7 días (warning) |

**** Conectividad (nuevos 2026-01)

| Monitor | Script | Icono | Descripción | Smart |
|---------+--------+-------+-------------+-------|
| VPN | =xmobar-vpn.sh= | 󰖂 | Pulse Secure activo | Solo si conectado |
| Bluetooth | =xmobar-bluetooth.sh= | 󰂯 | Dispositivos BT conectados | Solo si hay dispositivos |
| SSH | =xmobar-ssh.sh= | 󰣀 | Sesiones SSH (↓entrantes ↑salientes) | Solo si hay sesiones |
| Machines | =xmobar-machines.sh= | ●/○ | Estado máquinas remotas | ●=online ○=offline |
| Keyboard | =xmobar-keyboard-layout.sh= | US/ES | Layout teclado actual | Siempre visible |

**** Dispositivos (smart - se ocultan si no hay hardware)

| Monitor | Script | Icono | Descripción | Smart |
|---------+--------+-------+-------------+-------|
| Docker | =xmobar-docker.sh= | 󰡨 | Containers activos | Si no hay Docker/containers: oculto |
| Batería | =xmobar-battery.sh= | 󱊣 | Carga + estado (charging/discharging) | Si no hay batería: oculto |
| HHKB Hybrid | =xmobar-hhkb-battery.sh= | ⌨ | Batería teclado HHKB Hybrid BT | Si no hay HHKB BT: oculto |
| HHKB Pro | =xmobar-hhkb-hasu.sh= | ⌨Pro | HHKB con Hasu controller (USB) | Si no hay USB 4848:0001: oculto |
| Ratón | =xmobar-mouse-battery.sh= | 󰍽 | Batería ratón Logitech (HID++) | Si no hay ratón Logitech: oculto |
| WiFi | =xmobar-wifi.sh= | 󰤨 | SSID red WiFi | Si no hay WiFi/conectado: oculto |

*** GPU: Auto-detección (xmobar-gpu.sh)

El script unificado detecta automáticamente el tipo de GPU:

#+BEGIN_SRC bash
# Prioridad: NVIDIA > AMD > Intel
# En aurin (RTX 5080): usa nvidia-smi
# En macbook (Intel): usa /sys/class/drm/card*/gt_cur_freq_mhz
# Si no hay GPU dedicada: no muestra nada
#+END_SRC

Output ejemplo NVIDIA:
#+BEGIN_SRC text
󰢮04% 󰔐50° 󰍛1.3G 󰚥33W   (uso% temp VRAM watts)
#+END_SRC

*** Red: Monitor de Tráfico en Tiempo Real (xmobar-network.sh)

*Nuevo en 2026-01-27* - Monitor de red con tasas de descarga/subida en tiempo real y gradientes de color.

**** Características

| Feature | Descripción |
|---------+-------------|
| *Tasas en tiempo real* | Muestra 󰁞 download y 󰁆 upload en B/s, KB/s, MB/s |
| *Gradientes suaves* | Verde (bajo) → Amarillo (medio) → Naranja (alto) → Rojo (saturado) |
| *Iconos con color* | Los iconos 󰁞 y 󰁆 cambian de color según el tráfico |
| *Auto-detección* | Detecta WiFi (wl*, wlan*) o Ethernet (en*, eth*) automáticamente |
| *Filtrado inteligente* | Excluye br*, vnet*, tun*, docker*, virbr* (bridges, VPN, Docker) |
| *Precisión alta* | Usa milisegundos para cálculos precisos |
| *Clone-first* | Funciona en aurin (Ethernet), macbook (WiFi), vespino (Ethernet) |

**** Output Ejemplos

#+BEGIN_SRC text
# WiFi con señal + tráfico bajo
󰖩 75% 󰁞15KB 󰁆2KB (192.168.1.50)
  │    │  │       │     └─ IP (verde hacker)
  │    │  │       └─ Upload (verde = bajo)
  │    │  └─ Download (verde = bajo)
  │    └─ Señal WiFi 75% (verde = buena)
  └─ Icono WiFi

# Ethernet con tráfico medio
󰈀 󰁞2.5M 󰁆500K (192.168.1.100)
│  │      │      └─ IP
│  │      └─ Upload (amarillo-naranja = medio)
│  └─ Download (amarillo = medio)
└─ Icono Ethernet

# Tráfico alto (download de ISO)
󰈀 󰁞10M 󰁆5M (192.168.1.100)
│  │     │    └─ Upload (rojo = alto)
│  └─ Download (rojo = saturado ≥10MB/s)
#+END_SRC

**** Escala de Colores

| Tasa | Color | Hex | Visual |
|------+-------+-----+--------|
| 0-500 KB/s | Verde | =#98c379= | 🟢 Tráfico bajo |
| 500KB-2.5MB/s | Verde-amarillo | =#aac379= → =#c8c179= | 🟡 Tráfico moderado |
| 2.5MB-5MB/s | Amarillo-naranja | =#e5c07b= → =#f8bf7a= | 🟠 Tráfico alto |
| 5MB-10MB/s | Naranja-rojo | =#d19a66= → =#e39979= | 🔴 Tráfico muy alto |
| 10MB/s+ | Rojo | =#e06c75= | 🔴 Saturado |

**** Detalles Técnicos

#+BEGIN_SRC bash
# Cache por interfaz (evita race conditions)
/tmp/xmobar-network/enp7s0    # Ethernet
/tmp/xmobar-network/wlp0s20f0u7u4  # WiFi

# Contenido del cache (rx_bytes tx_bytes timestamp_ms)
19563831591 1446526760 1769543877123

# Cálculo de rate (bytes/s)
rx_rate = (rx_bytes_current - rx_bytes_prev) / time_diff_seconds
tx_rate = (tx_bytes_current - tx_bytes_prev) / time_diff_seconds

# Lectura de stats del kernel
/sys/class/net/IFACE/statistics/rx_bytes
/sys/class/net/IFACE/statistics/tx_bytes

# Intervalo de actualización
10 segundos (configurable en xmobar-monitors.hs)
#+END_SRC

**** Troubleshooting

| Problema | Solución |
|----------+----------|
| Muestra =0B 0B= siempre | Normal en primer run (no hay datos previos) |
| No aparece nada | No hay interfaz con IP activa |
| Muestra =br0= o =tun0= | Bug - reportar (deberían estar filtrados) |
| Números muy bajos | Normal si no hay tráfico activo |
| Colores no cambian | Generar tráfico: =curl https://ubuntu.com/...iso= |

*** HHKB Pro con Hasu Controller (xmobar-hhkb-hasu.sh)

*Nuevo en 2026-01-27* - Monitor para el HHKB original con Hasu controller conectado por USB.

**** Características

| Feature | Descripción |
|---------+-------------|
| *Detección USB* | Detecta USB ID =4848:0001= (Hasu controller) |
| *Icono distintivo* | Muestra =⌨Pro= en cyan brillante (#8be9fd) |
| *5 interfaces HID* | El Hasu expone teclado + mouse + system + consumer + extra |
| *Mouse Layer* | Emulación de ratón con =hold ;= (tu keymap "ultimate") |
| *Smart* | Se oculta si el HHKB no está conectado |

**** Interfaces HID del Hasu Controller

El HHKB con Hasu controller expone 5 interfaces diferentes:

#+BEGIN_SRC text
input7  → HHKB ANSI              (teclado principal)
input8  → HHKB ANSI Mouse        (mouse layer - hold ; para activar)
input9  → HHKB ANSI System       (power/sleep/wake)
input10 → HHKB ANSI Consumer     (media keys: play/pause/vol)
input11 → HHKB ANSI Keyboard     (teclado secundario para layers)
#+END_SRC

**** Output

#+BEGIN_SRC text
# Conectado por USB
⌨Pro    (cyan #8be9fd)

# No conectado
(vacío - el monitor se oculta)
#+END_SRC

**** Diferencia con HHKB Hybrid

| Modelo | Script | Detección | Info |
|--------+--------+-----------+------|
| HHKB Hybrid | =xmobar-hhkb-battery.sh= | BT =04fe:000b= | Batería % |
| HHKB Pro (Hasu) | =xmobar-hhkb-hasu.sh= | USB =4848:0001= | Estado conexión |

**** Verificar conexión

#+BEGIN_SRC bash
# Ver si el HHKB con Hasu está conectado
lsusb | grep 4848:0001

# Ver las 5 interfaces HID
cat /proc/bus/input/devices | grep -A5 "HHKB"

# Test del monitor
~/dotfiles/scripts/xmobar-hhkb-hasu.sh
#+END_SRC

*** Test en terminal: xmobar-test-all.sh

Script para ver todos los monitores en la terminal con los mismos colores que xmobar.
Se ejecuta automáticamente al abrir cada terminal (fish greeting).

#+BEGIN_SRC bash
# Ejecutar manualmente
~/dotfiles/scripts/xmobar-test-all.sh

# Output (con colores ANSI truecolor):
󰡨02  󰕾124%  󰈀(192.168.2.147)  󰋊21% 48°  󰢮04% 󰔐50° 󰍛1.3G 󰚥33W  󰍛08%  󰾅1.8GHz  󰻠04% 󰔐46°
#+END_SRC

*Magia del color*: Convierte tags xmobar =<fc=#RRGGBB>= a ANSI truecolor usando Perl:

#+BEGIN_SRC perl
# <fc=#56b6c2>text</fc> → \e[38;2;86;182;194mtext\e[0m
s/<fc=#(..)(..)(..)>([^<]*)<\/fc>/chr(27)."[38;2;".hex($1).";".hex($2).";".hex($3)."m".$4.chr(27)."[0m"/ge
#+END_SRC

*** Sistema de colores (xmobar-colors.sh)

Todos los scripts usan =xmobar-colors.sh= para gradientes consistentes:

#+BEGIN_SRC bash
# Colores One Dark
COLOR_GREEN="#98c379"   # Bueno (>70%)
COLOR_YELLOW="#e5c07b"  # Warning (30-70%)
COLOR_ORANGE="#d19a66"  # Alto
COLOR_RED="#e06c75"     # Crítico (<30%)

# Funciones
pct_to_color 75          # Devuelve color según % (0=verde, 100=rojo)
pct_to_color_inverse 75  # Inverso (0=rojo, 100=verde) - para batería
#+END_SRC

*** Recargar

#+BEGIN_SRC bash
# Recargar solo xmobar (sin reiniciar xmonad)
killall xmobar && xmonad --restart

# O simplemente
M-q  # Recompila y reinicia xmonad (relanza xmobars)
#+END_SRC

*** Personalización por máquina

En =modules/home-manager/machines/<maquina>.nix=:

#+BEGIN_SRC nix
dotfiles.xmobar = {
  enable = true;
  fontSize = 22;           # MacBook HiDPI
  gpuType = "intel";       # intel | nvidia | none
  showBattery = true;      # Laptops
  showWirelessMouse = false;
};
#+END_SRC

** Scripts (~/scripts/) .

*Scripts utiles del sistema*

| Script                   | Descripcion                               |
|--------------------------+-------------------------------------------|
| =vm-backupeitor.sh=      | Backup/restore VMs libvirt (XML + QCOW2)  |
| =bootstrap-doom.sh=      | Setup inicial Doom Emacs en máquinas nuevas |
| =sync-wallpapers.sh=     | Sincronizar wallpapers                    |
| =xmobar-*.sh=            | Scripts de monitorización para xmobar     |
| =xmobar-hhkb-battery.sh= | Batería HHKB Hybrid (BT) con colores      |
| =trayer-*.sh=            | Gestión de trayer (system tray)           |
| =wireless-mouse.sh=      | Monitor batería ratón wireless            |

** Wallpapers (~/wallpapers/) .

Fondos de pantalla

*Gestion:*
- Nitrogen (GUI)
- O directamente con =feh=

** Shell (bashrc/zshrc) .

*Posiblemente legacy* - Fish es el shell principal

Contiene:
- =.bashrc= - Configuracion Bash
- =.zshrc= - Configuracion Zsh

*Usar?* Probablemente no si usas Fish, pero mantienen compatibilidad

** Composer (~/.config/composer/) .

*PHP Composer* config

Probablemente para desarrollo PHP (tienes php83 en home.nix)

* Guia Stow - Gestion de Dotfiles

** Que es Stow?

GNU Stow crea *enlaces simbolicos* desde tu dotfiles repo a tu $HOME

** Comandos Esenciales

#+BEGIN_SRC bash
cd ~/dotfiles

# Crear enlaces (primera vez)
stow PAQUETE

# Quitar enlaces
stow -D PAQUETE

# Recrear enlaces (cuando cambias cosas)
stow -R PAQUETE

# Ver que va a hacer sin ejecutar
stow -n PAQUETE

# Verbose (siempre recomendado)
stow -v PAQUETE
#+END_SRC

** Workflows Comunes

*** Activar configuracion por primera vez:
#+BEGIN_SRC bash
cd ~/dotfiles
stow -v xmonad
stow -v fish
#+END_SRC

*** Actualizar configuracion existente:
#+BEGIN_SRC bash
# Editar archivo en dotfiles
vim ~/dotfiles/fish/.config/fish/config.fish

# Recrear enlaces (por si acaso)
stow -R fish
#+END_SRC

*** Para NixOS (necesita root y target /):
#+BEGIN_SRC bash
sudo stow -v -R -t / nixos-aurin

# Verificar
ls -la /etc/nixos/

# Aplicar cambios
sudo nixos-rebuild switch
#+END_SRC

** Flags Explicadas

| Flag     | Significado  | Uso                              |
|----------+--------------+----------------------------------|
| =-v=     | Verbose      | Ver que hace stow                |
| =-R=     | Restow       | Quitar y recrear enlaces         |
| =-D=     | Delete       | Solo quitar enlaces              |
| =-t DIR= | Target       | Directorio donde crear enlaces   |
| =-n=     | No execute   | Simular sin ejecutar (dry-run)   |

** Pro Tips

- *Nunca uses =sudo stow=* para archivos de usuario (home)
- *Siempre usa =sudo stow -t /=* para archivos del sistema (/etc)
- *Usa =-R=* cuando dudes, es mas seguro
- *Usa =-v=* siempre para ver que hace
- *Usa =-n=* para testing antes de ejecutar
- *Git tu dotfiles* para backup/sync

** Troubleshooting

*** "Conflicts with existing target"
#+BEGIN_SRC bash
# Ver que conflicta
stow -v -n PAQUETE

# Quitar archivo manual (backup primero)
mv ~/.config/fish/config.fish ~/.config/fish/config.fish.backup

# Volver a stow
stow fish
#+END_SRC

*** "Permission denied"
#+BEGIN_SRC bash
# Para archivos del sistema (/etc)
sudo stow -t / PAQUETE

# Para archivos de usuario (nunca uses sudo)
stow PAQUETE
#+END_SRC

*** Ver enlaces existentes
#+BEGIN_SRC bash
# Ver todos los symlinks en home
find ~ -maxdepth 3 -type l -ls | grep dotfiles

# Ver especifico
ls -la ~/.config/fish/config.fish
#+END_SRC

* Git Workflow

#+BEGIN_SRC bash
cd ~/dotfiles

# Ver cambios
git status
git diff

# Commit cambios
git add .
git commit -m "Update xmonad keybindings"
git push

# En otra maquina
git pull
stow -R xmonad  # Recrear enlaces
#+END_SRC

** Credenciales Git Persistentes

Las credenciales de git se almacenan permanentemente gracias a =credential.helper = store=
configurado en =modules/home-manager/passh.nix=.

*Ubicacion:* =~/.git-credentials=

*Formato del archivo:*
#+BEGIN_SRC text
https://usuario:password@bitbucket.vocento.com
https://usuario:token@github.com
#+END_SRC

*Primera vez:* Git pedira credenciales una vez y las guardara automaticamente.

*Regenerar credenciales:*
#+BEGIN_SRC bash
# Borrar credenciales existentes
rm ~/.git-credentials

# El proximo git push/pull pedira credenciales de nuevo
#+END_SRC

*Seguridad:* El archivo tiene permisos =600= (solo el usuario puede leer/escribir).

* Comandos Mas Usados

#+BEGIN_SRC bash
# Aplicar NixOS config (FLAKES - metodo actual)
sudo nixos-rebuild switch --flake ~/dotfiles#aurin --impure
sudo nixos-rebuild switch --flake ~/dotfiles#macbook
sudo nixos-rebuild switch --flake ~/dotfiles#vespino --impure

# Test sin aplicar permanentemente
sudo nixos-rebuild test --flake ~/dotfiles#aurin --impure

# Rollback
sudo nixos-rebuild switch --rollback

# Activar config stow (solo para xmonad ya)
stow -v -R xmonad
#+END_SRC

* Setup Completo Nueva Maquina

#+BEGIN_SRC bash
# 1. Clonar dotfiles
git clone <repo-url> ~/dotfiles
cd ~/dotfiles

# 2. Aplicar NixOS config (incluye home-manager)
# Cambiar 'aurin' por tu hostname
sudo nixos-rebuild switch --flake ~/dotfiles#aurin --impure

# 3. Aplicar xmonad (unico que aun usa stow)
stow -v xmonad

# 4. Recargar shell
fish

# 5. Reiniciar sesion X (logout/login) o:
xmonad --recompile && xmonad --restart
#+END_SRC

* Notas de Desarrollo

** Home Manager vs NixOS config

- *NixOS config* (=/etc/nixos/configuration.nix=): Sistema base, servicios, hardware
- *Home Manager* (=~/.config/home-manager/home.nix=): Paquetes usuario, configs user-level

** Channels

#+BEGIN_SRC bash
# Ver channels
nix-channel --list

# Deberia mostrar:
# home-manager https://github.com/nix-community/home-manager/archive/release-25.05.tar.gz
#+END_SRC

** Direnv

Tienes =direnv= instalado. Usa =.envrc= en proyectos para cargar variables automaticamente:

#+BEGIN_SRC bash
# En directorio proyecto
echo "use nix" > .envrc
direnv allow
#+END_SRC

* Ejecutar Programas sin Instalar: nix-index, comma y mas

Esta es probablemente la feature mas util de NixOS para el dia a dia.

** El Problema: "command not found"

Imagina este escenario clasico:

#+BEGIN_SRC bash
$ cowsay "hola"
bash: cowsay: command not found
#+END_SRC

En sistemas tradicionales (Ubuntu, Debian), tendrias que:
1. Buscar en Google que paquete contiene =cowsay=
2. Instalar el paquete con =apt install cowsay=
3. Ahora si, ejecutar =cowsay=

Esto es tedioso. Y en NixOS, donde no usamos =apt=, la solucion historica era
aun mas engorrosa. Pero hoy tenemos herramientas *mucho mejores*.

** La Solucion Moderna: comma (,)

#+BEGIN_SRC bash
# En vez de instalar nada, simplemente:
$ , cowsay "hola"
 ______
< hola >
 ------
        \   ^__^
         \  (oo)\_______
            (__)\       )\/\
                ||----w |
                ||     ||
#+END_SRC

*Si, es literal una coma seguida del comando.* comma busca automaticamente
que paquete contiene el binario, lo descarga temporalmente, y lo ejecuta.
No queda instalado. Magia.

** Historia: Como Llegamos Aqui

*** Era 1: command-not-found de Ubuntu

Ubuntu popularizo un sistema ingenioso: cuando escribias un comando que no
existia, te sugeria que paquete instalar. NixOS intento copiarlo con
=programs.command-not-found.enable = true=, pero la base de datos estaba
frecuentemente rota o desactualizada.

*** Era 2: nix-index (hazlo tu mismo)

Alguien penso: "Y si generamos nuestra propia base de datos indexando TODO nixpkgs?"

#+BEGIN_SRC bash
$ nix-index  # ADVERTENCIA: tarda 30-60 minutos!
#+END_SRC

Funcionaba, pero nadie quiere esperar una hora cada vez que necesita actualizar.

*** Era 3: nix-index-database (la comunidad al rescate)

La comunidad penso: "Por que cada usuario genera el mismo indice? Hagamoslo
una vez y compartamoslo."

#+BEGIN_SRC text
                  ANTES                         AHORA
                  -----                         -----
Usuario A:  nix-index (60 min)        Descarga DB precompilada (30 seg)
Usuario B:  nix-index (60 min)        Descarga DB precompilada (30 seg)
Usuario C:  nix-index (60 min)        Descarga DB precompilada (30 seg)
#+END_SRC

*** Era 4: comma (el atajo definitivo)

Y finalmente, comma une todo: busca en la DB + descarga + ejecuta, en un comando.

** Las Diferentes Formas de Ejecutar Programas

*** Metodo 1: =, comando= (comma) - USO DIARIO

#+BEGIN_SRC bash
$ , cowsay "el metodo mas facil"
$ , ffmpeg -i video.mp4 -c:v libx264 output.mp4
$ , ncdu /home  # analizar disco sin instalar ncdu
#+END_SRC

Busqueda automatica, cero friccion. *Este es el que usaras el 90% del tiempo.*

*** Metodo 2: =nix shell nixpkgs#= - Shell interactivo

#+BEGIN_SRC bash
$ nix shell nixpkgs#cowsay nixpkgs#lolcat
[shell]$ cowsay "tengo ambos disponibles" | lolcat
[shell]$ exit
#+END_SRC

Util cuando necesitas varios programas juntos en un shell temporal.

*** Metodo 3: =nix run nixpkgs#= - Ejecutar una vez

#+BEGIN_SRC bash
$ nix run nixpkgs#cowsay -- "ejecuto y termino"
$  # Ya volviste a tu shell normal
#+END_SRC

Similar a comma pero especificas el paquete exacto.

*** Metodo 4: =nix-shell -p= - Legacy pero funciona

#+BEGIN_SRC bash
$ nix-shell -p python3 python3Packages.requests
[nix-shell]$ python3 script.py
[nix-shell]$ exit
#+END_SRC

El metodo antiguo (pre-flakes). Todavia util para scripts con shebangs.

** Cuando Usar Cual

#+BEGIN_SRC text
Quiero ejecutar un programa que no tengo instalado
                    |
                    v
        Se que paquete es? ----No----> Usa: , comando
                    |
                   Si
                    |
                    v
        Necesito shell interactivo? --No--> nix run nixpkgs#pkg
                    |
                   Si
                    |
                    v
              nix shell nixpkgs#pkg
#+END_SRC

| Metodo              | Flakes | Interactivo | Busqueda | Caso de Uso        |
|---------------------+--------+-------------+----------+--------------------|
| =, comando=         | Si     | No          | Auto     | *USO DIARIO*       |
| =nix shell nixpkgs#= | Si     | Si          | Manual   | Shell temporal     |
| =nix run nixpkgs#=  | Si     | No          | Manual   | Ejecutar una vez   |
| =nix-shell -p=      | No     | Si          | Manual   | Scripts, legacy    |

** Buscar Paquetes con nix-locate

#+BEGIN_SRC bash
# Que paquete tiene el comando 'rg'?
$ nix-locate bin/rg
ripgrep.out        /nix/store/...-ripgrep-14.1/bin/rg

# Que paquete tiene libcurl?
$ nix-locate 'lib/libcurl.so'
curl.out           /nix/store/...-curl-8.5/lib/libcurl.so
#+END_SRC

** Configuracion en Este Repo

En el =flake.nix=, nix-index-database esta como input:

#+BEGIN_SRC nix
inputs = {
  nix-index-database = {
    url = "github:nix-community/nix-index-database";
    inputs.nixpkgs.follows = "nixpkgs";
  };
};
#+END_SRC

Y se habilita para todas las maquinas:

#+BEGIN_SRC nix
programs.nix-index-database.comma.enable = true;
#+END_SRC

La base de datos se actualiza automaticamente cuando haces =nix flake update=.

** Ejemplos Practicos de Uso Diario

#+BEGIN_SRC bash
# Probar un programa antes de instalarlo
$ , neofetch

# Convertir video (sin tener ffmpeg instalado)
$ , ffmpeg -i input.mp4 -c:v libx265 output.mp4

# Analizar uso de disco
$ , ncdu /home

# Generar QR code
$ , qrencode -o qr.png "https://example.com"

# Ver arbol de procesos
$ , pstree

# Benchmark rapido
$ , hyperfine 'sleep 0.1' 'sleep 0.2'
#+END_SRC

** Scripts Reproducibles con Dependencias

Puedes crear scripts que traen sus propias dependencias:

#+BEGIN_SRC bash
#!/usr/bin/env nix-shell
#!nix-shell -i bash -p cowsay lolcat

# Este script se auto-provee cowsay y lolcat
cowsay "Mis dependencias vienen conmigo" | lolcat
#+END_SRC

O con flakes (mas moderno):

#+BEGIN_SRC bash
#!/usr/bin/env -S nix shell nixpkgs#cowsay nixpkgs#lolcat --command bash

cowsay "Version con flakes" | lolcat
#+END_SRC

** Troubleshooting

*** nix-locate no encuentra nada

#+BEGIN_SRC bash
# Actualizar la base de datos
$ nix flake update nix-index-database ~/dotfiles
$ sudo nixos-rebuild switch --flake ~/dotfiles#aurin
#+END_SRC

*** comma pregunta multiples opciones

#+BEGIN_SRC bash
$ , python
Multiple packages provide 'python':
  1) python2
  2) python3
Select [1-3]:
#+END_SRC

Solucion: especifica el paquete exacto con =, python3 script.py=

* Multiples Versiones de nixpkgs: pkgs vs pkgsMaster

A veces necesitas un paquete mas nuevo de lo que hay en nixpkgs-unstable.
Este repo usa un sistema multi-channel para resolver eso.

** El Problema

#+BEGIN_SRC text
nixpkgs-master  -->  claude-code 2.1.6  (commits de hoy)
     |
     | ~2-7 dias de propagacion
     v
nixpkgs-unstable --> claude-code 2.1.2  (lo que tienes normalmente)
#+END_SRC

Los paquetes fluyen: master -> unstable -> stable. Puede tardar dias o semanas.

** La Solucion: Dos Fuentes de Paquetes

En =flake.nix= tenemos dos inputs de nixpkgs:

#+BEGIN_SRC nix
inputs = {
  nixpkgs.url = "github:NixOS/nixpkgs/nixos-unstable";
  nixpkgs-master.url = "github:NixOS/nixpkgs/master";  # Bleeding edge
};
#+END_SRC

Y exponemos ambos a todos los modulos:

#+BEGIN_SRC nix
# En el flake
pkgs = import nixpkgs { ... };          # 99% de paquetes
pkgsMaster = import nixpkgs-master { ... };  # Paquetes nuevos

specialArgs = { inherit pkgsMaster; };  # Disponible en todos los modulos
#+END_SRC

** Uso en Practica

#+BEGIN_SRC nix
# En cualquier modulo (configuration.nix, passh.nix, etc.)
{ pkgs, pkgsMaster, ... }:
{
  home.packages = with pkgs; [
    firefox        # De unstable (estable, testeado)
    git
    python3
  ] ++ [
    pkgsMaster.claude-code    # De master (bleeding edge)
    pkgsMaster.jetbrains-toolbox
  ];
}
#+END_SRC

** Cuando Usar Cada Uno

| Fuente      | Usar para                              | Riesgo      |
|-------------+----------------------------------------+-------------|
| =pkgs=      | 99% de paquetes, uso normal            | Bajo        |
| =pkgsMaster= | Paquetes que necesitas YA (nuevos)     | Medio-alto  |

*Nota:* =pkgsMaster= puede tener paquetes rotos ya que no han pasado todos los tests.
Usalo solo cuando realmente necesites la ultima version.

** Actualizar Independientemente

#+BEGIN_SRC bash
# Actualizar solo master (para paquetes nuevos)
nix flake lock --update-input nixpkgs-master ~/dotfiles

# Actualizar solo unstable (actualizacion normal)
nix flake lock --update-input nixpkgs ~/dotfiles

# Actualizar todo
nix flake update ~/dotfiles
#+END_SRC

* NixOS MacBook Pro 13,2 (2016)

** Hardware Status

| Componente         | Estado | Notas                                          |
|--------------------+--------+------------------------------------------------|
| Audio (CS8409)     | OK     | Driver snd-hda-macbookpro (davidjo)            |
| WiFi (BCM43602)    | NO     | Driver wl abandonado 2019, usar USB dongle     |
| Touch Bar (T1)     | NO     | Chip T1 en recovery mode, workaround con keyd  |
| Webcam FaceTime HD | NO     | Conectada via T1, requiere reset del T1        |
| Teclado/Trackpad   | OK     | Drivers applespi + keyd (fix ISO swap)         |
| Pantalla HiDPI     | OK     | 168 DPI, scaling 2x en GNOME                   |
| Bateria            | OK     | ACPI estandar                                  |
| F-keys (Fn+numero) | OK     | keyd: Fn+1=F1, Fn+=Esc, etc.                   |

** Audio CS8409 - Solucion

El MacBook Pro 2016 usa un chip Cirrus Logic CS8409 como bridge HDA.
El driver del kernel solo soporta Dell (subsystem IDs diferentes).

*Solucion implementada:*
- Modulo custom =snd-hda-macbookpro= compilado desde [[https://github.com/davidjo/snd_hda_macbookpro][davidjo/snd_hda_macbookpro]]
- Parchea el driver del kernel para anadir soporte Apple (GPIO init)
- Config en =nixos-macbook/etc/nixos/modules/snd-hda-macbookpro.nix=

*Verificar funcionamiento:*
#+BEGIN_SRC bash
# Diagnostico completo
audio-diag

# GPIO debe mostrar enable=1 para IO[0-3]
cat /proc/asound/card0/codec#0 | grep -A 10 "^GPIO:"

# Test altavoces
speaker-test -c 2 -t wav
#+END_SRC

** WiFi BCM43602 - Sin solucion

El chip Broadcom BCM43602 interno requiere el driver =wl= (broadcom-sta).

*Estado del driver:*
- El modulo *compila* correctamente (mantenido por RPMFusion/Debian)
- Pero *falla en runtime* con kernel 6.x: =wl driver failed with code 1=
- Error: =NULL ndev->ieee80211ptr, unable to deref wl=
- Causa: APIs internas del kernel incompatibles con el driver propietario

*Conclusion:* El driver esta muerto a nivel funcional, no solo de mantenimiento.
No hay solucion posible sin que Broadcom libere codigo o alguien haga ingenieria inversa.

*Alternativa funcional:* USB WiFi dongle con chip Realtek (RTL8xxxu)
- Ejemplo: TP-Link TL-WN725N o similar
- Funciona out-of-the-box con driver =rtl8xxxu= del kernel

** Touch Bar T1 - Sin solucion

El chip T1 (Touch Bar + Touch ID + *FaceTime HD Camera*) esta en "Recovery Mode" permanente.

*Dispositivos conectados al T1:*
- Touch Bar (OLED strip)
- Touch ID (sensor huella)
- FaceTime HD Camera (720p)
- Ambient Light Sensor

*Problema:*
- El T1 aparece como =05ac:1281= (DFU/Recovery) en lugar de =05ac:8600= (iBridge)
- El firmware esta corrupto o nunca fue inicializado
- Los drivers Linux (apple-ibridge, tiny-dfr, facetimehd) requieren el T1 en modo normal
- Sin T1 funcional, la camara no es visible para el sistema (no hay =/dev/video*=)

*Unica solucion:*
- Conectar a un Mac con macOS
- Usar Apple Configurator 2 para restaurar el firmware del T1
- Sin acceso a macOS = sin Touch Bar

*Workaround con keyd:*
- =Fn + = (tecla junto al 1) = Escape
- =Fn + 1-0= = F1-F10
- =Fn + -= = F11, =Fn + == = F12
- =Fn + Backspace= = Delete
- =Fn + Flechas= = Home/End/PageUp/PageDown

Configurado en =services.keyd= en configuration.nix.

** Teclado Spanish ISO - Fix teclas swapeadas

El teclado Apple SPI tiene las teclas = a= y =< >= intercambiadas respecto al layout fisico.

*Solucion:* keyd remapea las teclas a nivel kernel:
#+BEGIN_SRC nix
"grave" = "102nd";  # Swap tecla junto al 1
"102nd" = "grave";  # Swap tecla entre Shift y Z
#+END_SRC

Esto funciona tanto en X11 (XMonad) como en Wayland (GNOME) porque keyd opera a nivel evdev.

** Perifericos de Input

*** Teclados HHKB

- *HHKB original*: Hasu controller (QMK) - layout programado en hardware
- *HHKB Hybrid*: BT + USB-C, Topre switches

Filosofía: *el hardware manda, no el software*. Config en memoria del teclado.

*** Ratón: Logitech G Pro X Superlight 2

| Conexión | Device ID | Nombre |
|----------+-----------+--------|
| Lightspeed (receptor USB) | =046d:c54d= | Logitech USB Receiver |
| Cable USB | =046d:c09b= | Logitech PRO X 2 |
| Bluetooth | - | No probado |

*Estado soporte Linux (2026-01):*
- =libratbag= / =piper=: NO soportado
- =solaar=: NO soportado
- El ratón es muy nuevo para las herramientas open source

*Configurar DPI:*
1. Conectar a Windows (VM o dual boot)
2. Instalar Logitech G Hub
3. Configurar DPI y guardar en *onboard memory*
4. El DPI persiste en Linux (guardado en el ratón)

*Config libinput* (ya aplicada):
#+BEGIN_SRC nix
# Aceleración plana (raw input) - ideal para gaming
services.libinput.mouse.accelProfile = "flat";
#+END_SRC

*** Trackpad MacBook - Gestures

Solo en MacBook. Usa =libinput-gestures= para gestos multi-touch en XMonad.

| Gesto               | Acción                              |
|---------------------+-------------------------------------|
| 3 dedos izquierda   | Workspace siguiente (estilo macOS)  |
| 3 dedos derecha     | Workspace anterior (estilo macOS)   |
| 4 dedos arriba      | Toggle scratchpad Emacs (Mod+e)     |
| 4 dedos abajo       | Toggle scratchpad Terminal (Mod+a)  |

*Habilitar en* =machines/macbook.nix=:
#+BEGIN_SRC nix
dotfiles.libinput-gestures.enable = true;
#+END_SRC

*Config:* =modules/home-manager/programs/libinput-gestures.nix=

** Solucion: xmobar y fuentes HiDPI

*Estado:* RESUELTO

El problema era usar sintaxis *Xft* (=xft:Font:size=X=) que no escala bien en HiDPI.

*Solucion:* Usar sintaxis *Pango* que SI respeta el DPI del sistema:

#+BEGIN_SRC haskell
-- ANTES (Xft - NO funciona en HiDPI):
font = "xft:Monoid Nerd Font:size=16:bold"

-- DESPUES (Pango - funciona perfectamente):
font = "HeavyData Nerd Font 16"
#+END_SRC

El modulo =modules/home-manager/programs/xmobar.nix= usa Pango y calcula
automaticamente la altura de la barra segun el =fontSize=:

#+BEGIN_SRC nix
# En machines/macbook.nix
dotfiles.xmobar = {
  fontSize = 22;  # Pango escala con DPI automaticamente
};
#+END_SRC

La formula: =TopH = fontSize * 1.8= ajusta la altura de barra proporcionalmente.

** Consola TTY: Temas y Fuentes (2026-01-20)

*Estado:* FUNCIONAL con workaround

*** El problema

El EFI de Apple reporta resolucion incorrecta (3360x2100 en vez de 2560x1600).
Esto causa que el texto en TTY se salga de pantalla.

*Workaround:* =stty cols 160 rows 50= en fish login (automatico).

*** console-theme: Cambio de temas en caliente

Comando para cambiar colores de la consola TTY en caliente:

#+BEGIN_SRC bash
console-theme dark            # Spacemacs Dark (default)
console-theme light           # Spacemacs Light
console-theme commodore       # Fosforo verde CRT retro
console-theme mix             # Synthwave neon
console-theme amber           # CRT ambar retro
console-theme solarized-dark  # Solarized Dark
console-theme solarized-light # Solarized Light
console-theme list            # Ver todos
#+END_SRC

*Nota:* Solo funciona en TTY real (Ctrl+Alt+F1-F6), no en terminales graficos.
Para Alacritty usar =alacritty-theme=.

*** Fuentes de consola disponibles

Fuente actual: *Terminus* (ter-v32n) - LA clasica para programar.

*Terminus* (recomendada):
| Nombre | Tamano | Uso |
|--------+--------+-----|
| ter-v16n | 8x16 | Muchas lineas, puede ser pequena |
| ter-v24n | 12x24 | Balance |
| ter-v32n | 16x32 | HiDPI (actual) |
| ter-v32b | 16x32 | Bold |

*Spleen* (retro):
| Nombre | Tamano | Uso |
|--------+--------+-----|
| spleen-8x16 | 8x16 | Pequena |
| spleen-16x32 | 16x32 | HiDPI |
| spleen-32x64 | 32x64 | ENORME |

Cambiar fuente en =hardware/apple/macbook-pro-13-2.nix=:
#+BEGIN_SRC nix
console.font = lib.mkForce "ter-v32n";
console.packages = lib.mkForce [ pkgs.terminus_font ];
#+END_SRC

** Display Managers: La saga GDM -> LightDM -> greetd -> SDDM (RESUELTO 2026-01-24)

*Estado:* COMPLETO - X11 (XMonad) Y Wayland (Hyprland, niri) funcionan

*** Historia del problema

*Era 1: GDM (2026-01-19)*
Intentamos usar GDM + GNOME + XMonad como stack unificado.
*Ambas maquinas (aurin y macbook) dejaron de arrancar graficamente.*

GDM 49+ tiene problemas con:
1. *NVIDIA*: GDM en modo X11 busca =gnome-session-x11@gnome-login.target= que no existe
2. *XMonad puro*: GDM espera una sesion GNOME completa, no solo un WM

*Era 2: LightDM (2026-01-19)*
Cambiamos a LightDM. XMonad funcionaba, PERO:
- LightDM solo lee =xsessions/= (X11)
- Ignora completamente =wayland-sessions/=
- Hyprland, niri, GNOME Wayland NO aparecian

*Era 3: greetd + tuigreet (2026-01-24) - FALLIDO*
Intentamos greetd con tuigreet (TUI greeter), pero:
- X11 roto: Xorg sin suid necesita logind para permisos de input
- Sesiones Wayland funcionaban pero X11 (XMonad) no arrancaba
- Demasiado complejo configurar correctamente

*Era 4: SDDM (2026-01-24)*
Solucion definitiva: SDDM con tema astronaut.

*** Solucion actual: SDDM

Configurado en =modules/base/sddm.nix=:

#+BEGIN_SRC nix
services.displayManager.sddm = {
  enable = true;
  wayland.enable = false;  # GREETER en X11 (kwin_wayland crashea con NVIDIA)
  theme = "sddm-astronaut-theme";
  extraPackages = [ pkgs.sddm-astronaut ];
};
#+END_SRC

*IMPORTANTE*: =wayland.enable= controla el GREETER (pantalla de login), NO las sesiones.
El greeter usa kwin_wayland que crashea con NVIDIA. Con =false= usa X11 para el greeter
pero puedes seguir eligiendo sesiones Wayland (Hyprland, niri) sin problema.

*** Por que SDDM

| Caracteristica | GDM | LightDM | greetd | SDDM |
|----------------+-----+---------+--------+------|
| X11 sessions | Si (roto NVIDIA) | Si | Roto (suid) | Si |
| Wayland sessions | Si | NO | Si | Si |
| Dependencias | Pesadas (GNOME) | Medias | Minimas | Medias (Qt) |
| NVIDIA compatible | Problematico | Si | Parcial | Si |
| Configuracion | Compleja | Media | Compleja | Simple |

*** NVIDIA + Wayland: Que funciona y que NO (2026-01-25)

| Compositor/DE   | X11 | Wayland | Notas                                    |
|-----------------+-----+---------+------------------------------------------|
| XMonad          | OK  | N/A     | Solo X11                                 |
| Plasma (KDE)    | OK  | NO      | kwin_wayland crashea con NVIDIA          |
| GNOME           | OK  | NO      | Mutter tiene problemas con drivers open  |
| Hyprland        | N/A | OK      | Diseñado con soporte NVIDIA desde inicio |
| niri            | N/A | OK      | Diseñado con soporte NVIDIA desde inicio |

*SOLUCION PRACTICA*:
- Plasma/GNOME → usar sesiones X11 (funcionan perfecto)
- Wayland → usar Hyprland o niri (funcionan perfecto)

Los problemas de kwin y Mutter con NVIDIA son bugs upstream, no de NixOS.
El soporte NVIDIA en Wayland sigue mejorando - esto puede cambiar en el futuro.

*** Estado actual de las maquinas

| Maquina | DM | Sesiones probadas | Estado |
|---------+----+-------------------+--------|
| aurin   | SDDM | XMonad OK, Plasma X11 OK, Hyprland OK | Probado 2026-01-25 |
| macbook | SDDM | XMonad, GNOME, KDE, Hyprland, niri | OK (Intel, sin NVIDIA) |
| vespino | SDDM | XMonad, Hyprland | Pendiente test |

*** Variables de entorno NVIDIA

Las variables para Wayland van en =hardware/nvidia/*.nix=:

#+BEGIN_SRC nix
environment.sessionVariables = {
  LIBVA_DRIVER_NAME = "nvidia";
  __GLX_VENDOR_LIBRARY_NAME = "nvidia";
  # Para wlroots (Hyprland, etc):
  WLR_NO_HARDWARE_CURSORS = "1";  # Si hay problemas con el cursor
};
#+END_SRC

Estas variables se aplican DESPUES del login (sesion), no en SDDM (pre-login)

*** Picom: por que no esta en desktop.nix

Picom NO se habilita en =modules/base/desktop.nix= porque conflictua con Mutter (GNOME).

*Solucion implementada:*
1. Picom se configura en home-manager (=modules/home-manager/programs/picom.nix=)
2. =xmonad.hs= lo lanza en =startupHook: spawnOnce "picom"=
3. Cuando usas GNOME, picom NO corre (Mutter hace de compositor)
4. Cuando usas XMonad, picom SI corre

*** Comandos utiles para debugging

#+BEGIN_SRC bash
# Ver que Display Manager esta activo
systemctl status display-manager

# Ver sesiones disponibles
ls /usr/share/xsessions/
ls /usr/share/wayland-sessions/

# Ver variables de sesion
echo $XDG_SESSION_TYPE
echo $XDG_CURRENT_DESKTOP

# Ver si picom esta corriendo
pgrep -a picom

# Ver compositor activo
echo $WAYLAND_DISPLAY  # Si tiene valor, estas en Wayland
#+END_SRC

* CPU Frequency Scaling: Por que tu CPU no siempre va a tope

Imagina que tu coche tuviera solo dos opciones: 0 km/h o 200 km/h. Seria un
desastre: consumiria gasolina como loco y el motor se fundiria. Por eso los
coches tienen marchas. Tu CPU funciona igual.

** El problema: rendimiento vs consumo vs temperatura

Tu procesador puede funcionar a diferentes frecuencias:
- *Frecuencia alta* = mas rendimiento, pero mas calor y consumo
- *Frecuencia baja* = menos rendimiento, pero silencioso y eficiente

La pregunta es: *quien decide cuando subir o bajar?*

En Linux, esto se llama *CPU Frequency Scaling*, y hay dos partes:
1. *Driver*: el que sabe COMO cambiar la frecuencia (habla con el hardware)
2. *Governor*: el que decide CUANDO cambiarla (la politica)

** Los drivers: cada CPU habla su idioma

| Driver        | Para que CPUs                      | Notas                                   |
|---------------+------------------------------------+-----------------------------------------|
| intel_pstate  | Intel modernos (2013+)             | El kernel lo carga solo, muy eficiente  |
| amd-pstate    | AMD Ryzen (Zen 2+)                 | Igual que intel_pstate pero para AMD    |
| acpi-cpufreq  | CPUs antiguos o servidores         | Generico, funciona casi siempre         |

*El problema:* No todos los CPUs cargan su driver automaticamente.

- Los Intel de escritorio/laptop modernos: =intel_pstate= se carga solo
- Los AMD Ryzen modernos: =amd-pstate= se carga solo
- Los Xeon servidor y AMD FX antiguos: necesitan =acpi-cpufreq= manualmente

Por eso en este repo, =modules/common/boot.nix= carga =acpi-cpufreq= para todos.
Si tu CPU ya usa otro driver, simplemente se ignora.

** Los governors: quien manda aqui?

| Governor     | Que hace                                        | Cuando usarlo               |
|--------------+-------------------------------------------------+-----------------------------|
| performance  | Siempre al maximo                               | Compilando, gaming, benchs  |
| powersave    | Siempre al minimo                               | Bateria, silencio total     |
| schedutil    | El kernel decide segun carga (inteligente)      | Uso normal (recomendado)    |
| ondemand     | Sube rapido con carga, baja lento               | Alternativa a schedutil     |
| conservative | Sube/baja gradualmente                          | Servidores, estabilidad     |

*Ojo:* =intel_pstate= solo permite =performance= y =powersave=. El resto de
governors solo funcionan con =acpi-cpufreq=.

** Como esta configurado en cada maquina

*** Aurin - La bestia de 72 hilos

Aurin tiene dos Xeon E5-2699v3, que son CPUs de *servidor*. Curiosidad: aunque
son Intel, el kernel NO carga =intel_pstate= automaticamente para Xeons.
Por que? Porque en servidores se prefiere el control fino de =acpi-cpufreq=.

#+BEGIN_SRC bash
# En aurin veras esto:
$ cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_driver
acpi-cpufreq

$ cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_available_governors
conservative ondemand userspace powersave performance schedutil
#+END_SRC

Con 72 hilos y =schedutil=, el sistema sube/baja la frecuencia de cada core
independientemente. Muy eficiente para workloads variados.

*** Vespino - El veterano AMD FX

Vespino tiene un AMD FX-8350, un procesador de 2012 (arquitectura Bulldozer).
Estos CPUs son anteriores a =amd-pstate=, asi que usan =acpi-cpufreq=.

Antes de anadir el modulo en =boot.nix=, vespino no tenia *ningun* driver de
frecuencia cargado. El resultado? El script de xmobar mostraba "?.?" porque
no encontraba =/sys/devices/system/cpu/cpu0/cpufreq/=.

Ahora con =acpi-cpufreq= cargado, tiene todos los governors disponibles.

*** MacBook - El unico con intel_pstate

El MacBook Pro tiene un Intel Skylake, y este SI carga =intel_pstate= solo.
La diferencia es que solo tiene dos governors: =performance= y =powersave=.

#+BEGIN_SRC bash
# En macbook veras esto:
$ cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_driver
intel_pstate

$ cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_available_governors
performance powersave
# Solo dos! No hay schedutil ni ondemand
#+END_SRC

Por defecto usa =powersave= (es un laptop, prioriza bateria).

** La configuracion en NixOS

Todo esto se configura en dos archivos:

*** 1. Cargar el driver (modules/common/boot.nix)

#+BEGIN_SRC nix
# Una linea que soluciona el problema para CPUs antiguos
boot.kernelModules = [ "acpi-cpufreq" ];
#+END_SRC

Si tu CPU ya usa =intel_pstate= o =amd-pstate=, esta linea no hace nada
(los drivers nativos tienen prioridad). Solo ayuda a los que lo necesitan.

*** 2. GUI para cambiar governor (modules/common/cpupower-gui.nix)

=cpupower-gui= es una app grafica para cambiar el governor sin terminal.
El modulo configura:
- El paquete
- Polkit para no pedir password (si estas en grupo wheel)
- El servicio D-Bus necesario

** Uso diario: el monitor en xmobar

En xmobar veras algo como: =2.3GHz auto=

| Color  | Governor        | Significado                    |
|--------+-----------------+--------------------------------|
| Rojo   | performance     | Al maximo, cuidado con el calor |
| Verde  | powersave       | Ahorrando energia               |
| Cyan   | schedutil/auto  | El kernel decide (normal)       |
| Gris   | none            | Sin cpufreq (fallback a /proc)  |

*Click en el monitor* abre =cpupower-gui= para cambiar el governor.

** Comandos utiles

#+BEGIN_SRC bash
# Ver que tienes
cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_driver
cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_governor

# Cambiar governor (temporal, hasta reboot)
sudo cpupower frequency-set -g performance   # Al maximo
sudo cpupower frequency-set -g powersave     # Al minimo
sudo cpupower frequency-set -g schedutil     # Automatico

# O simplemente abre la GUI
cpupower-gui
#+END_SRC

** Bonus: ver consumo en watts (RAPL)

Solo CPUs Intel pueden reportar consumo real en watts via RAPL.

#+BEGIN_SRC nix
# En configuration.nix - dar acceso a RAPL sin root
services.udev.extraRules = ''
  SUBSYSTEM=="powercap", ACTION=="add", RUN+="${pkgs.coreutils}/bin/chmod g+r /sys/class/powercap/intel-rapl/intel-rapl:*/energy_uj"
  SUBSYSTEM=="powercap", ACTION=="add", RUN+="${pkgs.coreutils}/bin/chgrp wheel /sys/class/powercap/intel-rapl/intel-rapl:*/energy_uj"
'';
#+END_SRC

Solo disponible en CPUs Intel (Aurin y MacBook). El script =xmobar-cpu.sh= muestra
watts si RAPL esta disponible.

* Doom Emacs: Tree-Sitter y Syntax Highlighting en NixOS

** Que es Tree-Sitter

Tree-sitter es un sistema de parseo incremental que genera arboles de sintaxis abstracta (AST)
para codigo fuente. A diferencia del syntax highlighting tradicional basado en expresiones
regulares, tree-sitter:

- *Parsea el codigo completo* como lo haria un compilador
- *Es incremental*: solo reparsea las partes que cambian
- *Entiende la estructura* del codigo, no solo patrones de texto
- *Es mas rapido* para archivos grandes
- *Proporciona highlighting mas preciso* (distingue variables locales de globales, etc.)

** Tree-sitter en Emacs

Emacs 29+ incluye soporte nativo para tree-sitter (=treesit=), pero necesita *grammars compilados*
para cada lenguaje. Estos grammars son bibliotecas dinamicas (.so en Linux, .dylib en macOS).

*** El Problema en NixOS

Cuando Doom Emacs intenta usar tree-sitter, necesita compilar los grammars si no existen.
Esto falla o se congela en NixOS porque:

1. El compilador de C (gcc/clang) puede no estar en el PATH de Emacs
2. Las rutas de bibliotecas del sistema son diferentes en NixOS
3. El sandbox de Nix complica la compilacion dinamica

** Opciones Disponibles

*** Opcion 1: Deshabilitar Tree-sitter (Recomendado)

Si el syntax highlighting tradicional es suficiente, simplemente quita los flags =+tree-sitter=.

*En =~/.config/doom/init.el=:*

#+BEGIN_SRC emacs-lisp
;; ANTES (con tree-sitter)
:tools
tree-sitter

:lang
(nix +lsp +tree-sitter)
(php +lsp +tree-sitter)

;; DESPUES (sin tree-sitter)
:tools
;;tree-sitter  ; Comentado o eliminado

:lang
(nix +lsp)
(php +lsp)
#+END_SRC

*Aplicar cambios:*
#+BEGIN_SRC bash
~/.config/emacs/bin/doom sync
# Reiniciar Emacs
#+END_SRC

*Pros:* Sin configuracion adicional, funciona inmediatamente, LSP ya proporciona el 90% de los beneficios.

*** Opcion 2: Instalar Grammars via NixOS

NixOS puede preinstalar los grammars compilados. En =modules/home-manager/passh.nix=:

#+BEGIN_SRC nix
home.packages = with pkgs; [
  tree-sitter-grammars.tree-sitter-nix
  tree-sitter-grammars.tree-sitter-javascript
  tree-sitter-grammars.tree-sitter-php
  tree-sitter-grammars.tree-sitter-json
  tree-sitter-grammars.tree-sitter-html
  tree-sitter-grammars.tree-sitter-bash
];
#+END_SRC

Y en =~/.config/doom/config.el=:

#+BEGIN_SRC emacs-lisp
(when (and (fboundp 'treesit-available-p) (treesit-available-p))
  (setq treesit-extra-load-path
        (list (expand-file-name "~/.nix-profile/lib/tree-sitter")
              "/run/current-system/sw/lib/tree-sitter")))
#+END_SRC

** Verificar Estado

#+BEGIN_SRC emacs-lisp
;; M-x eval-expression RET
(treesit-available-p)              ; Deberia retornar t
(treesit-language-available-p 'nix) ; Ver si grammar de nix esta
#+END_SRC

*Modo activo:* Sin tree-sitter usa =nix-mode=, con tree-sitter usa =nix-ts-mode= (sufijo =-ts-=).

** Grammars Disponibles

#+BEGIN_SRC bash
nix search nixpkgs tree-sitter-grammars
#+END_SRC

| Lenguaje   | Paquete Nix                                 |
|------------+---------------------------------------------|
| Nix        | tree-sitter-grammars.tree-sitter-nix        |
| JavaScript | tree-sitter-grammars.tree-sitter-javascript |
| PHP        | tree-sitter-grammars.tree-sitter-php        |
| JSON       | tree-sitter-grammars.tree-sitter-json       |
| HTML       | tree-sitter-grammars.tree-sitter-html       |
| Bash       | tree-sitter-grammars.tree-sitter-bash       |

* Hosts Privados de Trabajo

** El Problema

Para desarrollo necesitamos hosts internos (staging, dev, etc.) que no pueden
estar en un repo publico. El archivo esta en un repo de trabajo externo.

** Por que requiere --impure

En NixOS Flakes, =builtins.pathExists= para paths externos al flake SIEMPRE
devuelve =false= en evaluacion pura. Por eso usamos =--impure=.

** Solucion Actual

#+BEGIN_SRC bash
# Aurin (con hosts de trabajo)
sudo nixos-rebuild switch --flake ~/dotfiles#aurin --impure

# Macbook (requiere clonar repo de trabajo primero)
git clone <work-repo> ~/src/work/work-env
sudo nixos-rebuild switch --flake ~/dotfiles#macbook --impure
#+END_SRC

** Opciones Futuras (cuando haga falta)

| Opcion | Puro? | Complejidad | Notas |
|--------|-------|-------------|-------|
| =--impure= (actual) | No | Baja | Simple, funciona |
| sops-nix | Si | Media | Encripta secretos en repo |
| Repo privado input | Si | Media | Flake input con SSH |

Para implementar sops-nix: [[https://github.com/Mic92/sops-nix][sops-nix GitHub]]

* Roadmap: Migracion Stow -> Home Manager

** Contexto

Con la adopcion de NixOS Flakes para multiples maquinas (aurin, macbook), GNU Stow
se vuelve redundante. Home Manager integrado con el flake ofrece:

- *Configs per-machine declarativas* (ej: xmobar con diferentes DPI/fonts)
- *Un solo comando* =nixos-rebuild switch= aplica sistema + usuario
- *Reproducibilidad total* - Todo versionado en el flake
- *Rollback automatico* - Generaciones anteriores disponibles

** Estado Actual (Anti-pattern)

En =modules/home-manager/passh.nix= linea ~253 hay un hack que usa stow dentro de home-manager:

#+BEGIN_SRC nix
activation.linkDotfiles = lib.hm.dag.entryBefore [ "checkLinkTargets" ] ''
  ${pkgs.stow}/bin/stow -v -R -t ${config.home.homeDirectory} \
    alacritty composer fish picom xmobar xmonad claude-code
'';
#+END_SRC

Esto debe eliminarse gradualmente migrando cada config a home-manager nativo.

** Arquitectura Propuesta

#+BEGIN_SRC text
modules/home-manager/
|-- default.nix           # Entry point, recibe hostname
|-- common.nix            # Configs compartidas (paquetes base)
|-- programs/
|   |-- xmobar.nix        # Xmobar con opciones parametrizables
|   |-- xmonad.nix        # XMonad config
|   |-- alacritty.nix     # Terminal
|   `-- fish.nix          # Shell (usa programs.fish nativo)
`-- machines/
    |-- aurin.nix         # fontSize=14, dpi=96, nvidia, interfaces de red
    `-- macbook.nix       # fontSize=22, dpi=168, intel, HiDPI
#+END_SRC

** Pasar hostname al flake

En =flake.nix=, funcion =mkNixosConfig=:

#+BEGIN_SRC nix
home-manager.extraSpecialArgs = {
  inherit inputs;
  hostname = hostname;  # Anadir esto
};
#+END_SRC

** Patron para configs per-machine

#+BEGIN_SRC nix
# modules/home-manager/programs/xmobar.nix
{ config, lib, ... }:

let cfg = config.custom.xmobar;
in {
  options.custom.xmobar = {
    fontSize = lib.mkOption {
      type = lib.types.int;
      default = 14;
    };
    dpi = lib.mkOption {
      type = lib.types.int;
      default = 96;
    };
  };

  config.home.file.".config/xmobar/xmobarrc".text = ''
    Config { font = "xft:Monoid Nerd Font:size=${toString cfg.fontSize}"
           , ...
    }
  '';
}
#+END_SRC

Luego en cada maquina:

#+BEGIN_SRC nix
# machines/macbook.nix
{ ... }: {
  custom.xmobar.fontSize = 22;
  custom.xmobar.dpi = 168;
}

# machines/aurin.nix
{ ... }: {
  custom.xmobar.fontSize = 14;
  # dpi usa default 96
}
#+END_SRC

** Orden de Migracion Recomendado

1. [X] *xmobar* - COMPLETADO (2026-01-04)
   - Modulo parametrizable en =modules/home-manager/programs/xmobar.nix=
   - Configs per-machine en =modules/home-manager/machines/{aurin,macbook}.nix=
   - Opciones: fontSize, gpuType, networkInterface, wifiInterface, showBattery, showTrayer, alsaMixer

2. [X] *alacritty* - COMPLETADO (2026-01-04)
   - Modulo en =modules/home-manager/programs/alacritty.nix=
   - Usa =programs.alacritty= nativo de home-manager
   - Opciones: fontSize (14 aurin, 18 macbook), theme (dark/light)
   - Tema Spacemacs inline

3. [X] *picom* - COMPLETADO (2026-01-04)
   - Modulo en =modules/home-manager/programs/picom.nix=
   - Opcion: backend (egl para NVIDIA, xrender para Intel)
   - Config completa inline (shadows, blur, animations)

4. [X] *fish* - COMPLETADO (2026-01-04)
   - Modulo en =modules/home-manager/programs/fish.nix=
   - Usa =programs.fish= nativo de home-manager
   - Aliases: git shortcuts, eza/ls, nixos helpers
   - Abbreviations: docker, nix

5. [ ] *xmonad* - Pendiente (config Haskell compleja)
   - Actualmente gestionado via stow
   - Requiere analisis de como parametrizar Haskell

6. [ ] *composer* - Pendiente (bajo riesgo, simple)
   - PHP tool, config estatico
   - Baja prioridad

7. [X] Reducir bloque =activation.linkDotfiles= - PARCIALMENTE
   - Solo quedan: xmonad, composer, claude-code
   - Documentado estado en passh.nix lineas 255-276

8. [ ] Eliminar carpetas stow vacias (cuando todo migre)

** Referencias

- [[https://nix-community.github.io/home-manager/][Home Manager Manual]]
- [[https://nixos.wiki/wiki/Flakes][NixOS Flakes Wiki]]

* Estructura Recomendada para Nuevas Configs

Al anadir nueva configuracion:

#+BEGIN_SRC text
dotfiles/
`-- nueva-app/
    `-- .config/
        `-- nueva-app/
            `-- config.conf
#+END_SRC

Luego:
#+BEGIN_SRC bash
stow nueva-app
#+END_SRC

Esto crea: =~/.config/nueva-app/config.conf= -> =~/dotfiles/nueva-app/.config/nueva-app/config.conf=

* Gestión Segura de Secretos con SOPS-NIX 🔐

** ¿Qué es SOPS?

SOPS (Secrets OPerationS) es una herramienta de Mozilla para encriptar archivos de configuración.
- Encripta *solo los valores*, no las claves (puedes ver qué secretos tienes, pero no su contenido)
- Usa age o GPG para encriptación
- Integración nativa con NixOS via =sops-nix=

** El Problema Actual

Actualmente tenemos passwords hardcodeadas en texto plano en archivos que van a GitHub:

#+BEGIN_EXAMPLE
┌─────────────────────────────────────────┐
│ hosts/aurin/default.nix (PÚBLICO)       │
├─────────────────────────────────────────┤
│ services.miniflux = {                   │
│   adminCredentialsFile = {              │
│     password = "MiPassword123";  ← 🔥 VISIBLE│
│   };                                    │
│ };                                      │
└─────────────────────────────────────────┘
                  │
                  ├─── git push
                  │
                  ▼
          ┌──────────────┐
          │   GitHub     │  ← TODO EL MUNDO PUEDE VER
          │  (PÚBLICO)   │     tu password
          └──────────────┘
#+END_EXAMPLE

*Secretos actualmente expuestos en este repo:*
- Passwords de servicios (Miniflux, Syncthing, VM Ubuntu)
- Email corporativo
- Paths a archivos externos (hosts Vocento)
- IPs y redes internas

** La Solución: SOPS-NIX

#+BEGIN_EXAMPLE
┌──────────────────────────────────────────────────┐
│ secrets.yaml (ENCRIPTADO con tu clave age/GPG)   │
├──────────────────────────────────────────────────┤
│ miniflux_password: ENC[AES256_GCM,data:Qw==,...] │ ← Encriptado
│ router_password: ENC[AES256_GCM,data:Ab==,...]   │
│ syncthing_key: ENC[AES256_GCM,data:Xy==,...]     │
└──────────────────────────────────────────────────┘
                  │
                  ├─── git push (SEGURO!)
                  │
                  ▼
          ┌──────────────┐
          │   GitHub     │  ← Solo ve texto encriptado
          │  (PÚBLICO)   │     Nadie puede leerlo
          └──────────────┘

EN TU MÁQUINA (cuando haces nixos-rebuild):
┌─────────────────────────────────────────┐
│ 1. sops-nix detecta secrets.yaml        │
│ 2. Desencripta con TU clave privada     │
│ 3. Crea archivos en /run/secrets/       │
│    (solo root puede leer)               │
│ 4. NixOS usa esos archivos              │
└─────────────────────────────────────────┘
#+END_EXAMPLE

** ¿Cómo Funciona? (Paso a Paso)

*** 1. Generar Clave Age

#+BEGIN_SRC bash
# Instalar age
nix-shell -p age

# Generar clave
mkdir -p ~/.config/sops/age
age-keygen -o ~/.config/sops/age/keys.txt

# Output:
# Public key: age1ql3z7hjy54pw3hyww5ayyfg7zqgvc7w3j2elw8zmrj2kg5sfn9aqmcac8p
#             └─ Guarda esta clave pública
#+END_SRC

*** 2. Crear Archivo de Secretos (Sin Encriptar Todavía)

#+BEGIN_SRC yaml
# secrets.yaml (TEMPORAL - lo encriptarás en el paso 4)
miniflux_password: SuperSecretPass123
router_password: OtraPasswordSegura456
syncthing_api_key: abc123def456ghi789
vm_ubuntu_password: MiPasswordVM2024
#+END_SRC

*** 3. Configurar .sops.yaml

#+BEGIN_SRC yaml
# .sops.yaml - Define qué clave usar
keys:
  - &passh age1ql3z7hjy54pw3hyww5ayyfg7zqgvc7w3j2elw8zmrj2kg5sfn9aqmcac8p
    # └─ TU clave pública del paso 1
creation_rules:
  - path_regex: secrets\.yaml$
    key_groups:
      - age:
          - *passh
#+END_SRC

*** 4. Encriptar el Archivo

#+BEGIN_SRC bash
nix-shell -p sops
sops secrets.yaml  # Se abre editor, guardas, y queda encriptado

# Ahora secrets.yaml tiene:
# miniflux_password: ENC[AES256_GCM,data:Qw==,iv:...,tag:...,type:str]
#+END_SRC

*** 5. Usar los Secretos en NixOS

#+BEGIN_SRC nix
# flake.nix - Añadir input
{
  inputs = {
    nixpkgs.url = "github:NixOS/nixpkgs/nixos-unstable";
    sops-nix.url = "github:Mic92/sops-nix";
  };
  
  outputs = { nixpkgs, sops-nix, ... }: {
    nixosConfigurations.aurin = nixpkgs.lib.nixosSystem {
      modules = [
        sops-nix.nixosModules.sops
        ./hosts/aurin
      ];
    };
  };
}

# hosts/aurin/default.nix
{
  sops = {
    defaultSopsFile = ../../secrets.yaml;
    age.keyFile = "/home/passh/.config/sops/age/keys.txt";
    
    secrets = {
      miniflux_password = {};  # Crea /run/secrets/miniflux_password
      router_password = {};
    };
  };
  
  # Usar el secreto
  services.miniflux = {
    adminCredentialsFile = {
      passwordFile = config.sops.secrets.miniflux_password.path;
      # └─ Apunta a /run/secrets/miniflux_password (solo root)
    };
  };
}
#+END_SRC

** Diagrama Completo del Flujo

#+BEGIN_EXAMPLE
TU MÁQUINA                        GITHUB (PÚBLICO)
===========                       ================

1. EDITAR SECRETOS
   ┌──────────────┐
   │ sops secrets │               
   │   .yaml      │──┐            
   └──────────────┘  │ Desencripta con tu clave
                     │ Abre editor
                     │ Editas password123 → nueva_pass
                     │ Guardas → Re-encripta
                     ▼
   ┌──────────────────────────────┐
   │ secrets.yaml (ENCRIPTADO)     │
   │ password: ENC[AES256...]      │
   └──────────────────────────────┘
                     │
2. GIT PUSH          │
   git add secrets.yaml
   git push          │
                     ▼
                ┌─────────────────┐
                │  GitHub Repo    │
                │  (PÚBLICO)      │  ← Solo cifrado
                └─────────────────┘
                     │
3. REBUILD EN        │ git pull
   OTRA MÁQUINA      │
                     ▼
   ┌──────────────────────────────┐
   │ nixos-rebuild switch         │
   │   ↓                          │
   │ sops-nix desencripta         │
   │   ↓                          │
   │ Crea /run/secrets/password   │
   └──────────────────────────────┘
#+END_EXAMPLE

** Ventajas vs Problemas

| Aspecto | SIN sops-nix (AHORA) | CON sops-nix |
|---------+----------------------+--------------|
| Passwords en repo | ❌ Texto plano visible | ✅ Encriptado |
| Git push seguro | ❌ Expones secretos | ✅ Solo cifrado sube |
| Multi-máquina | ❌ Cambiar en 3 sitios | ✅ 1 archivo, sync auto |
| Rotación passwords | ❌ Editar 10 archivos | ✅ =sops secrets.yaml= |
| Complejidad inicial | ✅ Cero setup | ⚠️ 30 min setup |
| Mantenimiento | ⚠️ Buscar/reemplazar | ✅ Centralizado |

** ¿Es Difícil?

*NO.* El setup inicial es simple:

| Paso | Tiempo | Dificultad |
|------+--------+------------|
| 1. Generar clave age | 10 segundos | ⭐ Trivial |
| 2. Crear =.sops.yaml= | 2 minutos | ⭐ Copy/paste |
| 3. Crear =secrets.yaml= | 5 minutos | ⭐⭐ Listar secretos |
| 4. Añadir sops-nix a flake | 5 minutos | ⭐⭐ 3 líneas código |
| 5. Migrar passwords | 15 minutos | ⭐⭐⭐ Buscar/reemplazar |
| *TOTAL* | *~30 minutos* | *Una sola vez* |

** Plan de Implementación Propuesto

*** FASE 1: Setup Inicial (Lectura, SIN Cambios)
- [ ] Leer documentación oficial sops-nix
- [ ] Identificar TODOS los secretos en el repo
- [ ] Diseñar estructura de =secrets.yaml=
- [ ] Decidir: age vs GPG (recomendado: age)

*** FASE 2: Implementación
- [ ] Generar clave age
- [ ] Crear =.sops.yaml= y =secrets.yaml=
- [ ] Añadir sops-nix al flake.nix
- [ ] Migrar passwords uno por uno:
  - [ ] Miniflux (aurin, vespino)
  - [ ] Syncthing
  - [ ] VM Ubuntu
- [ ] Eliminar passwords hardcoded del código
- [ ] Test en aurin: =sudo nixos-rebuild test --flake .#aurin --impure=
- [ ] Commit & push (secrets.yaml encriptado)

*** FASE 3: Limpieza (Opcional)
- [ ] Git history rewrite (borrar passwords del historial antiguo)
- [ ] Documentar en README cómo añadir secretos nuevos
- [ ] Aplicar en macbook y vespino

** Secretos Actualmente en el Repo

Lista de secretos que necesitan migrarse (VALORES SANITIZADOS - no reales):

| Secreto | Ubicación | Acción |
|---------+-----------+--------|
| VM Ubuntu password | README.org | Migrar a sops |
| Miniflux password (aurin) | hosts/aurin/default.nix | Migrar a sops |
| Miniflux password (vespino) | hosts/vespino/default.nix | Migrar a sops |
| Syncthing password | aurin-system-notes.org | Migrar a sops |
| Router password | CLAUDE.md | ⚠️ NO en NixOS - cambiar manualmente |
| Email corporativo | modules/home-manager/core.nix | ⚠️ Considerar si es secreto |
| Vocento hosts file (path) | hosts/*/default.nix | Evaluar si necesita sops |

#+BEGIN_QUOTE
*NOTA IMPORTANTE:* El router NO se gestiona con NixOS, así que su password debe cambiarse manualmente en la interfaz web del router.

*ADVERTENCIA:* Los valores mostrados en esta documentación son EJEMPLOS. Los valores reales están hardcodeados en varios archivos del repo y deben migrarse a sops-nix para mayor seguridad.
#+END_QUOTE

** Referencias

- [[https://github.com/Mic92/sops-nix][sops-nix - GitHub]]
- [[https://github.com/mozilla/sops][SOPS - Mozilla]]
- [[https://github.com/FiloSottile/age][age - Encryption Tool]]
- [[https://nixos.wiki/wiki/Sops-nix][NixOS Wiki - sops-nix]]

* TODO Tareas Pendientes

#+BEGIN_QUOTE
*Leyenda:*
- [X] = Completado
- [ ] = Pendiente (activo o futuro)
- *MAYÚSCULAS* = Título de tarea principal
- Fechas [YYYY-MM-DD] = Cuándo se completó
#+END_QUOTE

** Resumen de Estado

| Categoría | Completadas | Pendientes | Estado |
|-----------+-------------+------------+--------|
| AI Agents | 4/5 (80%) | 1 | ✅ Funcionando (aurin + macbook) |
| XMonad Migration | 3/3 (100%) | 0 | ✅ Completado (aurin + macbook) |
| XMobar/XMonad Portability | 0/1 (0%) | 1 | 📖 Documentado (no prioritario) |
| Repo Template | 2/3 (67%) | 1 | ⚠️ En progreso |
| Seguridad (SOPS) | 1/2 (50%) | 1 | 📖 Documentado |
| Infraestructura | 0/3 (0%) | 3 | 🔮 Futuro |
| Limpieza | 1/3 (33%) | 2 | ⚠️ En progreso |

#+BEGIN_QUOTE
*Nota:* vespino está esperando =git pull && nixos-rebuild= para aplicar todos los cambios recientes (AI agents modular, XMonad migration, SOPS docs). ¡Va a flipar! 😄
#+END_QUOTE

** AI Agents (claude, opencode, crush)
- [X] *REFACTOR MODULAR* [2026-01-24] - Arquitectura modular para AI agents
  - [X] Crear =modules/home-manager/programs/ai-agents/= directory
  - [X] Separar en archivos: =default.nix=, =claude-code.nix=, =opencode.nix=, =crush.nix=
  - [X] Cada agente en módulo independiente (~80-90 líneas)
  - [X] Escalable: añadir nuevo agente = crear archivo + 1 import
- [X] *CLAUDE-CODE INTEGRADO* [2026-01-22] - Migrado de stow a home-manager
  - [X] Crear módulo =claude-code.nix= (config symlinks)
  - [X] Integrar en =core.nix= (portable)
  - [X] Eliminar claude-code de stow en =passh.nix=
  - [X] Aplicar en aurin (TESTED: =claude --version= v2.1.12)
  - [X] Aplicar en macbook (TESTED: =claude --version= v2.1.12)
  - [ ] Aplicar en vespino (el abandonado - esperando =git pull=)
- [X] *OPENCODE INTEGRADO* [2026-01-24] - No experimental, funcional
  - [X] Crear módulo =opencode.nix= (package + config symlinks)
  - [X] Config en =~/dotfiles/opencode/.config/opencode/=
  - [X] Aplicar en aurin (TESTED & WORKING - usado para esta sesión!)
  - [X] Aplicar en macbook (TESTED & WORKING)
  - [ ] Aplicar en vespino (esperando =git pull=)
- [X] *CRUSH AÑADIDO* [2026-01-24] - Desde Charmbracelet NUR
  - [X] Crear módulo =crush.nix= con patrón =fetchGit + callPackage=
  - [X] Instalar desde NUR (no nixpkgs) para version reciente
  - [X] Config remota: Mac usa GPU desktop via =campo.zapto.org:11434=
  - [X] Aplicar en aurin (TESTED: =crush --version= v0.35.0)
  - [X] Aplicar en macbook (TESTED: usa desktop Ollama remotamente)
  - [ ] Aplicar en vespino
- [ ] *SHARED AGENTS* - Config multi-cliente (futuro)
  - [ ] Investigar compatibilidad entre clientes
  - [ ] Considerar shared agents directory si claude/opencode lo soportan

** Infraestructura
- [ ] *VPN VOCENTO EXPERIMENTAL* - Probar modulos en vespino antes de aurin
  - Copiar disco QCOW2 a vespino
  - Configurar =services.vocento-vpn-bridge= con interface correcta
  - Validar que funciona igual que en macbook
- [X] *WAYLAND RESUELTO* - SDDM soporta X11 y Wayland (2026-01-24)
  - Probado en macbook con Hyprland
  - Pendiente: probar en aurin y vespino (NVIDIA)
- [ ] *rEFInd* - Bootloader para fix geometria TTY en MacBook
  - =boot.loader.refind.enable = true=
  - Configurar =resolution 2560 1600= en refind.conf

** Repo como Template (Forkeable)
- [X] *MANIFIESTO HHKB* [2026-01-24] - Documentación filosófica
  - [X] Historia personal HHKB (extensión de manos → extensión de cerebro)
  - [X] Cita exacta Eiiti Wada (cowboys + saddle)
  - [X] Problemas universales (formatear, nuevo portátil, 3 máquinas)
  - [X] Énfasis: hardware totalmente diferente (Intel MacBook vs Desktop RTX)
  - [X] Legibilidad humana primero, LLM-friendly como bonus
- [X] *SECCIÓN BÁSICA AÑADIDA* [2026-01-22] - "Use This Repo (WIP)"
  - [X] Guía Quick Start para terceros
  - [X] Checklist de valores hardcoded a cambiar
  - [X] Tabla "What You Get" vs "What's Personal"
  - [X] Warning claro: WIP, requiere adaptación manual
  - [ ] Testing con usuario real (Daniel)
- [ ] *TEMPLATE COMPLETO* (después de stow migración completa)
  - [ ] Parametrizar username (mainUser variable)
  - [ ] Script generator: =./scripts/new-machine.sh=
  - [ ] Separación clara: personal vs. template
  - [ ] Secrets management (sops-nix o agenix)

** XMonad Migration (Home-Manager)
- [X] *PHASE 1: CLEANUP* [2026-01-23] - Limpieza de xmonad.hs
  - [X] Remover dead keybindings (TTS, keyboard layout)
  - [X] Fix hardcoded paths (picom, jetbrains-toolbox)
  - [X] Mover glmatrix-bg.sh a scripts/
- [X] *PHASE 2: HOME-MANAGER MODULE* [2026-01-23] - Migración a home-manager
  - [X] Crear =modules/home-manager/programs/xmonad.nix=
  - [X] Usar =.source= para hot-reload (10s compile)
  - [X] Absolute paths para scripts (reliability)
  - [X] Aplicar en aurin (TESTED & WORKING)
  - [X] Aplicar en macbook (TESTED & WORKING)
  - [ ] Aplicar en vespino (esperando =git pull=)
- [X] *PHASE 3: REJECTED* [2026-01-23] - Templates consideradas dañinas
  - [X] Intentado: =.text= template (365 lines in Nix)
  - [X] Resultado: Indentation bugs, xmobar broken
  - [X] Revertido: Commit cff214e
  - [X] Lección: =.source= > templates (pragmatismo > pureza)
  - [X] Documentado en =XMONAD-MIGRATION-SUMMARY.md=

** Seguridad (SOPS-NIX)
- [X] *DOCUMENTACIÓN SOPS-NIX* [2026-01-24] - Guía completa añadida
  - [X] Documentar qué es SOPS y por qué usarlo
  - [X] Diagramas de flujo (problema → solución)
  - [X] Guía paso a paso (5 pasos)
  - [X] Plan de implementación (3 fases)
  - [X] Auditoría de secretos actuales
- [ ] *IMPLEMENTACIÓN SOPS-NIX* (pendiente, cuando convenga)
  - [ ] FASE 1: Setup inicial (generar keys age, .sops.yaml)
  - [ ] FASE 2: Migrar passwords (Miniflux, Syncthing, VM Ubuntu)
  - [ ] FASE 3: Limpieza (git history, docs)

** XMobar + XMonad Portability (Fase 3 - Hardcoded Paths)
- [ ] *PORTABILITY REFACTOR* - Eliminar paths hardcoded /home/passh/
  - [ ] SCOPE: ~120 líneas en 4 archivos (xmobar.nix, xmonad.nix, xmobar-monitors.hs, xmobar-full.hs)
  - [ ] OBJETIVO: Funcionar con cualquier username (no solo "passh")
  - [ ] IMPACTO: Alto (mejora portabilidad, mejor práctica Nix)
  - [ ] ETA: ~50 minutos trabajo + 30 min testing

*** Problema Actual

Hardcoded paths en múltiples archivos:

#+BEGIN_SRC text
modules/home-manager/programs/xmobar.nix (líneas 80-112):
  Run Com "/home/passh/dotfiles/scripts/xmobar-vpn.sh" [] "vpn" 30
  ...24 scripts más con mismo pattern

xmobar/.config/xmobar/xmobar-monitors.hs (~24 líneas):
  Run Com "/home/passh/dotfiles/scripts/xmobar-bluetooth.sh" [] "bt" 30
  ...

xmonad/.config/xmonad/xmonad.hs (2 líneas):
  , ("M-t", spawn "/home/passh/dotfiles/scripts/trayer-toggle.sh")
  , ("M-S-m", spawn "/home/passh/dotfiles/scripts/glmatrix-bg.sh")
#+END_SRC

*Consecuencia:* Repo NO funciona si username != "passh"

*** Solución Propuesta

**** 1. xmobar.nix (Alto impacto, 70 líneas)

#+BEGIN_SRC nix
# ANTES:
Run Com "/home/passh/dotfiles/scripts/xmobar-vpn.sh" [] "vpn" 30

# DESPUÉS:
let
  scriptsDir = "${config.home.homeDirectory}/dotfiles/scripts";
in
  Run Com "${scriptsDir}/xmobar-vpn.sh" [] "vpn" 30
#+END_SRC

*Afecta:*
- xmobarrc (generado por xmobar.nix)
- 24 monitores × 3 configs = ~70 líneas

**** 2. xmonad.nix (Medio impacto, 5 líneas)

Convertir de =.source= (copy) a =.text= (templating):

#+BEGIN_SRC nix
# ANTES:
home.file.".config/xmonad/xmonad.hs".source = 
  ../../../xmonad/.config/xmonad/xmonad.hs;

# DESPUÉS:
home.file.".config/xmonad/xmonad.hs".text = 
  let scriptsDir = "${config.home.homeDirectory}/dotfiles/scripts";
  in builtins.replaceStrings 
    [ "/home/passh/dotfiles/scripts" ]
    [ scriptsDir ]
    (builtins.readFile ../../../xmonad/.config/xmonad/xmonad.hs);
#+END_SRC

*Afecta:* 2 scripts (trayer-toggle.sh, glmatrix-bg.sh)

**** 3. xmobar-monitors.hs (Medio impacto, 24 líneas)

*Opciones:*
- *Opción A (Recomendada):* Generar dinámicamente en xmobar.nix
  - Crear =xmobarMonitorsConfig= (como ya existe =xmobarConfig=)
  - Usar templating para paths
  - Más trabajo inicial, más mantenible
  
- *Opción B (Quick fix):* Usar =builtins.replaceStrings=
  - Similar al approach de xmonad.nix
  - Rápido pero menos elegante

**** 4. xmobar-full.hs (Bajo impacto, legacy)

- Legacy file (no usado actualmente)
- Aplicar mismo fix por consistencia
- O eliminar si ya no se necesita

*** Complejidad Estimada

| Archivo | Cambios | Dificultad | Tiempo | Impacto |
|---------+---------+------------+--------+---------|
| xmobar.nix | ~70 líneas | ⭐⭐ Media | 15 min | Alto |
| xmonad.nix | ~5 líneas | ⭐ Baja | 5 min | Medio |
| xmobar-monitors.hs | ~24 líneas | ⭐⭐⭐ Alta | 20 min | Medio |
| xmobar-full.hs | ~24 líneas | ⭐⭐ Media | 10 min | Bajo |
| *TOTAL* | *~120 líneas* | *⭐⭐ Media* | *50 min* | *Alto* |

*** Estrategia de Testing

#+BEGIN_SRC bash
# 1. Desarrollo en macbook (testbed)
sudo nixos-rebuild test --flake ~/dotfiles#macbook --impure
xmonad --recompile && xmonad --restart

# 2. Validar que NO quedan hardcoded paths
grep -r "/home/passh" ~/.config/xmonad/ ~/.config/xmobar/
# Debe estar vacío

# 3. Producción en aurin (después de test exitoso)
sudo nixos-rebuild switch --flake ~/dotfiles#aurin --impure
xmonad --recompile && xmonad --restart
#+END_SRC

*** Beneficios

1. *Portabilidad Total*
   - Dotfiles funcionarán con cualquier username
   - Facilitará compartir configs con otros usuarios NixOS

2. *Mejor Integración Nix*
   - Todo manejado por home-manager (no paths mágicos)
   - Fácil de debuggear (=nix repl= para ver variables)

3. *Preparación Multi-Usuario*
   - Si agregas otro usuario (ej: root, trabajo), funciona out-of-the-box

4. *Consistencia*
   - Mismo approach que ya usas en otros módulos (=config.home.homeDirectory=)

*** Decisiones Pendientes

Antes de implementar:

1. *¿Eliminar xmobar-full.hs?*
   - No se usa actualmente (solo xmobar-monitors.hs + xmobar-workspaces.hs)
   - Puede eliminarse o mantener como fallback

2. *¿Approach para xmobar-monitors.hs?*
   - Opción A: Generar dinámicamente (recomendada)
   - Opción B: =builtins.replaceStrings= (rápido)

3. *¿Testing en macbook primero?*
   - Recomendado: testear en macbook antes de aurin (PRODUCCIÓN)

*** Referencias

- TODO comments en archivos:
  - =modules/home-manager/programs/xmobar.nix= (líneas 30-35)
  - =modules/home-manager/programs/xmonad.nix= (líneas 19-25)
- Consultar nixos-guru: =~/dotfiles/claude-code/.claude/agents/nixos-guru.md=

*** Estado: DOCUMENTADO (no implementado)

Esta mejora está documentada pero NO es prioritaria. Implementar cuando:
- Se requiera portabilidad real (compartir repo con terceros)
- Preparación para template público (post-SOPS)
- O cuando haya tiempo libre para mejoras técnicas

*Nota:* XMobar funciona perfectamente en aurin y macbook con paths actuales.
Esta refactor es mejora técnica, no fix de bug.

** Whisper Voice Input - EXPERIMENTAL [2026-01-27]

*** Problema

Escribir prompts largos en chats (OpenCode, Claude, etc.) es lento y tedioso.
Necesidad de dictar por voz con transcripción *local* (sin cloud, privacidad total).

*** Solución: Whisper (OpenAI)

Modelo de transcripción speech-to-text completamente local:
- Modelo =ggml-small.bin= (466 MB, multilíngüe: español + inglés + 97 idiomas)
- *GPU Vulkan*: 0.14x tiempo real (2.6s para transcribir 18.5s de audio!)
- *CPU fallback*: ~4x tiempo real (aceptable para uso ocasional)
- Idioma por defecto: *Español* (5x más rápido que auto-detect)
- Precisión: Excelente en español (capta palabras complejas correctamente)
- Sin internet: 100% local, sin API calls, máxima privacidad

*** Estado Actual

| Máquina | Hardware | Status | Performance |
|---------+----------+--------+-------------|
| aurin | RTX 5080 Vulkan | ✅ PROBADO | 2.6s para 18.5s audio (0.14x) |
| macbook | Intel CPU | ⏳ PENDIENTE | ~35s estimado (4x tiempo real) |
| vespino | RTX 2060 Vulkan | ⏳ PENDIENTE | ~similar a aurin |

*Performance detallado (aurin RTX 5080 + español forzado):*

| Fase | Tiempo |
|------|--------|
| Load model | 447ms |
| Mel spectrogram | 86ms |
| Encode | *22ms* (GPU acelerado) |
| Decode | 8ms |
| Batch decode | 1679ms |
| *TOTAL* | *2.6s* |

*** Arquitectura Clone-First

Patrón: *Clone-First Puro* (sin options por máquina)

#+BEGIN_SRC
dotfiles/scripts/
  ├── whisper-brutal         # Transcripción + stats (español default)
  └── whisper-auto-detect    # Helper: detecta GPU Vulkan vs CPU

modules/home-manager/programs/whisper.nix
  → Instala whisper-cpp + whisper-cpp-vulkan
  → Auto-select mejor backend en runtime (GPU/CPU)
  → Symlinks scripts a ~/.local/bin/
  → NO requiere config por máquina (always enabled)

modules/home-manager/core.nix
  → Import whisper.nix (activo en TODAS las máquinas)
#+END_SRC

*Filosofía:*
- Sin =enable= option (always-on)
- Sin opciones por máquina (auto-detect hardware)
- Mismo código funciona en aurin (GPU), macbook (CPU), vespino (GPU)
- Scripts detectan micrófono automáticamente (RØDE NT-USB Mini o default)

*** Instalación

1. El módulo ya está enabled globalmente (clone-first)
2. Rebuild en cualquier máquina:
   #+BEGIN_SRC bash
   cd ~/dotfiles
   git pull
   sudo nixos-rebuild switch --flake ~/dotfiles#aurin --impure
   #+END_SRC

3. Descargar modelo (primera vez, manual):
   #+BEGIN_SRC bash
   mkdir -p ~/.local/share/whisper/models
   cd ~/.local/share/whisper/models
   curl -L -O https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-small.bin
   #+END_SRC

*Nota:* El modelo NO se descarga automáticamente (466 MB). Es un paso manual para evitar descargas inesperadas en rebuilds.

*** Uso

#+BEGIN_SRC bash
# Transcribir audio por voz
whisper-brutal

# 1. Habla al micrófono (detectado automáticamente)
# 2. Ctrl+C cuando termines de hablar
# 3. Espera transcripción (~2-3s con GPU, ~30s con CPU)
# 4. Resultado en pantalla con stats de performance
#+END_SRC

*Ejemplo de transcripción:*

#+BEGIN_EXAMPLE
🎤 Recording... (Ctrl+C to stop)
^C
Transcribing...

ggml_vulkan: Found 1 Vulkan devices:
ggml_vulkan: 0 = NVIDIA GeForce RTX 5080
whisper_backend_init_gpu: using Vulkan0 backend

 Esta es una prueba de Whisper con GPU Vulkan.
 Funciona muy rápido usando la RTX 5080 de NVIDIA.

whisper_print_timings:     load time =   447 ms
whisper_print_timings:   encode time =    22 ms (GPU accelerated!)
whisper_print_timings:    total time =  2602 ms
#+END_EXAMPLE

*** Detalles Técnicos

**** GPU Detection (Automático)

Scripts detectan hardware en runtime:
- Si Vulkan disponible + GPU NVIDIA/AMD → usa =whisper-cpp-vulkan=
- Si no GPU o solo Intel integrada → usa =whisper-cpp= (CPU)
- Mismo script funciona en todas las máquinas (clone-first)

**** Microphone Detection (Automático)

Scripts detectan micrófono automáticamente:
- Si RØDE NT-USB Mini conectado → lo usa (mejor calidad)
- Si no → usa =default= de PulseAudio/PipeWire
- No requiere config manual

**** Idioma

- *Default:* Español (=-l es=) - 5x más rápido que auto-detect
- Cambiar a inglés: editar =scripts/whisper-brutal= (=-l en=)
- Auto-detect multilenguaje: =-l auto= (más lento pero universal)

**** Modelo

Actualmente: =ggml-small.bin= (466 MB)
- Equilibrio perfecto precisión/velocidad
- Español + inglés excelentes
- 99 idiomas soportados

Alternativas futuras:
- =ggml-base.bin= (142 MB): más rápido, menos preciso
- =ggml-medium.bin= (1.5 GB): más preciso, más lento
- =ggml-large.bin= (2.9 GB): máxima precisión, muy lento

*** Módulo Nix

#+BEGIN_SRC nix
# modules/home-manager/programs/whisper.nix
{ config, lib, pkgs, ... }:

let
  dotfilesDir = "${config.home.homeDirectory}/dotfiles";
in
{
  # Always enabled (clone-first: no options)
  # Auto-detect hardware at runtime

  home.packages = with pkgs; [
    whisper-cpp           # CPU backend (fallback)
    whisper-cpp-vulkan    # GPU Vulkan backend
    sox                   # Audio utilities
  ];

  home.file = {
    ".local/bin/whisper-brutal".source = 
      config.lib.file.mkOutOfStoreSymlink 
        "${dotfilesDir}/scripts/whisper-brutal";
    ".local/bin/whisper-auto-detect".source = 
      config.lib.file.mkOutOfStoreSymlink 
        "${dotfilesDir}/scripts/whisper-auto-detect";
  };
}
#+END_SRC

*** TODO Phase 2 (Futuro)

- [ ] Script =whisper-clipboard= (transcribe + copia al portapapeles)
- [ ] Keybindings XMonad opcionales (Mod+Shift+v)
- [ ] Integration con Emacs (voice-to-text in buffers)
- [ ] Probar modelo =medium= (mejor precisión, ¿aceptable con GPU?)
- [ ] Script de descarga automática de modelos (opcional)

*** Referencias

- Whisper.cpp: https://github.com/ggerganov/whisper.cpp
- Modelos: https://huggingface.co/ggerganov/whisper.cpp
- OpenAI Whisper paper: https://arxiv.org/abs/2212.04356

** Arquitectura Clone-First - Refactorizacion [2026-01-28]

#+BEGIN_QUOTE
*Contexto:* Auditoria del nixos-guru reveló configuraciones que violan la filosofia clone-first.
Hay duplicacion entre hosts que deberia estar en =modules/base/= o =modules/core/=.
#+END_QUOTE

*** Estado de Tareas

| # | Tarea | Estado | Motivo |
|---+-------+--------+--------|
| 1 | Crear =modules/core/firewall.nix= | DONE [2026-01-28] | Puertos dev duplicados en aurin/vespino |
| 2 | Limpiar puertos aurin | DONE [2026-01-28] | Eliminar duplicados, solo dejar específicos |
| 3 | Limpiar puertos vespino | DONE [2026-01-28] | Eliminar duplicados, solo dejar específicos |
| 4 | Crear =modules/services/syncthing.nix= | PENDING | Config casi idéntica en aurin/vespino |
| 5 | Eliminar open-webui | DONE [2026-01-28] | Código muerto eliminado, Docker documentado |
| 6 | Eliminar nsswitch.conf duplicado | PENDING | Idéntico en aurin/vespino, evaluar si va a base |
| 7 | Limpiar paquetes vespino | PENDING | Paquetes que ya están en base o home-manager |
| 8 | Crear =modules/services/ollama.nix= | PENDING (baja prioridad) | Config muy específica por hardware |

*** Tarea 1: firewall.nix - COMPLETADA [2026-01-28]

*Problema:* Puertos de desarrollo duplicados en =hosts/aurin/default.nix= y =hosts/vespino/default.nix=.

*Solución:* Crear =modules/core/firewall.nix= con puertos comunes:
- 22 (SSH), 53/80/443 (DNS, HTTP/S)
- 3000, 4200, 5173, 8000, 8080, 8081, 8888 (dev servers)
- 5900, 5901 (VNC), 8096 (Jellyfin), 22000 (Syncthing), 631 (CUPS)

*Archivos modificados:*
- =modules/core/firewall.nix= (NUEVO)
- =modules/base/default.nix= (añadido import)

*Próximo paso:* Limpiar puertos duplicados de aurin (#2) y vespino (#3).

*** Tarea 2: Limpiar puertos aurin - PENDIENTE

*Motivo:* Después de firewall.nix, aurin solo necesita puertos ESPECÍFICOS:
- 11434 (Ollama - servidor AI principal)
- 8385 (Syncthing GUI custom)
- 34279 (Claude Code)
- 5990-5993 (custom)

*Eliminar:* 22, 53, 80, 443, 8080, 8081, 8000, 8888, 3000, 5900, 5901, 22000, 8096, 631 (ya en base).

*** Tarea 3: Limpiar puertos vespino - PENDIENTE

*Motivo:* Vespino solo necesita puertos ESPECÍFICOS:
- 111, 2049 (NFS - único servidor NFS)
- 8384 (Syncthing GUI custom)
- Minecraft abre sus puertos via módulo

*** Tarea 4: Syncthing modular - PENDIENTE

*Motivo:* Config casi idéntica en aurin (líneas 429-466) y vespino (líneas 243-277).
Mismos devices (vespino, aurin, cohete, pocapullos), mismo folder "org".

*Solución:* Módulo =modules/services/syncthing.nix= con:
- Devices compartidos
- Folder "org" común
- Opción =guiPort= configurable por host

*** Tarea 5: Eliminar open-webui - COMPLETADA [2026-01-28]

*Problema:* Código muerto (=enable = false=) en aurin y vespino.
Paquete roto en nixpkgs-unstable por fallo de build en ctranslate2.

*Solución:* Eliminado código, documentado Docker como alternativa:
#+BEGIN_SRC bash
docker run -d -p 3000:8080 --add-host=host.docker.internal:host-gateway \
  -v open-webui:/app/backend/data \
  -e OLLAMA_BASE_URL=http://host.docker.internal:11434 \
  --name open-webui ghcr.io/open-webui/open-webui:main
#+END_SRC

*** Tarea 6: nsswitch.conf - PENDIENTE (evaluar)

*Motivo:* Idéntico en aurin y vespino. Opciones:
1. Mover a =modules/core/= si todas las máquinas lo necesitan
2. Dejarlo solo en aurin si es específico de VPN Vocento
3. Ya existe en =vocento-vpn-bridge.nix= - eliminar duplicados

*** Tarea 7: Paquetes vespino - PENDIENTE

*Motivo:* =environment.systemPackages= en vespino incluye paquetes que ya están en base:
- xorg.* (setxkbmap, xmodmap, xinput, xset)
- Audio: alsa-utils, pavucontrol (ya via pipewire)
- dunst, libnotify, xclip (deberían estar en home-manager o base)

*** Tarea 8: Ollama modular - BAJA PRIORIDAD

*Motivo:* Config muy específica por hardware (GPU layers, CUDA memory, threads).
Beneficio de modularizar es menor porque los valores son muy diferentes entre máquinas.

** Limpieza
- [ ] Revisar y limpiar configs obsoletas (shell/, composer/)
- [ ] Documentar scripts utiles en scripts/
- [X] Migrar xmonad a home-manager - DONE (Phase 2)
- [ ] Eliminar stow cuando composer migre (CRÍTICO: composer en PROD Vocento)

* Apps Utiles de GNOME y KDE (para usar en XMonad)

Al tener GNOME y KDE Plasma instalados, tenemos acceso a todas sus aplicaciones
que funcionan perfectamente en XMonad u otros WMs minimalistas.

** Configuracion del Sistema

| App                        | DE    | Comando                    | Para que                              |
|----------------------------+-------+----------------------------+---------------------------------------|
| GNOME Settings             | GNOME | =gnome-control-center=     | Config general del sistema            |
| KDE System Settings        | KDE   | =systemsettings=           | Config general del sistema            |
| Display/Monitors           | -     | =arandr=                   | Configurar monitores (xrandr GUI)     |
| Network Connections        | GNOME | =nm-connection-editor=     | WiFi, VPN, proxies                    |
| Bluetooth                  | -     | =blueman-manager=          | Dispositivos Bluetooth                |
| Audio/Volume               | -     | =pavucontrol=              | Mezclador de audio detallado          |
| Printers                   | GNOME | =system-config-printer=    | Configurar impresoras                 |
| Users                      | GNOME | =gnome-control-center user-accounts= | Gestion de usuarios        |
| Date & Time                | GNOME | =gnome-control-center datetime= | Zona horaria, NTP               |
| Power                      | GNOME | =gnome-control-center power= | Energia, suspension                 |
| KDE Connect                | KDE   | =kdeconnect-app=           | Conectar movil al PC                  |

** Gestion de Archivos

| App                        | DE    | Comando                    | Para que                              |
|----------------------------+-------+----------------------------+---------------------------------------|
| Dolphin                    | KDE   | =dolphin=                  | File manager muy potente              |
| Nautilus                   | GNOME | =nautilus=                 | File manager GNOME                    |
| Thunar                     | XFCE  | =thunar=                   | File manager ligero                   |
| Disks                      | GNOME | =gnome-disks=              | Particiones, montar discos, SMART     |
| KDE Partition Manager      | KDE   | =partitionmanager=         | Particiones avanzado                  |
| Filelight                  | KDE   | =filelight=                | Uso de disco visual (sunburst)        |
| Baobab                     | GNOME | =baobab=                   | Uso de disco visual                   |

** Multimedia

| App                        | DE    | Comando                    | Para que                              |
|----------------------------+-------+----------------------------+---------------------------------------|
| Gwenview                   | KDE   | =gwenview=                 | Visor imagenes con edicion basica     |
| Eye of GNOME               | GNOME | =eog=                      | Visor imagenes simple                 |
| Okular                     | KDE   | =okular=                   | PDFs, ePub, comics, anotaciones       |
| Evince                     | GNOME | =evince=                   | PDFs simple                           |
| Totem                      | GNOME | =totem=                    | Reproductor video                     |
| Elisa                      | KDE   | =elisa=                    | Reproductor musica                    |
| Rhythmbox                  | GNOME | =rhythmbox=                | Reproductor musica + podcasts         |

** Screenshots y Grabacion

| App                        | DE    | Comando                    | Para que                              |
|----------------------------+-------+----------------------------+---------------------------------------|
| Spectacle                  | KDE   | =spectacle=                | Screenshots con anotaciones           |
| GNOME Screenshot           | GNOME | =gnome-screenshot=         | Screenshots basico                    |
| Flameshot                  | -     | =flameshot gui=            | Screenshots con edicion               |
| OBS Studio                 | -     | =obs=                      | Grabacion/streaming profesional       |
| Peek                       | -     | =peek=                     | GIFs animados                         |

** Editores y Documentos

| App                        | DE    | Comando                    | Para que                              |
|----------------------------+-------+----------------------------+---------------------------------------|
| Kate                       | KDE   | =kate=                     | Editor texto avanzado                 |
| GNOME Text Editor          | GNOME | =gnome-text-editor=        | Editor texto simple                   |
| Gedit                      | GNOME | =gedit=                    | Editor texto clasico                  |
| LibreOffice                | -     | =libreoffice=              | Suite offimatica completa             |
| Apostrophe                 | GNOME | =apostrophe=               | Editor Markdown                       |

** Utilidades

| App                        | DE    | Comando                    | Para que                              |
|----------------------------+-------+----------------------------+---------------------------------------|
| Calculator                 | GNOME | =gnome-calculator=         | Calculadora cientifica                |
| KCalc                      | KDE   | =kcalc=                    | Calculadora                           |
| System Monitor             | GNOME | =gnome-system-monitor=     | Procesos, CPU, memoria                |
| KSysGuard                  | KDE   | =ksysguard=                | Monitor sistema clasico               |
| Plasma System Monitor      | KDE   | =plasma-systemmonitor=     | Monitor sistema moderno               |
| Characters                 | GNOME | =gnome-characters=         | Mapa de caracteres, emojis            |
| KCharSelect                | KDE   | =kcharselect=              | Mapa de caracteres                    |
| Fonts                      | GNOME | =gnome-font-viewer=        | Visor de fuentes                      |
| Archive Manager            | GNOME | =file-roller=              | Comprimir/descomprimir                |
| Ark                        | KDE   | =ark=                      | Comprimir/descomprimir                |
| Secrets (Keyring)          | GNOME | =gnome-keyring=            | Gestor de passwords del sistema       |
| KWallet                    | KDE   | =kwalletmanager5=          | Gestor de passwords del sistema       |

** Desarrollo

| App                        | DE    | Comando                    | Para que                              |
|----------------------------+-------+----------------------------+---------------------------------------|
| D-Spy                      | GNOME | =d-spy=                    | Explorador D-Bus                      |
| Qt Designer                | KDE   | =designer=                 | Disenar interfaces Qt                 |
| Glade                      | GNOME | =glade=                    | Disenar interfaces GTK                |

** Herramientas CLI utiles (bonus)

| Comando                    | Para que                                          |
|----------------------------+---------------------------------------------------|
| =xrandr=                   | Configurar monitores desde terminal               |
| =autorandr=                | Perfiles de monitor automaticos                   |
| =xdotool=                  | Automatizar clicks y teclas                       |
| =xclip= / =xsel=           | Clipboard desde terminal                          |
| =wl-copy= / =wl-paste=     | Clipboard Wayland                                 |
| =notify-send=              | Enviar notificaciones                             |
| =pactl=                    | Control PulseAudio/PipeWire                       |
| =playerctl=                | Controlar reproductores multimedia                |
| =brightnessctl=            | Controlar brillo de pantalla                      |
| =nmcli=                    | NetworkManager desde terminal                     |
| =bluetoothctl=             | Bluetooth desde terminal                          |

** Nota sobre instalacion

La mayoria de estas apps ya estan instaladas al tener GNOME y KDE Plasma habilitados.
Si alguna falta, se puede instalar temporalmente con:

#+begin_src bash
nix-shell -p gnome.gnome-disk-utility  # Ejemplo
#+end_src

O anadirla permanentemente en =modules/core/packages.nix=.

* Referencias

- [[https://nixos.org/manual/nixos/stable/][NixOS Manual]]
- [[https://nix-community.github.io/home-manager/][Home Manager Manual]]
- [[https://xmonad.org/][XMonad Documentation]]
- [[https://www.gnu.org/software/stow/manual/stow.html][GNU Stow Manual]]
- [[https://github.com/alacritty/alacritty][Alacritty]]
- [[https://wiki.archlinux.org/title/Picom][Picom Wiki]]
- [[file:nixos-aurin/README.org][NixOS Aurin Specific Docs]]

---

*Ultima actualizacion:* 2026-01-24
*Arquitectura:* Clone-First (mkSystem)
*Display Manager:* SDDM (todas las maquinas)
*Sistemas:* Aurin, MacBook, Vespino (NixOS 25.05)
*Shell principal:* Fish
*Window Manager:* XMonad (X11), Hyprland/niri (Wayland)
*Terminal:* Alacritty

*Documentacion adicional:*
- [[file:docs/MACBOOK-INSTALL-GUIDE.org][Guia instalacion MacBook]] - Paso a paso para instalar NixOS en MacBook Pro 2016
- [[file:CLAUDE.md][CLAUDE.md]] - Instrucciones rapidas para Claude AI
