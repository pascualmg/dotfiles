...
*Status bar* para XMonad con soporte para modo split (dos barras).
...
*** Modo Split (actual en MacBook)

#+BEGIN_SRC text
┌─────────────────────────────────────────────────────────────┐
│ 🕐 Hora │ [1] 2 3 4 │ Layout │ Título ventana              │  ← Arriba (workspaces)
└─────────────────────────────────────────────────────────────┘
                    ... contenido ...
┌─────────────────────────────────────────────────────────────┐
│                    Docker Vol Bat ⌨HHKB Red Disk GPU RAM CPU│  ← Abajo (monitores)
└─────────────────────────────────────────────────────────────┘
#+END_SRC
...
| =xmobar-monitors.hs= | Bottom | CPU, RAM, GPU, Red, Batería, HHKB, etc. |
| =xmobarrc= | (Full) | Todo junto (generado por home-manager) |

*** Configuración en xmonad.hs

#+BEGIN_SRC haskell
main = do
    -- MODO SPLIT: Dos barras
    -- Workspaces (arriba) - recibe info de XMonad via pipe
    ...
    -- Monitor secundario (full)
    xmproc1 <- spawnPipe "xmobar -x 1"
#+END_SRC

*** Monitores disponibles

| Monitor | Script | Descripción |
|---------+--------+-------------|
| CPU | =xmobar-cpu.sh= | Uso %, temperatura, consumo W |
| CPU Freq | =xmobar-cpu-freq.sh= | Frecuencia y governor |
| RAM | =xmobar-memory.sh= | Uso con gradiente de color |
| GPU | =xmobar-gpu-nvidia.sh= / =intel.sh= | Temp, uso, VRAM |
| Red | =xmobar-network.sh= | IP, velocidad (auto-detecta eth/wifi) |
| Discos | =xmobar-disks.sh= | Uso NVMe/SATA/USB |
| Batería | =xmobar-battery.sh= | Carga con icono y color |
| HHKB | =xmobar-hhkb-battery.sh= | Batería teclado Bluetooth |
| Docker | =xmobar-docker.sh= | Containers activos |
| Volumen | =xmobar-volume.sh= | Nivel con gradiente |
...
| Maquina | DM | Sesiones disponibles | Estado |
|---------+----+----------------------+--------|
| macbook | SDDM | XMonad, GNOME, KDE, Hyprland, niri | OK (probado) |
| aurin   | SDDM | XMonad, GNOME, KDE, Hyprland, niri | OK (probado) |
| vespino | SDDM | XMonad, GNOME, KDE, Hyprland, niri | Pendiente test |

*** NVIDIA + Wayland (aurin, vespino)

Para Wayland en NVIDIA, las variables de entorno van en =hardware/nvidia/*.nix=:

#+BEGIN_SRC nix
environment.sessionVariables = {
  GBM_BACKEND = "nvidia-drm";
  __GLX_VENDOR_LIBRARY_NAME = "nvidia";
  WLR_NO_HARDWARE_CURSORS = "1";  # Necesario para wlroots (Hyprland)
};
#+END_SRC
