...
** Problema Conocido: xmobar y fuentes HiDPI

*Estado:* SIN RESOLVER

En pantalla Retina (2560x1600, Xft.dpi=227), el parametro =size=X= en la
especificacion de fuente xft NO cambia el tamano visual en xmobar.

| Probado                                  | Resultado   |
|------------------------------------------+-------------|
| =xft:Monoid Nerd Font:size=54:bold=      | No cambia   |
| =pixelsize=64= en vez de =size=          | No cambia   |
| Config minimalista (solo =Run Date=)     | SI funciona |

*Hipotesis:* Algo en el config completo de xmobar interfiere (templates con
=<fn=1>=, StdinReader, o interaccion Xft.dpi/xmobar).

*Workaround actual:* GNOME con GDK_SCALE=2 funciona bien. xmonad en Retina
usa fuentes diminutas pero legibles.

*Alternativas a investigar:* polybar, lemonbar, eww.
