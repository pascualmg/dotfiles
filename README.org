...
* Live ISO - Tu Entorno Booteable

** Que es

Una ISO de NixOS con TODO tu entorno pre-instalado:
- XMonad + xmobar configurados
- Fish shell con tus aliases
- Todos tus paquetes (200+)
- Alacritty, doom-emacs, neovim...
- Git + flake listo para instalar en cualquier maquina
...
# Generar ISO (tarda unos minutos la primera vez)
nix build .#nixosConfigurations.live.config.system.build.isoImage

# Resultado
ls -lh result/iso/
# nixos-passh-live-25.05.*-x86_64-linux.iso
#+END_SRC
...
#+BEGIN_SRC bash
# Si tienes Ventoy en el USB, solo copia la ISO
cp result/iso/nixos-*.iso /run/media/passh/Ventoy/
#+END_SRC

Ventoy permite tener MULTIPLES ISOs en un solo USB.

*** Opcion 2: dd (USB dedicado)

#+BEGIN_SRC bash
# CUIDADO: Esto borra todo el USB
# Verificar dispositivo con lsblk primero!
...
2. Seleccionar "NixOS Live"
3. Tu entorno XMonad arranca automaticamente
4. Usuario: =passh= (sin password)
...
# 2. Particionar disco (ejemplo: GPT + EFI)
...
# 5. Generar hardware-configuration.nix
nixos-generate-config --root /mnt

# 6. Instalar desde flake (elige tu hostname)
nixos-install --flake github:pascualmg/dotfiles#aurin
# o
nixos-install --flake github:pascualmg/dotfiles#macbook
# o
nixos-install --flake github:pascualmg/dotfiles#vespino
...
flake.nix
└── nixosConfigurations.live   # Entry point
#+END_SRC
...
|----------------+-------------|
| Portable | Tu entorno en cualquier PC |
| Rescue | Recuperar sistemas rotos |
| Demo | Mostrar tu setup a otros |
| Install | Instalador personalizado |
| Ventoy | Compatible con multi-boot |
| Offline | Todo pre-descargado |

** Tamano aproximado

- ISO comprimida: ~3-4 GB
- USB minimo: 8 GB (16 GB recomendado con Ventoy)
