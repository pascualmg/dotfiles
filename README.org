...
#+BEGIN_SRC text
dotfiles/
├── flake.nix         ★ Entry point NixOS (multi-máquina)
├── modules/          ★ Módulos NixOS compartidos
...
** Arquitectura de Módulos NixOS

Filosofía: *módulos compartidos* con *opciones configurables* por máquina.

#+BEGIN_SRC text
modules/
├── common/
│   ├── packages.nix      # Paquetes compartidos (todas las máquinas)
...
# En nixos-aurin/configuration.nix - usa el módulo
...
*Hardware de Input - Filosofía HHKB:*
El módulo =desktop/xmonad.nix= incluye =libinput= con perfil =flat= (raw input).
El hardware manda, no el software. Ratones gaming usan su DPI sin aceleración del sistema.
